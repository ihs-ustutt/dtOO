...
class radMeridional_hubZero:
    ...
    def create(self,) -> dtOO.dtBundle:
        ...
        # Measurements of the machine in meters and degree
        d_inlet = 2.58
        l_inExt = 0.50
        d_outHub = 0.4
        d_outShroud = 1.865
        ...
        radMeridionalContour = analyticGeometryLayers_piecewiseMeridionalRotContour( 
          ...
          #hub_splits = [ [], [], [], [],],         # splits at 0 are allowed splits at 1 not  
          #shroud_splits = [ [], [], [], []],
          layer_thickness = 0.2,
          ...
          interface_curvature = [[0.0, 0.5, 1],
                                 #[0.3, 0.5, 1],
                                 [0.2, 0.4, -1],],# [curvature offset percent, hub to shroud percent, direction]
          #interface_hub = [],
          #interface_shroud = [],
          #interface_curvature = [],
        ...
        #)
        #container = rotatingMap2dTo3d_splitFitted # splits chanel into segments
        #  label = "xyz_fitChannel", 
        #  channel = aG["xyz_channel"], 
        #  internals = blocks, #[aG["xyz_meshBlock"],], 
        #  splitDir = 1,
        #  segmentDir = 2,                               # direction of segmentat (2->spanwise (along LE / TE))
        ...
        #  dtOO.map3dTo3d.MustDownCast( blocks[0] ).segmentConstUPercent( 0.0 ) # first block, face at U = 0 
        #)
        #for block in blocks:
        #  couplingFaces.append( 
        ...
        #  )
        #couplingFaces.append( 
        #  dtOO.map3dTo3d.MustDownCast( blocks[-1] ).segmentConstUPercent( 1.0 ) # last block, face at U = 1
        #)
        #from dtOOPythonApp.builder import (
