...
class radMeridional:
    ...
    The meridional contour of the flow machine is created with the method 
    :meth:`createMeridional` through its hub and shroud curves.
    ...
    .. _ruMesh0:
    .. figure:: bladeFigs/runner_mesh.png
       :width: 95%
       :align: center

       Mesh of the runner segment.
    ...
    .. figure:: meridionalFigs/layersMesh.png
       ...
       Mesh of a draft tube cone segment.
    ...
    def createMeridional(self, configM, hubCurves, shroudCurves): 
        ...
        the hub and shroud curves, which are not part of the regular channels.
        ...
        `radMeridional` class. Through this object, the regular channels and the layer 
        region curves can be accessed in the other methods.
        """
    ...
    def createLayerRegion(self, configL):
        ...
        .. _createLayerRegion:
        .. figure:: meridionalFigs/createLayerRegion.png
           :width: 50%
           :align: center

           Activity diagram of the method :meth:`createLayerRegion`. Solid lines indicate the
           sequence of activities, while dashed lines indicate the flow of objects
           between activities.
        ...
        **Get Layer Region Curves**
            ...
            The basis for the draft tube cone geometry are the hub and shroud curves,
            as well as the inlet and outlet curves of the flow domain.
            These curves are returned by the object ``radMeridionaContour`` of the class
            `analyticGeometry_piecewiseMeridionalRotContour`, with the getter method 
            ``getLayerRegionCurves()``.

            :numref:`speCurves` shows the curves that are passed to the
            :meth:`createLayerRegion` method from the object created in :meth:`createMeridional`.
            ...
            .. figure:: meridionalFigs/speCurves.png
               ...
               Hub (speHub) and Shroud (speShroud) curves (black),
        ...
        **Build Layer Region Geometry**
            ...
            curves and then rotated to generate volumes.
            ...
            The generation of the bounding curves on the hub layer is shown in
        ...
        **Get Layer List**
            ...
            a nested list ``layers`` containing the layer volumes and information indicating which 
        ...
        **Get Multiple Bounded Volume and Bounding Faces**
            ...
            With the class method `getUnstructuredRegion`, the multiple bounded volume ``mv`` 
            and a list of its bounding surfaces ``bs`` can be returned.
    ...
    def createBlade(self, configB):
        ...
        - Maps the blade, the mesh blocks and the meanplanes onto a regular channel returned by the
        ...
        stored in the containers instantiated by this class. The objects can later be
        retrieved from the containers using their labels. The following activity diagram
        shows the operations performed and the flow of geometries from the operations
        where they are created to the operations where they are used.

        .. _createBlade_activity:
        .. figure:: bladeFigs/createBlade.png
           :width: 100%
           :align: center

           Activity diagram of the method :meth:`createBlade`. Solid lines indicate the
           sequence of activities, while dashed lines indicate the flow of geometries
           between activities.
        ...
        this method receive the string value stored under ``label`` as part of their
        physical names.

        **Get the Regular Channel**

            The key ``regChannel`` in the dictionary specifies the regular channel in
            which the blade is created. The regular channel is returned by the
            ``getRegChannel()`` method of the ``radMeridionalContour`` object of class
            `analyticGeometry_piecewiseMeridionalRotContour`. The returned channel volume
        ...
        **Create Blade's Thickness Distribution**
        ...
        list of `dtPoint2` objects and the function order as input.

        To create these inputs, the parameter lists are passed to the method
        :meth:`fillInputList`. This method creates a list of `dtPoint2` objects, which
        contain the parameter values from the input list and the corresponding
        percentage of the blade span at which those values are applied. Furthermore,
        the function order is defined in this method.
        ...
        **Create Conformal Mapping Object**
            ...
            To map the geometries created in parameter space into the regular channel,
            a mapping object of type `uVw_phirMs` is prepared. Its settings are applied
            through a `jsonPrimitive` object. The mapping object is labeled
            ``"uVw_phirMs"``.
        ...
        **Mesh Block Volumes and FE-Meanplane Curves**
            ...
            **Create Mesh Block Volumes**
                ...
                Depending on the number of splits and their positions, specified by the
                input variable ``splits``, multiple mesh blocks are created. The volumes
                are labeled using ``label + "_meshBlock"`` with an integer suffix.

                The numbering sequence of the mesh blocks follows the direction of the
                u-parameter of the blade surface, starting with the first mesh block at
                u = 0% and ending with the last mesh block at u = 100%.

                The first mesh block surrounding the blade is labeled with the suffix
                ``_1`` and the n-th mesh block with the suffix ``_n``. The split
            ...
            **Create FE-Meanplane Curves**
                ...
                The suffix ``<in/out>`` indicates whether the curves are offset toward
                the inlet or the outlet. The suffix ``<0/1>`` indicates whether the
                curve is the base curve on the mesh block (``0``) or the offset curve
                (``1``).

                The offset meanplane curves and their base curves are shown in the
                following figure.
                ...
                .. figure:: bladeFigs/meanplane_CurvesPushed.png
                   ...
                   Offset and base meanplane curves. The labels ``in0``, ``in1``,
                   ``out0``, and ``out1`` correspond to the established naming
                   convention.

                If the Boolean input variable ``meanplaneFromBlocks`` is ``False``,
                these curves are not created.

        **Create FE-Meanplane Surfaces Extending from the Mesh Blocks**

            A FE-meanplane surfaces extending from the mesh blocks are created by
            skinning the meanplane curves.

            The skinning direction corresponds to the upstream direction of the
            regular channel in order to maintain consistent parameter directions
            with the mesh blocks.
            ...
            The suffix ``<in/out>`` indicates whether the surface belongs to the
            meanplane extending toward the inlet or the outlet.

            The creation of the extending meanplane surfaces is shown in
            :numref:`feMean0`.
            ...
            .. figure:: bladeFigs/guideVane_feMeanplane0.png
               ...
               Creation of the meanplane surfaces with the suffixes ``_out0`` and
               ``_in0``. The offset meanplane curves are shown in blue.
        ...
        **Create the FE-Meanplane Surfaces Connecting to the Interfaces**
            ...
            Two meanplane surfaces that connect the inlet and outlet interfaces to
            the tangentially offset meanplane curves are created using the subclass
            ...
            The following figure shows the four FE-meanplane surfaces together with
            their label suffixes.
        ...
        **Create Lists of Boundary Faces**
            ...
            number of mesh blocks that are part of the meanplane. Based on the current
            iterator position, the corresponding faces are appended to the two lists.

            The following figure shows the surfaces contained in ``meanplaneFaces`` and
            ``couplingFaces``.

            .. _meanplanesAndCouplings0:
        ...
        **Create Grid Channel**
            ...
            The input key ``orientation`` is passed to the constructor to account for
            the orientation of the blade within the regular channel. If the blade is
            oriented in the positive u-direction of the regular channel, the value is
            ``1``. If the blade extends in the negative u-direction, the value is
            ``-1``.

            :numref:`bounds0` shows the bounding surfaces of the grid channel and the
            mesh blocks surrounding the blade.
            ...
            .. figure:: bladeFigs/gidChannel_bound.png
               ...
               Bounding surfaces of the grid channel (colored). Bounding curves of the
               hub surface are shown in pink. The blade (grey) is shown for reference.

            The grid channel is bounded by the coupling faces (cyan) and the meanplane
            surfaces created as extensions of the mesh blocks (:numref:`feMean`,
            yellow) on one side of the channel. On the opposite side, it is bounded by
        ...
        **Create the Mesh Topology**
            ...
            A mesh resulting from this topology is shown in :numref:`gvMesh0`.
        """
