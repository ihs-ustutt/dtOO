...
namespace dtOO {
bool ofOpenFOAMMixingPlaneRule::_registrated = ofOpenFOAMSetupRule::registrate(
...
std::vector<std::string> ofOpenFOAMMixingPlaneRule::factoryAlias(void) const
{
...
) const
{
  ...
  ::Foam::mixingInterfacePolyPatch *mix0 = new ::Foam::mixingInterfacePolyPatch(
    bM[id0].name(),
    dd0,
    bM[id0].index(),
    bM[id0].boundaryMesh(),
  ...
  ::Foam::mixingInterfacePolyPatch *mix1 = new ::Foam::mixingInterfacePolyPatch(
    bM[id1].name(),
    dd1,
    bM[id1].index(),
    bM[id1].boundaryMesh(),
    ...
    dt__forAllIndex(options, ii)
    {
      ...
      else
      {
        ofOpenFOAMSetupRule::executeOnVolVectorField(
          ::boost::assign::list_of(rule[0])(csvString[ii])("")(field.name())(
            options[ii]
          ),
          field
        );
