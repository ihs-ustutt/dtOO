...
#include "ofOpenFOAMCylindricalInletRule.h"
...
namespace dtOO {
bool ofOpenFOAMCylindricalInletRule::_registrated =
  ofOpenFOAMSetupRule::registrate(dt__tmpPtr(
    ofOpenFOAMCylindricalInletRule, new ofOpenFOAMCylindricalInletRule()
  ));

ofOpenFOAMCylindricalInletRule::ofOpenFOAMCylindricalInletRule() {}

ofOpenFOAMCylindricalInletRule::~ofOpenFOAMCylindricalInletRule() {}

std::vector<std::string> ofOpenFOAMCylindricalInletRule::factoryAlias(void
) const
{
  return ::boost::assign::list_of("OpenFOAMCylindricalInletRule");
}

void ofOpenFOAMCylindricalInletRule::executeOnVolVectorField(
...
) const
{
  ...
  forAll(bF, i)
  {
    ...
    if (field.mesh().boundary()[i].name() == rule[1])
    {
      ...
      if (stringPrimitive::stringContains("rotationAxis", thisRule) &&
          stringPrimitive::stringContains("origin", thisRule))
      {
        ...
        if (stringPrimitive::stringContains("value", thisRule))
        {
          ...
          {
            bF[i][j] =
              ::Foam::vector(tangVelo[j] + hatAxis * axial + radial * dhat[j]);
          }

          return;
        }
        else if (stringPrimitive::stringContains("function", thisRule))
        {
          //
          // scale r and z
          //
          ...
          dt__ptrAss(
            ...
            vec3dThreeD::ConstDownCast(ofOpenFOAMSetupRule::refAF().get(
          ...
          // reference axis
          //
          ::Foam::vector refAxis = parseOptionVector("referenceAxis", thisRule);
          ...
          ::Foam::vector ref2Axis = (z_ ^ refAxis) / ::Foam::mag(z_ ^ refAxis);
          ...
          if (stringPrimitive::stringContains("swirlFlowZ", thisRule))
          {
            ...
            else
            {
              ...
              if (targetSwirlFlowZ == 0.)
                swirlFlow.replace(2, 1.);
              s_cT =
                (targetSwirlFlowZ / swirlFlow.z()) / (targetVolFlow / volFlow);
            }
          }
