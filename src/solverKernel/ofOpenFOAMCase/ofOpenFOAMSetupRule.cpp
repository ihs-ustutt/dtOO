...
namespace dtOO {
  ...
  bool ofOpenFOAMSetupRule::_registrated 
  =
  ...
  std::vector< std::string > ofOpenFOAMSetupRule::factoryAlias( void ) const {
    return std::vector< std::string >();//::boost::assign::list_of("OpenFOAMSetupRule");
  }  
  
  bool ofOpenFOAMSetupRule::registrate( 
    ofOpenFOAMSetupRule const * const rule 
  ) {
  ...
  ofOpenFOAMSetupRule * ofOpenFOAMSetupRule::create( 
    ...
    forAll(bF, i) {
      ...
      if ( field.mesh().boundary()[i].name() ==  rule[1] ) {
        ...
        if ( 
          ...
          bF.set(
            i, 
            new ::Foam::turbulentMixingLengthDissipationRateInletFvPatchScalarField(
                field.mesh().boundary()[i], 
                field, 
                parseOptionDict(
  ...
  std::vector< std::string > ofOpenFOAMSetupRule::optionVector(
    std::string const & str
  ) {
    return stringPrimitive::convertToCSVStringVector(str);
  }  
  
  std::vector< std::string > ofOpenFOAMSetupRule::availableOptions(
    std::string const & str
  ) {
    std::vector< std::string > names;
    dt__forAllRefAuto( optionVector(str), aOpt ) {
      names.push_back(
        stringPrimitive::getStringBetween( "", "(", aOpt )
      );
    }
    return names;
  }
