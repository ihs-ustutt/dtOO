...
bool ofOpenFOAMCase::_registrated =
  dtCaseFactory::registrate(dt__tmpPtr(ofOpenFOAMCase, new ofOpenFOAMCase()));
...
)
{
  ...
  // get boundedVolumes
  //
  _bV = boundedVolume::VectorCastConst(jE.lookupVecRaw<boundedVolume>("", bV));
}
...
)
{
  ...
  {
    dt__warning(init(), << "noWriteRule will be ignored!");
  }
  ...
  {
    bVVec.push_back(bV->get(dtXmlParser::getAttributeStr("label", aChild)));
  }
...
)
{
  ...
  dt__forAllRefAuto(_bV, aBV)
  {
    ...
    dt__forAllRefAuto(entities, anEntity)
    {
      ...
      dtInt locPhysInt =
        gm->getPhysicalNumber(dimension, gm->getPhysicalString(anEntity));
      dtInt globPhysInt = -1;
      // check if  it is already in map
      std::map<std::tuple<dtGmshModel *, int, dtInt>, dtInt>::iterator
        thisPhysical = globLoc.find(
      ...
      if (thisPhysical == globLoc.end())
      {
        ...
        globLoc[std::tuple<dtGmshModel *, int, dtInt>(
          gm, dimension, locPhysInt
        )] = globPhysInt;
      ...
      // in map
        ...
        globPhysInt = thisPhysical->second;
      ...
      {
        allElems[eC] = std::pair<::MElement *, dtInt>(
...
void ofOpenFOAMCase::runCurrentState(void)
{
  ...
  std::string wDir = dtCase::getDirectory(cState);
  ...
  dt__onlyMaster
  {
    ...
    try
    {
      ...
      // create mesh
      //
      dt__pH(::Foam::polyMesh) mesh(dtFoamLibrary::readMesh(
        allVerts,
        allElems,
        physicalNames,
        runTime,
      ...
      dt__forAllRefAuto(_fieldRule, aRule)
      {
        ...
        if (aRule[0] == "volVectorField")
        {
          ...
          volVector_.push_back(::Foam::volVectorField(
            ::Foam::IOobject(
              aRule[1],
              runTime.timeName(),
              runTime,
          ...
          if (aRule.size() == 4)
          {
            volVector_.back().ref() = ::Foam::dimensionedVector(
              ::Foam::dimensionSet(::Foam::IStringStream(aRule[2])()),
        ...
        {
          volScalar.push_back(::Foam::volScalarField(
            ::Foam::IOobject(
              aRule[1],
              runTime.timeName(),
              runTime,
          ...
          if (aRule.size() == 4)
          {
            volScalar.back().ref() = ::Foam::dimensionedScalar(
              ::Foam::dimensionSet(::Foam::IStringStream(aRule[2])()),
