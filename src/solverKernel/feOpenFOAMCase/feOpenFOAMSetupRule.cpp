...
bool feOpenFOAMSetupRule::_registrated = feOpenFOAMSetupRule::registrate(
...
bool feOpenFOAMSetupRule::registrate(feOpenFOAMSetupRule const *const rule)
{
...
feOpenFOAMSetupRule *feOpenFOAMSetupRule::create(std::string const &str)
{
...
) const
{
  ...
  forAll(bF, i)
  {
    ...
    if (field.mesh().boundary()[i].name() == rule[1])
    {
      ...
      if (stringPrimitive::getStringBetween("", "(", thisRule) == "fixedValue")
      {
        //
      ...
      else if (stringPrimitive::getStringBetween("", "(", thisRule) ==
               "zeroGradient")
      {
        //
        // create and set new patch
      ...
      else if (stringPrimitive::getStringBetween("", "(", thisRule) == "slip")
      {
        //
        // create and set new patch
        //
  ...
  forAll(bF, i)
  {
    ...
    if (field.mesh().boundary()[i].name() == rule[1])
    {
               ...
               "kqRWallFunction")
      {
        ...
        bF.set(
          i,
          new ::Foam::incompressible::RASModels ::kqRWallFunctionFvPatchField<
            ::Foam::scalar>(
            field.mesh().boundary()[i],
            field,
...
lvH_analyticFunction const &feOpenFOAMSetupRule::refAF(void) const
{
  return *_aF;
}
} // namespace dtOO
