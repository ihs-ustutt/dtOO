...
bool feOpenFOAMCase::_registrated =
  dtCaseFactory::registrate(dt__tmpPtr(feOpenFOAMCase, new feOpenFOAMCase()));
...
)
{
  ...
  // get dict rules
  //
  _dictRule = dtXmlParser::replaceDependencies(
  ...
  dt__forAllIndex(tmpSetupRule, ii)
  {
    ...
    dt__info(init(), << "feOpenFOAMSetupRule[ " << ii << " ] = " << ruleStrVec);
  }
  ...
  {
    _bV.push_back(bV->get(dtXmlParser::getAttributeStr("label", aChild)));
  }
...
void feOpenFOAMCase::runCurrentState(void)
{
  ...
  //
  if (inPipeline(dtXmlParser::constReference().currentState()))
    return;
