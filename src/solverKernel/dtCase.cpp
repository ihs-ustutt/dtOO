...
)
{
  ...
  //
  labelHandling::setLabel(dtXmlParserBase::getAttributeStr("label", element));
...
void dtCase::runCurrentState(void) { dt__throwUnexpected(runCurrentState()); }

std::string dtCase::createDirectory(std::string const &state) const
{
  std::string wDir = getDirectory(state);
...
jsonPrimitive &dtCase::config(void) { return _config; }

jsonPrimitive const &dtCase::config(void) const { return _config; }
} // namespace dtOO
