...
std::string qtXmlPrimitive::convertToString(::QDomNode const node)
{
  ...
  else
  {
    ...
    for (int ii = 0; ii < attrs.size(); ii++)
    {
      ...
      if (!attr.isNull())
      {
        head << attr.name().toStdString() << "=" << attr.value().toStdString()
             << std::endl;
      }
    }
    retStr = head.str();
...
qtXmlPrimitive::getSiblingVector(::QDomElement const element)
{
  std::vector<::QDomElement> sibVec;
  bool goBack = false;
...
std::vector<::QDomElement>
qtXmlPrimitive::getChildVector(::QDomElement const element)
{
...
::QDomElement
qtXmlPrimitive::getChild(std::string const sibName, ::QDomElement const element)
{
  bool goBack;

  dt__pH(::QDomElement)
    tmpElementP(new ::QDomElement(element.firstChildElement()));
...
)
{
  ...
  dt__throw(
    getChildByName(),
    << "There is no child " << sibName << " with " << attributeName << " "
    << attributeValue
  );
}
...
)
{
  ...
  dt__throw(
    getChildElement(),
    << "No " << elementTag << "-Element with " << dt__eval(labelAttributeVal)
  );
}
...
)
{
  ...
  while ((tmpElement.tagName().toStdString() != sibName) &&
         (!tmpElement.isNull()))
  {
...
)
{
  if (stringContains("|", attName))
  {
    ...
    {
      if (std::find(existing.begin(), existing.end(), anAtt) == existing.end())
        return false;
    }
...
)
{
  ...
  {
    str = stringPrimitive::replaceStringInString(
...
std::string qtXmlPrimitive::getAttributeStr(
  std::string const attName, const ::QDomElement element, std::string const def
)
{
...
{
  return stringToBool(element.attribute(attName.c_str()).toStdString());
}
...
qtXmlPrimitive::getAttributeLabelVector(::QDomElement const &element)
{
...
::QDomElement
qtXmlPrimitive::createElement(::QDomDocument &doc, std::string const name)
{
...
)
{
  ...
  for (int ii = 0; ii < elements.size(); ii++)
  {
    ...
    {
      if (getAttributeStr(elAttribute, foundEl) == elAttributeValue)
      {
...
std::vector<QDomAttr>
qtXmlPrimitive::getAttributeVector(::QDomElement const element)
{
