...
std::vector<std::string> vec3dThreeD_transVolAFXmlBuilder::factoryAlias(void
) const
{
  return ::boost::assign::list_of("vec3dTransVolThreeD")(
...
) const
{
  ...
  sFunP->push_back(vec3dTransVolThreeD_skinBSplineSurfaces(&aF).result());
}
...
) const
{
  ...
  vec3dTransVolThreeDCompound *vec3dTVC = new vec3dTransVolThreeDCompound();
  for (int cc = 0; cc < nComponents; cc++)
  {
    ...
    //
    dt__pH(vec3dTransVolThreeD)
      tv(vec3dTransVolThreeD_skinBSplineSurfaces(&iAF).result());
