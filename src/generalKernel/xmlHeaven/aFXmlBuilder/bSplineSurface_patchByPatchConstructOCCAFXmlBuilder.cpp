...
) const
{
  ...
  logC() << "Create wire: size " << elV.size() << " x " << wire.size()
         << " )\n";
  ...
  dt__forAllIndex(elV, ii)
  {
    ...
    dt__pH(analyticFunction)
      theF(dtXmlParserBase::createAnalyticFunction(elV[ii], bC, cV, aF));
    dt__ptrAss(
    ...
    { // split curves at points in wire
      logC() << "  - split 0% <-> " << wire.front() * 100. << "%\n";
      cc[ii][0].reset(trimmedCurve_uBounds(
      ...
      {
        logC() << "  - split " << wire[jj - 1] * 100. << "% <-> "
               << wire[jj] * 100. << "%\n";
        cc[ii][jj].reset(trimmedCurve_uBounds(
                           v3d1d->ptrConstDtCurve(),
      ...
      logC() << "  - split " << wire.back() * 100. << "% <-> 1.0 \n";
      cc[ii].back().reset(trimmedCurve_uBounds(
                            v3d1d->ptrConstDtCurve(),
...
dt__pH(dtSurface)
  bSplineSurface_patchByPatchConstructOCCAFXmlBuilder::sortAndInit(
    dtInt const &fixJ,
    twoDArrayHandling<dt__pH(dtCurve)> const &cc,
  ...
  ) const
{
  ...
  {
    if (ii < skin_nInitCuts)
      ccSkin.push_back(cc[ii][fixJ].get());
    else
      ccFill.push_back(cc[ii][fixJ].get());
  }

  return dt__pH(dtSurface)(bSplineSurface_skinConstructOCC(
...
) const
{
  ...
  dt__pH(dtCurve)
    bound_0(geomFillBoundWithSurf_surfaceConstructOCC(
              boundSurf_0,
              dt__pH(dtCurve2d)(bSplineCurve2d_pointConstructOCC(
              ...
              .result());
  dt__pH(dtCurve) bound_1(bSplineCurve_bSplineCurveSplitConstructOCC(
                            boundCurv_1,
  ...
  dt__pH(dtCurve) bound_3(bSplineCurve_bSplineCurveSplitConstructOCC(
                            boundCurv_3,
...
) const
{
  ...
  dt__forFromToIndex(0, cc.size(1), jj)
  {
    ...
    dtC.push_back(dt__pH(dtCurve)(
      bSplineCurve_pointInterpolateConstructOCC(
        pp.fixJ(jj),
        dtXmlParserBase::getAttributeInt("pointInterpolate_degMin", toBuild),
        dtXmlParserBase::getAttributeInt("pointInterpolate_degMax", toBuild)
      )
        .result()
    ));
    ...
    {
      logC() << "    - Filling curve " << kk << " / " << ccFill.size()
             << " .\n";
      dtS[jj].push_back(dt__pH(dtSurface)(fillPatch(
        dtS[jj].back().get(),
        dtC[1].get(),
        ccFill[kk],
        dtC[0].get(),
  ...
  if (dtXmlParserBase::getAttributeBool("composite", toBuild))
  {
    ...
    {
      if (jj == 0)
        ssV0[ii][jj] = dtS[ii][jj].get();
      else
      {
        ssV1[ii][jj - 1] = dtS[ii][jj].get();
      }
    };
    result->push_back(new vec3dSurfaceTwoD(dt__tmpPtr(
      dtSurface, compositeSurface_surfaceConnectConstructOCC(ssV0).result()
    )));
    result->push_back(new vec3dSurfaceTwoD(dt__tmpPtr(
      dtSurface, compositeSurface_surfaceConnectConstructOCC(ssV1).result()
    )));
  }
  else
  {
