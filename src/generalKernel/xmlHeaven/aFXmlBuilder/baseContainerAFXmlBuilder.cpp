...
namespace dtOO {
bool baseContainerAFXmlBuilder::_registrated = aFXmlBuilderFactory::registrate(
...
) const
{
  std::vector<::QDomElement> wElement =
    dtXmlParserBase::getChildVector(toBuildP);
  ...
  for (int ii = 0; ii < wElement.size(); ii++)
  {
    if (dtXmlParserBase::is("Point_3", wElement[ii]))
    {
      ...
      dtXmlParserBase::createBasic(
        &wElement[ii], bC, cValP, depSFunP, &workingPointP
      );
    }
