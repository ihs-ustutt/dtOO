...
bSplineCurve2d_3PointMeanlineConstructAFXmlBuilder ::factoryAlias(void) const
{
...
) const
{
  ...
  if (dtXmlParserBase::hasAttribute("alpha_one", toBuildP))
  {
    config.append(
      "alphaOne",
      dtXmlParserBase::getAttributeFloatMuParse("alpha_one", toBuildP, cV, aF)
    );
  }
  if (dtXmlParserBase::hasAttribute("alpha_two", toBuildP))
  {
    config.append(
      "alphaTwo",
      dtXmlParserBase::getAttributeFloatMuParse("alpha_two", toBuildP, cV, aF)
    );
  }
  if (dtXmlParserBase::hasAttribute("ratio", toBuildP))
  {
    config.append(
      "ratio",
      dtXmlParserBase::getAttributeFloatMuParse("ratio", toBuildP, cV, aF)
    );
  }
  ...
  if (dtXmlParserBase::hasAttribute("delta_x", toBuildP))
  {
    config.append(
      "deltaX",
      dtXmlParserBase::getAttributeFloatMuParse("delta_x", toBuildP, cV, aF)
    );
  }
  ...
  if (dtXmlParserBase::hasAttribute("delta_y", toBuildP))
  {
    config.append(
      "deltaY",
      dtXmlParserBase::getAttributeFloatMuParse("delta_y", toBuildP, cV, aF)
    );
  }

  if (dtXmlParserBase::hasAttribute("targetLength", toBuildP) &&
  ...
  if (dtXmlParserBase::hasAttribute("originOnLengthPercent", toBuildP))
  {
    config.append(
  ...
  {
    config.append(
      "revert", dtXmlParserBase::getAttributeBool("revert", toBuildP)
    );
  }
