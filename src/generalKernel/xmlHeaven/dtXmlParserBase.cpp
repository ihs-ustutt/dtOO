...
)
{
  ...
  if (!wElement.isNull())
  {
    ...
    {
      dtTransformerP = dtTransformerFactory::createFromPlugin(
...
)
{
  ...
  createBasic(&wEl, bC, cV, aF, aG, dtTransformerP.get(), &pp);
...
)
{
  ...
  {
    std::vector<dtReal> cXYZ = muParseCSString(replaceDependencies(
  ...
  else if (hasAttribute("attribute", *toBuildP))
  {
    ...
    if (attribute == "pick_order_from_file")
    {
      ...
      if (stringContains(".csv", filename))
      {
        ...
        dt__throw(createBasic(), << "Unknown file extension.");
    }
  }
...
)
{
  ...
  if (hasAttribute("label", *toBuildP))
  {
    ...
    {
      std::vector<std::string> forStr = convertToStringVector(
      ...
      for (int ii = from; ii <= to; ii = ii + spacing)
      {
        ...
        if (bC->ptrPointContainer()->has(pointName))
        {
          basicP->push_back(dtPoint3(bC->ptrPointContainer()->get(pointName)));
        }
      }
      return;
    }
  ...
  {
    std::vector<dtReal> cXYZ = muParseCSString(
  ...
  else if (hasAttribute("attribute|part_label", *toBuildP))
  {
    ...
    analyticGeometry const *anAG = aG->get(partLabel);
    ...
    if (attribute == "pick_from_part")
    {
      ...
      if (m2d)
      {
        ...
        {
          dtReal cX = getAttributeFloatMuParse(
            "parameter_one_percent", *toBuildP, cV, aF
          );
          dtReal cY = getAttributeFloatMuParse(
      ...
      {
        dtReal cX =
          getAttributeFloatMuParse("parameter_one_percent", *toBuildP, cV, aF);
      ...
      else if (m3d)
      {
            ...
            ))
        {
          ...
          dtReal cZ = getAttributeFloatMuParse(
    ...
    else if (attribute == "pick_order_from_part")
    {
      ...
      if (m1d && hasAttribute("number_points_one", *toBuildP))
      {
        ...
        if (hasAttribute("parameter_one_percent_function", *toBuildP))
        {
          dt__ptrAss(
            scaOneD const *sF,
            scaOneD::ConstDownCast(aF->get(
              getAttributeStr("parameter_one_percent_function", *toBuildP)
            ))
          );
          std::vector<dtReal> ff = float_scaOneDPoint(sF, nPointsOne).result();
        ...
        else if (hasAttribute("length_one_percent_function", *toBuildP))
        {
          dt__ptrAss(
            ...
            scaOneD::ConstDownCast(
              aF->get(getAttributeStr("length_one_percent_function", *toBuildP))
            )
          );
               ...
               ))
      {
        ...
        for (int ii = 0; ii < nPointsOne; ii++)
        {
          for (int jj = 0; jj < nPointsTwo; jj++)
          {
            ...
            basicP->push_back(dtPoint3(m2d->getPointPercent(paraOne, paraTwo)));
          }
        }
      }
    }
  ...
  else if (hasAttribute("attribute|function_label", *toBuildP))
  {
    ...
    analyticFunction const *anAF = aF->get(functionLabel);
    ...
    if (attribute == "pick_from_function")
    {
      ...
      else if (v2D && hasAttribute("x_one_percent|x_two_percent", *toBuildP))
      {
        dtReal cX =
          getAttributeFloatMuParse("x_one_percent", *toBuildP, cV, aF);
        dtReal cY =
  ...
  if (hasAttribute("label", *toBuildP))
  {
    ...
    if (basicP->size() > 1)
    {
      ...
      bC->ptrPointContainer()->add(basicP->at(0), label);
  }
}
...
)
{
  ...
  else
  {
    ...
    if (toBuildP->hasAttribute("x") && toBuildP->hasAttribute("y"))
    {
      ...
      //
      dtReal cX = muParseString(
        replaceDependencies(getAttributeStr("x", *toBuildP), cV, aF)
      );
      dtReal cY = muParseString(
        replaceDependencies(getAttributeStr("y", *toBuildP), cV, aF)
      );
      //
      // create vector
      //
    ...
    else if (toBuildP->hasAttribute("u") && toBuildP->hasAttribute("v"))
    {
      ...
      //
      dtReal cX = muParseString(
        replaceDependencies(getAttributeStr("u", *toBuildP), cV, aF)
      );
      dtReal cY = muParseString(
        replaceDependencies(getAttributeStr("v", *toBuildP), cV, aF)
      );
      //
      // create vector
      //
...
)
{
  ...
  {
    dt__throw(createDtVector3(), << dt__eval(qPrintable(toBuildP->tagName())));
  }
  else
  {
    //
    ...
    {
      if (bC->ptrVectorContainer()->has(getAttributeStr("label", *toBuildP)))
      {
        vv = bC->ptrVectorContainer()->get(getAttributeStr("label", *toBuildP));
        return vv;
      }
    }
    //
        ...
        toBuildP->hasAttribute("z"))
    {
      ...
      dtReal cZ = muParseString(
        replaceDependencies(getAttributeStr("z", *toBuildP), cV, aF)
      );
      //
      // create vector
      //
    ...
    if (hasAttribute("label", *toBuildP))
    {
      bC->ptrVectorContainer()->add(vv, getAttributeStr("label", *toBuildP));
    }
    //
    // output
    //
...
{
  return dtXmlParserBase::replaceDependencies(expression, &bC, &cV, &aF, &aG);
}
...
{
  ptrHandling<dtTransformer> dtT(createTransformer(toBuildP, bC, cV, aF));
  ::QDomElement wEl = *toBuildP;
  if (is("transformer", *toBuildP))
  {
    wEl = toBuildP->nextSiblingElement();
...
)
{
  ...
  {
    std::vector<dtReal> cXY = muParseCSString(
           ...
           hasAttribute("part_label", *toBuildP))
  {
    ...
    analyticSurface_partToPickFromP = analyticSurface::ConstDownCast(anAG);
    //
    // attribute: pick_from_part
    //
    if (toBuildP->attribute("attribute") == "pick_from_part")
    {
      ...
      if (analyticSurface_partToPickFromP)
      {
            ...
            toBuildP->hasAttribute("parameter_two_percent"))
        {
          ...
          // create coordinates
          //
          dtReal pX = muParseString(replaceDependencies(
            getAttributeStr("parameter_one_percent", *toBuildP), cV, aF
          ));
          dtReal pY = muParseString(replaceDependencies(
            getAttributeStr("parameter_two_percent", *toBuildP), cV, aF
          ));
          //
          ...
          dt__throwUnexpected(createBasic());
      }
      else
        dt__throwUnexpected(createBasic());
    }
    ...
    if (getAttributeStr("attribute", *toBuildP) == "pick_order_from_function")
    {
          ...
          hasAttribute("number_points", *toBuildP))
      {
        ...
        dt__ptrAss(
          ...
          scaOneD::ConstDownCast(
            aF->get(getAttributeStr("parameter_percent_function", *toBuildP))
          )
        );
        ...
        for (int ii = 0; ii < nPoints; ii++)
        {
          ...
          basicP->push_back(
            theF->YdtPoint2Percent(thePara->YFloat(iiF / (nPointsF - 1.)))
          );
        }
      }
      else
  ...
  {
    dt__debug(
      createBasic(), << "created point:" << dt__point2d((basicP->at(ii)))
    );
  }
}
...
)
{
  ...
  // create transformer if necessary
  //
  ptrHandling<dtTransformer> dtT(createTransformer(toBuildP, bC, cV, aF, aG));
  ::QDomElement wEl = *toBuildP;
  if (is("transformer", *toBuildP))
  {
    wEl = toBuildP->nextSiblingElement();
...
)
{
  ...
  else if (stringContains("{", label) && stringContains("}", label))
  {
    ...
    //
    std::vector<std::string> labelV = convertToStringVector("{", "}", label);
    dt__forAllIndex(labelV, ii) advancedP->push_back(aG->get(labelV[ii]));
  }
  else
    advancedP->push_back(aG->get(label)->clone());
...
)
{
  ...
  if (stringContains("*", label))
  {
    ...
    // string contains "*" --> return set of analyticGeometries
    //
    std::string pattern = stringRemoveSingle("*", label);
