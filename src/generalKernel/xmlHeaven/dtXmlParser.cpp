...
dtXmlParser *
dtXmlParser::init(std::string const &inFile, std::string const &outFile)
{
  if (_pH)
    delete _pH;
...
dtXmlParser::dtXmlParser(std::string const &inFile, std::string const &outFile)
...
void dtXmlParser::parse(void) { parseFile(_inFile.c_str()); }
...
void dtXmlParser::parseFile(char const *const fileName)
{
  ...
  _rootRead.push_back(xmlDocument.documentElement().cloneNode(true).toElement()
  );
  //
  ...
  //
  dt__forAllRefAuto(getChildVector("constant", _rootRead.back()), aConstant)
  {
  ...
  dt__forAllRefAuto(getElementRecursive("replace"), forElement)
  {
    ...
    dt__forAllRefAuto(convertToStringVector("{", "}", valueStr), aValue)
    {
      dt__forAllRefAuto(getChildVector(forElement), aChild)
      {
        ...
        ::QDomNode checkOne = forElement.parentNode().insertBefore(
  ...
  dt__forAllRefAuto(getElementRecursive("vectorReplace"), forElement)
  {
    ...
    {
      dt__debug(parse(), << "vectorReplace[ " << iiInd << " ] = " << aValueVec);
      dt__forAllRefAuto(getChildVector(forElement), aChild)
      {
        ::QDomElement childClone = aChild.cloneNode(true).toElement();
...
)
{
  ...
  dt__forAllIndex(cValRef, ii)
  {
    if (cValRef[ii]->loadable())
    {
      ...
      {
        cValRef[ii]->setValue(getAttributeFloat(
          "value",
          getChildElement("constValue", cValRef[ii]->getLabel(), stateElement)
        ));
      }
      else
      {
        dt__warning(
          loadStateToConst(),
          << cValRef[ii]->getLabel() << "-Element not in state file."
          << std::endl
    ...
    else
    {
      dt__warning(
        loadStateToConst(),
        << "Defer not loadable constValue > " << cValRef[ii]->getLabel()
        << " <."
      );
    }
  }
...
) const
{
  ...
  dt__forAllIndex(stateName, ii)
  {
    ...
    appendChildElement(_rootLoadDoc, newState);
    dt__forAllRefAuto(cValP[ii], aCV)
      aCV->writeToElement(_rootLoadDoc, newState);
  }
  ...
  dt__info(
    write(),
    << "Save " << stateName.size() << " states  to file = " << fileName
    << std::endl
    << dt__eval(currentState())
  );
}

void dtXmlParser::write(lvH_constValue const &cValP) const { write(&cValP); }
...
) const
{
  ...
  if (!xmlFile.exists())
  {
    dt__warning(
      checkFile,
      << "File " << dt__eval(fileName) << " does not exist. Try to create file."
    );
  ...
  if (!xmlDocument.setContent(&xmlFile, &qString, &line, &column))
  {
    ...
    // encoding
    xmlDocument.appendChild(xmlDocument.createProcessingInstruction(
      "xml", "version=\"1.0\" encoding=\"ISO-8859-1\""
    ));
    // machine name
    xmlDocument.appendChild(xmlDocument.createElement(_rootRead[0].tagName()));
    // write to file
    ::QTextStream(&xmlFile) << xmlDocument.toString();
  }
  else
  {
    ...
    if (xmlDocument.documentElement().tagName() == _rootRead[0].tagName())
    {
    ...
    else
    {
      ...
      dt__throw(
        ...
        << "Root element name of current parser : " << getTagName(_rootRead[0])
      );
    }
  }
...
void dtXmlParser::freeState(void) const { lVHOstateHandler().makeState(""); }

bool dtXmlParser::stateLoaded(void) const
{
  if (lVHOstateHandler().commonState() == "")
    return false;

  return true;
}

void dtXmlParser::createConstValue(lvH_constValue *cValP) const
{
  if (!lVHOstateHandler::initialized())
  {
  ...
  dt__forAllRefAuto(getLabels("constValue"), label)
  {
    ...
    constValue *aCV = constValue::create(
  ...
  {
    aCV->setValue(getAttributeFloatMuParse(
      "value", getElement("constValue", aCV->getLabel()), cValP
    ));
  }
  ...
  if (!cVInit.isNull())
  {
    ...
    if (dtXmlParser::reference().hasState(getAttributeStr("state", cVInit)))
    {
...
void dtXmlParser::createConstValue(lvH_constValue &cValRef) const
{
...
std::vector<std::string> dtXmlParser::getLabels(std::string lookType) const
{
...
) const
{
  ...
  dt__throw(
    getElement(), << "No " << lookType << "-Element with " << dt__eval(lookName)
  );
}
...
dtXmlParser::getElementRecursive(std::string const lookType) const
{
...
) const
{
  ...
  ptrHandling<aFXmlBuilder> builder(aFXmlBuilderFactory::create(builderName));
  ...
  if (!buildCompound)
    builder->buildPart(tE, bC, cVP, aFP, &tmpAF);
  else
    builder->buildPartCompound(tE, bC, cVP, aFP, &tmpAF);
  ...
  if (stringPrimitive::stringContains("{", label))
  {
    ...
    dt__forAllIndex(tmpAF, ii)
    {
      ...
      else
      {
        ...
        logC() << "setLabel( " << baseName + "_" + intToString(counterLabel)
               << " )" << std::endl;
        counterLabel++;
      }
  ...
  else
  {
    if (tmpAF.size() > 1)
    {
      dt__forAllIndex(tmpAF, ii)
      {
        logC() << "setLabel( " << label + "_" + intToString(ii) << " )"
               << std::endl;
...
) const
{
  ...
  ptrHandling<aGXmlBuilder> builder(aGXmlBuilderFactory::create(builderName));

  if (!buildCompound)
    builder->buildPart(tE, bC, cVP, sFP, aGP, &tmpAGeo);
  else
    builder->buildPartCompound(tE, bC, cVP, sFP, aGP, &tmpAGeo);
  ...
  for (int ii = 0; ii < tmpAGeo.size(); ii++)
  {
    ...
    else
    {
      if (!isFullWildcard(label))
      {
        ...
        //
        tmpAGeo[ii]->setLabel(stringPrimitive::replaceStringInString(
...
) const
{
  ...
  //
  bVP->push_back(boundedVolumeFactory::create(getAttributeStr("name", wEl)));
  //
...
) const
{
  ...
  {
    pLP->push_back(dtPluginFactory::createFromPlugin(
  ...
  {
    pLP->push_back(dtPluginFactory::create(getAttributeStr("name", wEl)));
  }
...
) const
{
  ...
  // create new plugin with factory
  //
  dCP->push_back(dtCaseFactory::create(getAttributeStr("name", wEl)));
  //
  // initialize dtPlugin
  //
