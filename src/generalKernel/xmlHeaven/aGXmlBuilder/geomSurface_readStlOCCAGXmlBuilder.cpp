...
bool geomSurface_readStlOCCAGXmlBuilder::_registrated =
  aGXmlBuilderFactory::registrate(dt__tmpPtr(
    geomSurface_readStlOCCAGXmlBuilder, new geomSurface_readStlOCCAGXmlBuilder()
  ));

geomSurface_readStlOCCAGXmlBuilder ::geomSurface_readStlOCCAGXmlBuilder() {}
...
{
  dt__throwIf(!dtXmlParserBase::hasAttribute("filename", toBuild), buildPart());
  ...
  std::pair<omHalfedgeH, omHalfedgeH> hePair(heH, om.next_halfedge_handle(heH));
  ...
  do
  {
    ...
    dtVector3 v0 = dtGmshModel::extractPosition(om.at(vH1)) -
                   dtGmshModel::extractPosition(om.at(vH0));
    dtVector3 v1 = dtGmshModel::extractPosition(om.at(vH2)) -
  ...
  dt__info(
    buildPart(),
    << logMe::dtFormat("max %8.3e %8.3e %8.3e %8.3e") % maxAngle[0] %
           maxAngle[1] % maxAngle[2] % maxAngle[3]
    << std::endl
    << logMe::dtFormat("pos %8i %8i %8i %8i") % maxAnglePos[0] %
