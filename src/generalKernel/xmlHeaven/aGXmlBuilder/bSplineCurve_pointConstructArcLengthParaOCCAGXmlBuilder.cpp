...
) const
{
  ...
  dt__throwIf(
    !dtXmlParserBase::hasAttribute("tolerance", toBuild), buildPart()
  );
  dt__throwIf(
    !dtXmlParserBase::hasAttribute("max_order", toBuild), buildPart()
  );
  dt__throwIf(
    !dtXmlParserBase::hasAttribute("num_segments", toBuild), buildPart()
  );

  dtInt const maxOrder =
    dtXmlParserBase::getAttributeIntMuParse("max_order", toBuild, cV, aF);
  ...
  dt__ptrAss(
    analyticCurve const *sC3, analyticCurve::ConstDownCast(aG_t.get())
  );
  result->push_back(
    new analyticCurve(bSplineCurve_pointConstructArcLengthParaOCC(
