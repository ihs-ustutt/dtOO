...
) const
{
  ...
  // radius
  //
  dtReal const r0 = dtXmlParserBase::getAttributeFloatMuParse(
    "radius_one", toBuild, cV, aF, aG
  );
  dtReal const r1 = dtXmlParserBase::getAttributeFloatMuParse(
    "radius_two", toBuild, cV, aF, aG
  );
  dt__info(buildPart(), << "r0 = " << r0 << std::endl << "r1 = " << r1);
  ...
  if (wE_v.size() == 1)
  {
    ...
    dtC.reset(
      ellipse_radiusRadiusCoordinateSystemConstructOCC(pp, xx, r0, r1).result()
    );
  }
  else if (wE_v.size() == 2)
  {
    ...
    dtC.reset(
      ellipse_radiusRadiusCoordinateSystemConstructOCC(pp, nn, xx, r0, r1)
        .result()
    );
  }
  else
  ...
  if (dtXmlParserBase::hasAttribute("angle", toBuild))
  {
    ...
    // trim curve
    //
    dtC.reset(trimmedCurve_uBounds(dtC.get(), dtC->getUMin(), uMax).result());
  }
