...
bool trans6SidedCubeAGXmlBuilder::_registrated =
  aGXmlBuilderFactory::registrate(
    dt__tmpPtr(trans6SidedCubeAGXmlBuilder, new trans6SidedCubeAGXmlBuilder())
  );
...
) const
{
  ...
  if (wElementVec.size() == 6)
  {
    ...
    {
      exchange = dtXmlParserBase::getAttributeBoolVector("exchange", toBuild);
    }
    ...
    dt__forAllIter(std::vector<::QDomElement>, wElementVec, it)
    {
      ...
      dt__pH(analyticGeometry const)
        aG_t(dtXmlParserBase::createAnalyticGeometry(*it, bC, cV, aF, aG));
      //
      // check if it is a map2dTo3d
      //
      dt__pH(dtSurface) dtS(analyticSurface::MustConstDownCast(aG_t.get())
                              ->ptrConstDtSurface()
                              ->clone());
      if (exchange[counter])
      {
      ...
      if (reverse[counter] != "")
      {
        ...
        {
          dtS.reset(
            geomSurface_surfaceReverseConstructOCC(dtS.get(), true, false)
              .result()
          );
        }
        ...
        {
          dtS.reset(
            geomSurface_surfaceReverseConstructOCC(dtS.get(), false, true)
              .result()
          );
        }
        ...
        {
          dtS.reset(
            geomSurface_surfaceReverseConstructOCC(dtS.get(), true, true)
              .result()
          );
        }
        else
  ...
  else if (wElementVec.size() == 2)
  {
    ...
    {
      dt__pH(dtSurface const)
        thisSurface(analyticSurface::ConstDownCast(
                      dt__tmpPtr(
                        analyticGeometry,
      ...
      if (reverse[ii] != "")
      {
        if (reverse[ii] == "u")
        {
          thisSurface.reset(geomSurface_surfaceReverseConstructOCC(
    ...
    dt__forAllIndex(dtS, ii) { mm.push_back(new analyticSurface(dtS[ii])); }
    cDtS.destroy();
    dtS.destroy();
  }
  else
  ...
  //
  result->push_back(new trans6SidedCube(
