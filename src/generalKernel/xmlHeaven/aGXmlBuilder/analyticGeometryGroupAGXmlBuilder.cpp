...
bool analyticGeometryGroupAGXmlBuilder::_registrated =
  aGXmlBuilderFactory::registrate(dt__tmpPtr(
    analyticGeometryGroupAGXmlBuilder, new analyticGeometryGroupAGXmlBuilder()
  ));

analyticGeometryGroupAGXmlBuilder::analyticGeometryGroupAGXmlBuilder() {}
analyticGeometryGroupAGXmlBuilder::~analyticGeometryGroupAGXmlBuilder() {}

std::vector<std::string> analyticGeometryGroupAGXmlBuilder ::factoryAlias(void
) const
{
  return ::boost::assign::list_of("map1dTo3dGroup")("map2dTo3dGroup")(
    "map3dTo3dGroup"
  );
}
...
) const
{
  ...
  {
    if (!dtXmlParserBase::isWildcard(
          dtXmlParserBase::getAttributeStr("label", wE)
        ))
    {
      mXd.push_back(dtXmlParserBase::createAnalyticGeometry(wE, bC, cV, aF, aG)
      );
    }
    else
    {
      ...
      {
        if (dtXmlParserBase::matchWildcard(wCard, anAG->getLabel()))
        {
