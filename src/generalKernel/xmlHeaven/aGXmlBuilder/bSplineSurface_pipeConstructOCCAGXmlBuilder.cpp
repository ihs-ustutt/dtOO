...
) const
{
  ...
  if ((wE_v.size() == 1) && dtXmlParserBase::hasAttribute("radius", toBuild))
  {
    dt__pH(analyticGeometry)
      aPath(dtXmlParserBase::createAnalyticGeometry(wE_v[0], bC, cV, aF, aG));
    dtS.reset(
  ...
  else if (wE_v.size() == 2)
  {
    dt__pH(analyticGeometry)
      aPath(dtXmlParserBase::createAnalyticGeometry(wE_v[0], bC, cV, aF, aG));
    dt__pH(analyticGeometry)
      aSecOne(dtXmlParserBase::createAnalyticGeometry(wE_v[1], bC, cV, aF, aG));
  ...
  else if (wE_v.size() == 3)
  {
    ...
    dt__pH(analyticGeometry)
      aSecOne(dtXmlParserBase::createAnalyticGeometry(wE_v[1], bC, cV, aF, aG));
    dt__pH(analyticGeometry)
      aSecTwo(dtXmlParserBase::createAnalyticGeometry(wE_v[2], bC, cV, aF, aG));
    dtS.reset(bSplineSurface_pipeConstructOCC(
                analyticCurve::MustDownCast(aPath.get())->ptrConstDtCurve(),
                analyticCurve::MustDownCast(aSecOne.get())->ptrConstDtCurve(),
