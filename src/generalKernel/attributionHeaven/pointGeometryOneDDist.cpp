...
pointGeometryOneDDist::pointGeometryOneDDist(
  dtReal const &value, analyticGeometry const *const aG, dtInt const &dir
)
  : _value(value), _dir(dir), _dimension(aG->dim()), _aG(aG)
{
}
...
dtReal pointGeometryOneDDist::operator()(std::vector<dtReal> const &xx) const
{
  return fabs(_aG->getPointPercent(&(xx[0]))[_dir] - _value);
}

dtInt const &pointGeometryOneDDist::dimension() const { return _dimension; }
