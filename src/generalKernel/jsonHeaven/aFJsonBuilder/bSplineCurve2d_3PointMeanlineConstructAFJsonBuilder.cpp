...
) const
{
  ...
  if (config.contains("ratio") && !config.contains("deltaX") &&
      config.contains("deltaY"))
  {
    dtC2d.reset(bSplineCurve2d_angleRatioDeltaYConstructOCC(
  ...
  else if (!config.contains("ratio") && config.contains("deltaX") &&
           config.contains("deltaY"))
  {
    //
    // get necessary values
    //
    dtC2d.reset(bSplineCurve2d_angleDeltaXDeltaYConstructOCC(
                  config.lookup<dtReal>("alphaOne"),
                  config.lookup<dtReal>("alphaTwo"),
  ...
  {
    dt__throw(buildPart(), << "config = " << config.toStdString());
  }

  if (config.contains("targetLength"))
  {
    ...
    {
      dtReal scale = config.lookup<dtReal>("targetLength") / dtC2d->length();
      ...
      logC() << logMe::dtFormat("( %3d ) : %5.2f -> %5.2f") % ii % scale %
  ...
  if (config.contains("originOnLengthPercent"))
  {
    ...
    dt__forFromToIndex(0, dtC2d->nControlPoints(), ii)
    {
      dtC2d->setControlPoint(
        ii, dtLinearAlgebra::toDtPoint2(dtC2d->controlPoint(ii) - adjustP)
      );
    }
  }
