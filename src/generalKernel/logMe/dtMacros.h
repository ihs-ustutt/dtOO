...
#define dt__classOnlyName(name)                                                \
  static std::string className(void) { return std::string(#name); }            \
  virtual std::string virtualClassName(void) const                             \
  {                                                                            \
    return std::string(#name);                                                 \
  }                                                                            \
  template <class name> bool Is(name obj) const                                \
  {                                                                            \
...
#define dt__class(name, castFrom)                                              \
  ...
  static name *DownCast(castFrom *obj) { return dynamic_cast<name *>(obj); }   \
  ...
  static name *SecureCast(castFrom *obj) { return dynamic_cast<name *>(obj); } \
  ...
  static name *CastConst(name const *obj) { return const_cast<name *>(obj); }  \
  static std::vector<name *> VectorCastConst(std::vector<name const *> obj)    \
  {                                                                            \
    std::vector<name *> ret;                                                   \
    dt__forAllIndex(obj, ii) { ret.push_back(const_cast<name *>(obj[ii])); }   \
    return ret;                                                                \
  }                                                                            \
...
#define dt__H_addCloneForpVH(type) type *new_clone(type const &aF);
#define dt__C_addCloneForpVH(type)                                             \
  type *new_clone(type const &aF) { return aF.clone(); }
...
#define dt__forAllIter(type, vector, iter)                                     \
  for (type::iterator iter = (vector).begin(); iter != (vector).end(); ++iter)
...
#define dt__mustCast(object, type, result)                                     \
  ...
  if (result == NULL)                                                          \
    dt__throw(                                                                 \
      dt__mustCast, << "dynamic_cast of " #object " to " #type " fails"        \
    );
