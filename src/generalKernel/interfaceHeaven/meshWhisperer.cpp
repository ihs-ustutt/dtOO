...
meshWhisperer::meshWhisperer(dtGmshModel *const gm)
  : _gm(gm), _thisRank(staticPropertiesHandler::getInstance()->thisRank()),
...
void meshWhisperer::add(::GEntity *ge)
{
  if (dtGmshEdge::ConstDownCast(ge))
  {
    ...
    dt__forAllRefAuto(vertices, aVert) { add(aVert); }

    std::vector<::MLine *> &line = thisEdge->lines;
    dt__forAllRefAuto(line, aLine) { add((::MElement *)aLine, thisEdge); }
  }
  else if (dtGmshFace::ConstDownCast(ge))
  {
    ...
    dt__forAllRefAuto(tri, aTri) { add((::MElement *)aTri, thisFace); }
    std::vector<::MQuadrangle *> &quad = thisFace->quadrangles;
    dt__forAllRefAuto(quad, aQuad) { add((::MElement *)aQuad, thisFace); }
    //      _generalInfo().push_back()
    std::vector<std::vector<::MVertex *>> &tFV = thisFace->transfinite_vertices;
    if (tFV.size() != 0)
    {
      _generalInfo().append(
  ...
  else if (dtGmshRegion::ConstDownCast(ge))
  {
    ...
    dt__forAllRefAuto(hex, aHex) { add((::MElement *)aHex, thisRegion); }
    std::vector<::MPyramid *> &pyr = thisRegion->pyramids;
    dt__forAllRefAuto(pyr, aPyr) { add((::MElement *)aPyr, thisRegion); }
    std::vector<::MTetrahedron *> &tet = thisRegion->tetrahedra;
    dt__forAllRefAuto(tet, aTet) { add((::MElement *)aTet, thisRegion); }
    std::vector<::MPrism *> &pri = thisRegion->prisms;
    dt__forAllRefAuto(pri, aPri) { add((::MElement *)aPri, thisRegion); }
    std::vector<::MPolyhedron *> &pol = thisRegion->polyhedra;
    dt__forAllRefAuto(pol, aPol) { add((::MElement *)aPol, thisRegion); }
    //      std::vector< ::MQuadrangle * > & quad = thisFace->quadrangles;
...
{
  std::vector<::MVertex *>::iterator it = std::find(_mv.begin(), _mv.end(), mv);
    ...
    dt__forFromToIndex(0, _num.global()[rr].size(), ii)
    {
      ...
      if (rr != _thisRank)
      {
        ...
        else if (thisMvType == 2)
        {
          ...
          dt__throwUnexpected(addRenumberedVertices());

        if (thisOnWhatDim == 0)
        {
          _gm->getDtGmshVertexByTag(thisOnWhatTag)->addMeshVertex(aNewMv);
          aNewMv->setEntity(_gm->getDtGmshVertexByTag(thisOnWhatTag));
        }
        else if (thisOnWhatDim == 1)
        {
          _gm->getDtGmshEdgeByTag(thisOnWhatTag)->addMeshVertex(aNewMv);
          aNewMv->setEntity(_gm->getDtGmshEdgeByTag(thisOnWhatTag));
        }
        else if (thisOnWhatDim == 2)
        {
          _gm->getDtGmshFaceByTag(thisOnWhatTag)->addMeshVertex(aNewMv);
          aNewMv->setEntity(_gm->getDtGmshFaceByTag(thisOnWhatTag));
        }
        else if (thisOnWhatDim == 3)
        {
          _gm->getDtGmshRegionByTag(thisOnWhatTag)->addMeshVertex(aNewMv);
  ...
  dt__forFromToIndex(0, _nRanks, rr)
  {
    if (rr != _thisRank)
    {
      ...
      dt__forAllRefAuto(gVector, anEntry)
      {
        ...
        if (stringPrimitive::stringContains("transfinite_vertices", anEntry))
        {
          dt__info(
            addRenumberedVertices(),
            << dt__eval(stringPrimitive::getStringBetween(
      ...
      dt__forFromToIndex(0, _tFVertexOnWhatTag.global()[rr].size(), ii)
      {
        ...
        _gm->getDtGmshFaceByTag(tFVOnWhatTag)
...
void meshWhisperer::addRenumberedElements(void)
{
  ...
  dt__forFromToIndex(0, _nRanks, rr)
  {
    ...
    dt__forFromToIndex(0, _meNum.global()[rr].size(), ii)
    {
      ...
      {
        verts.push_back(_gm->getMeshVertexByTag(thisMeVertexNum[jj]));
        dt__throwIf(
      ...
      if (rr != _thisRank)
      {
        ...
        else if (thisMeOnWhatDim == 3)
        {
          ...
          dt__throwUnexpected(addRenumberedElements());
      }
      else
      {
