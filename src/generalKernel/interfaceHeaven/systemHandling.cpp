...
void systemHandling::renameFile(std::string const &from, std::string const &to)
{
...
void systemHandling::deleteDirectory(std::string const &dirname)
{
  try
  {
    boost::filesystem::remove_all(boost::filesystem::path(dirname.c_str()));
       ...
       ++file)
  {
    try
    {
      ...
      if (fs::is_directory(current))
      {
        ...
        copyDirectory(
          current.string(), (destination / current.filename()).string()
        );
      }
      else
      {
...
{
  return ::boost::filesystem::path(::boost::filesystem::current_path())
    .string();
}
...
void systemHandling::unsetEnv(std::string const &envName)
{
  ...
  {
    dt__throw(unsetEnv(), << "Cannot unset environment variable " << envName);
  }
}

std::vector<std::string> systemHandling::directoryList(std::string const &path)
{
