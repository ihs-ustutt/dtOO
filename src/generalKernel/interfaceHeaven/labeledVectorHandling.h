...
public:
  ...
  reference operator[](int ii) { return std::vector<T>::at(ii); }
  const_reference operator[](int ii) const { return std::vector<T>::at(ii); }
...
template <typename T>
labeledVectorHandling<T>::labeledVectorHandling(const std::vector<T> &orig)
  : vectorHandling<T>(orig)
{
}
...
template <typename T> labeledVectorHandling<T>::~labeledVectorHandling() {}

template <typename T> void labeledVectorHandling<T>::set(T const &toSet)
{
...
template <typename T>
std::vector<dtInt> labeledVectorHandling<T>::getIndices(std::string const &label
) const
{
  ...
  dt__throwIfWithMessage(
    ret.empty(), getIndices(), << "No elements for " << label
  );
  return ret;
}
...
std::vector<std::string> labeledVectorHandling<T>::labels(void) const
{
  ...
  dt__forAllIndex(*this, ii) { ret.push_back(this->getLabel(ii)); }
  return ret;
}
...
template <typename T> void labeledVectorHandling<T>::dump(void) const
{
  ...
  logC() << logMe::dtFormat("[ %40s ] -> %s") % "label" % "type" << std::endl;
  ...
  dt__forAllIndex(*this, ii)
  {
    ...
    logC() << logMe::dtFormat("[ %40s ] -> %s") % obj->getLabel() %
                obj->virtualClassName()
           << std::endl;
  }
}

template <typename T> void labeledVectorHandling<T>::sort(void)
{
...
template <typename T>
std::vector<labelHandling *> labeledVectorHandling<T>::internalStdVector(void)
{
...
template <typename T>
dt__pVH(lVHOInterface) const &labeledVectorHandling<T>::refObserver(void) const
{
  return _observers;
}
} // namespace dtOO
