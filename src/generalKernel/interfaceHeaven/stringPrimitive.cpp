...
std::string stringPrimitive::floatToString(dtReal const value)
{
  ...
  buff << std::defaultfloat
       << std::setprecision(std::numeric_limits<dtReal>::max_digits10) << value;
...
)
{
  ...
  if (!stringContains(pattern, str))
  { //|| !stringContains(pattern, str) ) {
    return std::string("");
  }
...
std::string stringPrimitive::getStringBetween(
  std::string signStart, std::string signEnd, std::string const str
)
{
  if (signStart.empty())
    signStart = "^";
  else
    signStart = regex_escape(signStart);

  if (signEnd.empty())
    signEnd = "$";
  else
    signEnd = regex_escape(signEnd);

  ::boost::smatch what;
  if (::boost::regex_search(
        str, what, ::boost::regex(signStart + "(.*?)" + signEnd)
      ))
  {
    if (what.size() == 2)
      return what[1].str();
...
)
{
  ...
  if ((fromTo.first == 0) && (fromTo.second == 0))
    return std::string("");
...
{
  std::string strBetween = getStringBetweenRespectOcc(signStart, signEnd, *str);

  *str = replaceOnceStringInString(signStart + strBetween + signEnd, "", *str);
  return strBetween;
}
...
)
{
  ...
  for (int ii = 0; ii < str.length(); ii++)
  {
    ...
    if (valueStr.length() == 0 || !stringContains(signStart, valueStr) ||
...
stringPrimitive::convertToCSVStringVector(std::string str)
{
...
{
  if (!stringContains(toReplace, str))
    return str;

  std::string retStr = str;
  retStr.replace(retStr.find(toReplace), toReplace.length(), with);
...
)
{
  ...
  while (counter < str.length())
  {
    ...
    ret.push_back(str.substr(fromTo.first + 1, fromTo.second - fromTo.first - 1)
    );
  }
...
)
{
      ...
      (str.find_first_of(signStart.c_str(), from + 1) < to))
  {
    ...
    std::vector<dtInt> ocSignEnd = getOccurences(signEnd, str, to + 1);
...
)
{
  ...
  while (true)
  {
    ...
    //
    if ((aMatch == std::string::npos) || (aMatch > to))
      return matches;
...
)
{
  ...
  dt__forAllIndex(matchStart, ii)
  {
    ...
    dt__forAllIndex(matchEnd, jj)
    {
      ...
      if (dist[jj] <= 0)
        dist[jj] = std::numeric_limits<int>::max();
    }
    std::vector<dtInt>::iterator minIt =
      std::min_element(dist.begin(), dist.end());
    occMap[matchStart[ii]] = matchEnd[std::distance(dist.begin(), minIt)];
...
std::string stringPrimitive::regex_escape(std::string text)
{
  ...
  text = ::boost::xpressive::regex_replace(
