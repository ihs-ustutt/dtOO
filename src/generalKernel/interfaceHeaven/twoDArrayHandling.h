...
// cpp
//----------------------------------------------------------------------------
template <typename T> twoDArrayHandling<T>::twoDArrayHandling() {}

template <typename T>
twoDArrayHandling<T>::twoDArrayHandling(dtInt const sizeI, dtInt const sizeJ)
...
twoDArrayHandling<T>::twoDArrayHandling(const twoDArrayHandling &orig)
{
  ...
  {
    dt__forAllIndex(orig[ii], jj) { this->at(ii).at(jj) = T(orig[ii][jj]); }
  }
}
...
template <typename T> void twoDArrayHandling<T>::resize(dtInt const sizeI)
{
  std::vector<std::vector<T>>::resize(sizeI);
}

template <typename T> dtInt twoDArrayHandling<T>::size(dtInt const dim) const
{
  if (dim == 0)
  {
...
std::vector<T> twoDArrayHandling<T>::fixJ(dtInt const jj) const
{
  ...
  dt__forAllIndex(*this, ii) { ret[ii] = this->at(ii).at(jj); }
  return ret;
}
...
template <typename T> void twoDArrayHandling<T>::clear(void)
{
  dt__forAllIndex(*this, ii) { this->at(ii).clear(); }
