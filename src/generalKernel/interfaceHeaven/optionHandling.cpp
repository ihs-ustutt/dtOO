...
void optionHandling::jInit(jsonPrimitive const &jE)
{
  ...
  {
    dt__forAllRefAuto(jE.lookup<std::vector<jsonPrimitive>>("option"), anOpt)
    {
      this->setOption(
        anOpt.lookup<std::string>("name"), anOpt.lookup<std::string>("value")
      );
    }
  }
}
...
)
{
  ...
  )
  {
    ...
    option.append<std::string>(
      "name", dtXmlParserBase::getAttributeStr("name", anOption)
    );
    option.append<std::string>(
      "value",
...
void optionHandling::setOption(std::string const name, std::string const value)
{
  ...
  else
  {
    ...
    _optionValue[static_cast<dtInt>(it - _optionName.begin())] = value;
  }
}

std::string
optionHandling::getOption(std::string const name, std::string const val) const
{
...
{
  return staticPropertiesHandler::getInstance()->gDebug() ||
         optionTrue("debug");
}
} // namespace dtOO
