...
std::string dtParser::operator[](std::string const expression) const
{
  ...
  {
    wExpression = stringPrimitive::replaceStringInString(
      "STATE", lVHOstateHandler().commonState(), wExpression
    );
  }

  return dtParser::replaceDependencies(wExpression, _bC, _cV, _aF, _aG);
}

std::vector<dtReal> dtParser::operator()(std::string const expression) const
{
...
)
{
  ...
  while (found < returnExpression.size())
  {
    ...
    if (stringPrimitive::stringContains("[", cVLabel))
    {
      ...
      cVLabel =
        cVLabel + "_" +
        stringPrimitive::intToString(intHandling::round(muParseString(cVOption))
        );
    }
  ...
  while (found < returnExpression.size())
  {
    ...
    returnExpression.replace(
      ...
      stringPrimitive::floatToString(
        muParseString(returnExpression.substr(replaceStart + 1, replaceEnd - 2))
      )
    );
...
{
  if (!aF)
    return replaceDependencies(expression, cV);
  ...
  //
  dt__forAllRefAuto(
    stringPrimitive::crumbleDown("(", ")", expression), aCrumble
  )
  {
    if (!stringPrimitive::stringContains(_AFSIGN, aCrumble))
      continue;
    returnExpression = stringPrimitive::replaceStringInString(
      aCrumble, replaceDependencies(aCrumble, cV, aF), returnExpression
    );
  }
  ...
  while ((found < returnExpression.size()))
  {
    ...
    // replace in argument
    //
    std::string arg = replaceDependencies(
    ...
    {
      aFOption = stringPrimitive::getStringBetweenAndRemove("[", "]", &aFLabel);
    }
    ...
    if (!arg.empty())
    {
      argCS = muParseCSString(replaceDependencies(arg, cV, aF));
    }
    ...
    if (sF)
    {
      ...
      else if (aFOption == "%")
      {
        ...
        dt__throwUnexpected(replaceDependencies());
    }
    else if (v2dF)
    {
      if (aFOption == "")
    ...
    else if (v3dF)
    {
      ...
      else if (stringPrimitive::matchWildcard("%d*Yx", aFOption) ||
    ...
    {
      returnExpression.replace(
        replaceStart, replaceEnd, stringPrimitive::floatToString(pp[0])
      );
    }
    else
      dt__throwUnexpected(replaceDependencies());
...
{
  if (!aG)
    return replaceDependencies(expression, cV, aF);
  ...
  {
    if (!stringPrimitive::stringContains(_AGSIGN, aCrumble))
      continue;
    returnExpression = stringPrimitive::replaceStringInString(
      aCrumble, replaceDependencies(aCrumble, cV, aF, aG), returnExpression
    );
  }
  ...
  while ((found < returnExpression.size()))
  {
    ...
    {
      aGOption = stringPrimitive::getStringBetweenAndRemove("[", "]", &aGLabel);
    }
    ...
    if (m1d)
    {
      ...
      {
        pp = dtLinearAlgebra::toStdVector(m1d->getPoint((*m1d) % argCS[0]));
      }
      else if (aGOption == "%x")
      {
      ...
      {
        pp = dtLinearAlgebra::toStdVector(
          dtLinearAlgebra::toDtPoint3(m1d->firstDerU((*m1d) % argCS[0]))
        );
      }
      else if (aGOption == "%ndU")
      {
        pp = dtLinearAlgebra::toStdVector(dtLinearAlgebra::toDtPoint3(
      ...
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m1d->firstDerU((*m1d) % argCS[0])).x()
        );
      }
      else if (aGOption == "%ndUy")
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m1d->firstDerU((*m1d) % argCS[0])).y()
        );
      }
      else if (aGOption == "%ndUz")
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m1d->firstDerU((*m1d) % argCS[0])).z()
        );
      }
      else if (stringPrimitive::matchWildcard("-1%*X=", aGOption))
      {
        ...
        {
          pp.push_back(
            m1d->percent_u(float_map1dTo3dPointConstCartesian(
      ...
      else if (stringPrimitive::matchWildcard("-1%ClosestPointTo*", aGOption))
      {
        map2dTo3d const *const closeMap2d = map2dTo3d::MustConstDownCast(
          aG->get(stringPrimitive::getStringBetween("@", "@", aGOption))
        );
        pp.push_back(m1d->percent_u(
          pairUUV_map1dTo3dClosestPointToMap2dTo3d(m1d, closeMap2d)
            .result()
            .first
        ));
      }
    ...
    {
      dt__throwIf(
        (argCS.size() != 2) && (argCS.size() != 3), replaceDependencies()
      );
      ...
      if (aGOption == "")
      {
        pp = dtLinearAlgebra::toStdVector(m2d->getPoint(argCS[0], argCS[1]));
      }
      else if (aGOption == "%")
      {
        pp =
          dtLinearAlgebra::toStdVector(m2d->getPointPercent(argCS[0], argCS[1])
          );
      }
      else if (aGOption == "%x")
      {
      ...
      {
        pp = dtLinearAlgebra::toStdVector(
          dtLinearAlgebra::toDtPoint3(m2d->firstDerU(argCS[0], argCS[1]))
        );
      }
      else if (aGOption == "dV")
      {
        pp = dtLinearAlgebra::toStdVector(
          dtLinearAlgebra::toDtPoint3(m2d->firstDerV(argCS[0], argCS[1]))
        );
      }
      else if (aGOption == "%dU")
      {
        pp = dtLinearAlgebra::toStdVector(
          dtLinearAlgebra::toDtPoint3(m2d->firstDerU(
      ...
      {
        pp = dtLinearAlgebra::toStdVector(
          dtLinearAlgebra::toDtPoint3(m2d->firstDerV(
            m2d->uv_percent(argCS[0], argCS[1]).x(),
            m2d->uv_percent(argCS[0], argCS[1]).y()
          ))
        );
      ...
      {
        pp = dtLinearAlgebra::toStdVector(
          dtLinearAlgebra::toDtPoint3(dtLinearAlgebra::normalize(
            m2d->firstDerU(m2d->u_percent(argCS[0]), m2d->v_percent(argCS[1]))
          ))
        );
      }
      else if (aGOption == "%ndV")
      ...
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m2d->firstDerU(argCS[0], argCS[1])).x()
        );
      }
      else if (aGOption == "ndUy")
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m2d->firstDerU(argCS[0], argCS[1])).y()
        );
      }
      else if (aGOption == "ndUz")
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m2d->firstDerU(argCS[0], argCS[1])).z()
        );
      }
      else if (aGOption == "%ndUx")
      {
      ...
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m2d->firstDerV(argCS[0], argCS[1])).x()
        );
      }
      else if (aGOption == "ndVy")
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m2d->firstDerV(argCS[0], argCS[1])).y()
        );
      }
      else if (aGOption == "ndVz")
      {
        pp.push_back(
          dtLinearAlgebra::normalize(m2d->firstDerV(argCS[0], argCS[1])).z()
        );
      }
      else if (aGOption == "%ndVx")
      {
        pp.push_back(
          dtLinearAlgebra::normalize(
            m2d->firstDerV(m2d->u_percent(argCS[0]), m2d->v_percent(argCS[1]))
          )
            .x()
        );
      }
      ...
      {
        pp.push_back(
          m2d->reparamPercentOnFace(dtPoint3(argCS[0], argCS[1], argCS[2])).x()
        );
      }
      else if (aGOption == "-1%v")
      {
        pp.push_back(
          m2d->reparamPercentOnFace(dtPoint3(argCS[0], argCS[1], argCS[2])).y()
        );
      }
      else
        dt__throwUnexpected(replaceDependencies());
    ...
    else if (m3d)
    {
      ...
      {
        pp.push_back(m3d->getPointPercent(argCS[0], argCS[1], argCS[2]).x());
      }
      else if (aGOption == "%y")
      {
        pp.push_back(m3d->getPointPercent(argCS[0], argCS[1], argCS[2]).y());
      }
      else if (aGOption == "%z")
      {
        pp.push_back(m3d->getPointPercent(argCS[0], argCS[1], argCS[2]).z());
      }
      else if (aGOption == "-1")
      {
        pp = dtLinearAlgebra::toStdVector(
...
{
  if (!bC)
    return replaceDependencies(expression, cV, aF, aG);
  ...
  while ((found < returnExpression.size()) && bC)
  {
    ...
    {
      option = stringPrimitive::getStringBetweenAndRemove("[", "]", &label);
    }
    ...
    {
      returnExpression.replace(
        replaceStart, replaceEnd, stringPrimitive::floatToString(thePoint.x())
      );
    }
    else if (option == "y")
    {
      returnExpression.replace(
        replaceStart, replaceEnd, stringPrimitive::floatToString(thePoint.y())
      );
    }
    else if (option == "z")
    {
      returnExpression.replace(
        replaceStart, replaceEnd, stringPrimitive::floatToString(thePoint.z())
      );
    }
    else if (option == "")
    {
  ...
  while ((found < returnExpression.size()) && bC)
  {
    ...
    // find start and end
    //
    dtUnsInt foundEnd = returnExpression.find_first_of(_VECTORSIGN, found + 1);
    dtInt replaceStart = found;
    dtInt replaceEnd = foundEnd - found + 1;
    std::string replaceString =
      returnExpression.substr(replaceStart + 1, replaceEnd - 2);
    ...
    {
      argCS = muParseCSString(replaceDependencies(
        stringPrimitive::getStringBetweenAndRemove("(", ")", &replaceString),
        bC,
        cV,
        aF,
        aG
    ...
    {
      returnExpression.replace(
        replaceStart, replaceEnd, stringPrimitive::floatToString(theVector.x())
      );
    }
    else if (option == "y")
    {
      returnExpression.replace(
        replaceStart, replaceEnd, stringPrimitive::floatToString(theVector.y())
      );
    }
    else if (option == "z")
    {
      returnExpression.replace(
        replaceStart, replaceEnd, stringPrimitive::floatToString(theVector.z())
      );
    }
    ...
    {
      dtVector3 resVector = dtLinearAlgebra::crossProduct(
  ...
  while ((found < returnExpression.size()) && bC)
  {
    ...
    {
      aGOption = stringPrimitive::getStringBetweenAndRemove("[", "]", &TLabel);
    }
...
std::vector<dtReal> dtParser::muParseCSString(std::string const expression)
{
