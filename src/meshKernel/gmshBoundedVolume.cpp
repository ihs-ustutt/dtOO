...
namespace dtOO {
bool gmshBoundedVolume::_registrated = boundedVolumeFactory::registrate(
...
vectorHandling<renderInterface *> gmshBoundedVolume::getExtRender(void) const
{
  ...
  if (toRender == "gmsh")
  {
    dt__forAllRefAuto(_gm->dtEdges(), anEdge)
    {
      ...
      vvV[2] = dtGmshModel::extractPosition(anEdge->getEndVertex()) -
...
void gmshBoundedVolume::makePreGrid(void)
{
  ...
  for (int ii = 0; ii < oG.size(); ii++)
  {
    dt__info(
      makePreGrid(),
      << "GmshSetOption(" << oG[ii].first[0] << ", " << oG[ii].first[1] << ", "
      << oG[ii].second << ");"
    );
  ...
  for (int ii = 0; ii < oGSym.size(); ii++)
  {
    dt__info(
      makePreGrid(),
      << "Define Gmsh symbol " << oGSym[ii].first[0] << " -> "
      << oGSym[ii].second
    );
...
void gmshBoundedVolume::updateBoundingBox(void)
{
  ...
  dt__info(
    ...
    << "size: " << _gm->getFields()->size() << std::endl
