...
namespace dtOO {
bool bVOEvilGodfather::_registrated = bVOInterfaceFactory::registrate(
...
void bVOEvilGodfather::postUpdate(void)
{
  ...
  dt__onlyMaster
  {
    ...
    std::vector<std::pair<std::vector<::MTetrahedron *>, ::GRegion *>>
      newTetrahedra;
    ...
    dt__forFromToIndex(0, static_cast<dtInt>(_perTet * tet.size()), ii)
    {
      ...
      dt__forAllRefAuto(jinxNode, aNode)
      {
        ...
        {
          isFree = false;
          logC() << "Jinx not free, because node is not in region."
                 << std::endl;
          isFree = false;
        }
        if (!isFree)
        ...
        {
          if (dynamic_cast<::MPrism *>(anMe) ||
              dynamic_cast<::MPyramid *>(anMe))
          {
            logC() << "Jinx not free, because pyramid or prism is connected."
                   << std::endl;
            isFree = false;
            break;
          }
      ...
      logC() << dtFormat("(0) MTetrahedron[ %6i ]: V = %8.3e, "
      ...
      if (_perturbTet > 0.)
      {
        ...
        logC() << dtFormat("(1) MTetrahedron[ %6i ]: V = %8.3e, "
                           "skew = %8.3e angleShapeMeasure = %8.3e") %
                    jinx->getNum() % jinx->getVolume() % jinx->skewness() %
                    jinx->angleShapeMeasure()
               << std::endl;
      ...
      else if (_perturbTet < 0.)
      {
        ...
        {
          logC() << dtFormat("(1a) MTetrahedron[ %6i ]: V = %8.3e, "
                             "skew = %8.3e angleShapeMeasure = %8.3e") %
    ...
    std::vector<std::pair<std::vector<::MPyramid *>, ::GRegion *>> newPyramids;
    ...
    dt__forFromToIndex(0, static_cast<dtInt>(_perHex * hex.size()), ii)
    {
      ...
      logC() << dtFormat("(0) MHexahedron[ %6i ]: V = %8.3e, "
                         "skew = %8.3e angleShapeMeasure = %8.3e") %
                  jinx->getNum() % jinx->getVolume() % jinx->skewness() %
                  jinx->angleShapeMeasure()
             << std::endl;
      ...
      if (_perturbHex > 0.)
      {
        ...
        logC() << dtFormat("(1) MHexahedron[ %6i ]: V = %8.3e, "
                           "skew = %8.3e angleShapeMeasure = %8.3e") %
                    jinx->getNum() % jinx->getVolume() % jinx->skewness() %
                    jinx->angleShapeMeasure()
               << std::endl;
      ...
      else if (_perturbHex < 0.)
      {
        ...
        {
          logC() << dtFormat("(1a) MPyramid[ %6i ]: V = %8.3e, "
                             "skew = %8.3e angleShapeMeasure = %8.3e") %
    ...
    {
      dt__ptrAss(::GRegion * gr, dynamic_cast<::GRegion *>(aVert->onWhat()));
    ...
    {
      progHelper::removeChildren(aTetPair.second->tetrahedra, aTetPair.first);
    ...
    {
      progHelper::removeChildren(aHexPair.second->hexahedra, aHexPair.first);
...
)
{
  ...
  dt__forFromToIndex(0, jinx->getNumEdges(), jj)
  {
    ...
    perturbVector.push_back(dtVector3(
...
)
{
  ...
  {
    ::GRegion *gr = dynamic_cast<::GRegion *>(jinx->getVertex(ii)->onWhat());
  ...
  newVertex =
    new ::MVertex(baryXYZ.x(), baryXYZ.y(), baryXYZ.z(), newTets.second);
  ...
  if (jinx->getVolume() > 0.)
  {
    ...
    {
      if (newTets.first[ii]->getVolume() < 0.)
        newTets.first[ii]->reverse();
    }
  }
  else
  {
...
)
{
  ...
  newVertex =
    new ::MVertex(baryXYZ.x(), baryXYZ.y(), baryXYZ.z(), newPyrs.second);
  ...
  if (jinx->getVolume() > 0.)
  {
    ...
    {
      if (newPyrs.first[ii]->getVolume() < 0.)
        newPyrs.first[ii]->reverse();
    }
  }
  else
  {
