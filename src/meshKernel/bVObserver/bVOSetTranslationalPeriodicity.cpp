...
bool bVOSetTranslationalPeriodicity::_registrated =
  bVOInterfaceFactory::registrate(dt__tmpPtr(
    bVOSetTranslationalPeriodicity, new bVOSetTranslationalPeriodicity()
  ));
...
)
{
  ...
  _dtT = translate::PointerDownCast(jE.lookupClone<dtTransformer>(
    "_dtT", bC->constPtrTransformerContainer()->lVH()
  ));
}
...
void bVOSetTranslationalPeriodicity::preUpdate(void)
{
  ...
  {
    dtPoint3 aVertex_t = (*_dtT)(dtGmshModel::extractPosition(aVertex), -1);

    auto minSlave = std::min_element(
      slaveVertex.cbegin(),
      slaveVertex.cend(),
      [=](const ::GVertex *vA, const ::GVertex *vB) {
        dtPoint3 const vA_t = (*_dtT)(dtGmshModel::extractPosition(vA), -1);
        dtPoint3 const vB_t = (*_dtT)(dtGmshModel::extractPosition(vB), -1);
        return (fabs(aVertex_t.x() - vA_t.x()) +
                fabs(aVertex_t.y() - vA_t.y()) + fabs(aVertex_t.z() - vA_t.z())
               ) < (fabs(aVertex_t.x() - vB_t.x()) +
                    fabs(aVertex_t.y() - vB_t.y()) +
                    fabs(aVertex_t.z() - vB_t.z()));
      }
    );
    ...
    distance.push_back(dtLinearAlgebra::distance(
    ...
    dt__info(
      ...
      << std::endl
      << "_dtT[-1](master) = ( "
      << (*_dtT)(dtGmshModel::extractPosition(aVertex), -1) << " )" << std::endl
  ...
  dt__forAllIndex(slaveEdge, ii)
  {
    dtInt masterTag = gm->getDtGmshEdgeTagByFromTo(
  ...
  dt__forAllIndex(masterVertex, ii)
  {
    ...
    //
    if (xyzSlv.distance(xyzTfo) >
        CTX::instance()->geom.tolerance * CTX::instance()->lc)
    {

      double cScale = xyzSlv.distance(xyzTfo) /
                      (CTX::instance()->geom.tolerance * CTX::instance()->lc);
