...
bool bVOSetPrescribedElementSize::_registrated =
  bVOInterfaceFactory::registrate(
    dt__tmpPtr(bVOSetPrescribedElementSize, new bVOSetPrescribedElementSize())
  );
...
std::vector<std::string> bVOSetPrescribedElementSize::factoryAlias(void) const
{
  return ::boost::assign::list_of("bVOSetPrescribedFirstElementSize")(
...
)
{
  ...
  const scaOneDPolyInterface &s1dPoly = scaOneDPolyInterface::ConstRefCast(
    jE.lookupRef<analyticFunction>("_grading", aF)
  );
  dt__throwIf(!s1dPoly.isCompound(), init());
  _grading = analyticFunctionCompoundTrojanHorse<scaOneDPolyInterface>(
...
)
{
  ...
  jE.append<analyticFunction const *>(
    "_grading", aF->get(qtXmlBase::getAttributeStr("gradingLabel", element))
  );
...
void bVOSetPrescribedElementSize::preUpdate(void)
{
  ...
  logContainer<bVOSetPrescribedElementSize> logC(logINFO, "preUpdate()");

  dt__forAllRefAuto(gm->dtEdges(), aEdge)
  {
    if (aEdge->meshAttributes.typeTransfinite ==
        config().lookup<dtInt>("_type"))
    {
      ...
      //
      _polyI.reset(scaOneDPolyInterface::MustDownCast(_polyI->weakClone()));
