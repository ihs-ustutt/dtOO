...
)
{
  ...
  _dtT = xYz_rPhiZ::PointerDownCast(jE.lookupClone<dtTransformer>(
...
void bVOSetRotationalPeriodicity::preUpdate(void)
{
  ...
  dt__forAllIndex(masterEdge, kk)
  {
    ...
    dt__forAllIndex(masterEdge, ii)
    {
      dt__forAllIndex(slaveEdge, jj)
      {
        ...
        {
          dt__info(
            preUpdate(), << "Remove edge with tag " << slaveEdge[jj]->tag()
          );
  ...
  {
    dtPoint3 aVertex_rPhiZ = (*_dtT)(dtGmshModel::extractPosition(aVertex), -1);

    auto minSlave = std::min_element(
      slaveVertex.cbegin(),
      slaveVertex.cend(),
      [=](const ::GVertex *vA, const ::GVertex *vB) {
        dtPoint3 const vA_rPhiZ = (*_dtT)(dtGmshModel::extractPosition(vA), -1);
        dtPoint3 const vB_rPhiZ = (*_dtT)(dtGmshModel::extractPosition(vB), -1);
        return (fabs(aVertex_rPhiZ.x() - vA_rPhiZ.x()) +
                fabs(aVertex_rPhiZ.z() - vA_rPhiZ.z())) <
               (fabs(aVertex_rPhiZ.x() - vB_rPhiZ.x()) +
                fabs(aVertex_rPhiZ.z() - vB_rPhiZ.z()));
      }
    );
