...
bool bVOPMeshRule::_registrated =
  bVOInterfaceFactory::registrate(dt__tmpPtr(bVOPMeshRule, new bVOPMeshRule()));

bVOPMeshRule::bVOPMeshRule() {}

bVOPMeshRule::~bVOPMeshRule() { _meshOperator.destroy(); }
...
)
{
  ...
  _rule1D = qtXmlBase::convertToStringVector(
    "{", "}", qtXmlPrimitive::getStringBetweenAndRemove(":", ":", &rule)
  );
  _rule2D = qtXmlBase::convertToStringVector(
    "{", "}", qtXmlPrimitive::getStringBetweenAndRemove(":", ":", &rule)
  );
  _rule3D = qtXmlBase::convertToStringVector(
    "{", "}", qtXmlPrimitive::getStringBetweenAndRemove(":", ":", &rule)
  );
  _only = qtXmlBase::getAttributeStrVector("only", element);
  dt__throwIf(_only.size() != 0, init());
  _edgeWeight = dtXmlParserBase::getAttributeFloatVectorMuParse(
    "edgeWeight", element, cV, aF
  );
  _faceWeight = dtXmlParserBase::getAttributeFloatVectorMuParse(
    "faceWeight", element, cV, aF
  );
  _regionWeight = dtXmlParserBase::getAttributeFloatVectorMuParse(
...
{
  if (gent->_status == ::GEntity::MeshGenerationStatus::DONE)
  {
    return true;
  }
  else
...
template <typename T> bool bVOPMeshRule::isMeshed(T const *const gent)
{
  if (gent->meshStatistics.status == ::GEntity::MeshGenerationStatus::DONE)
  {
    return true;
  }
  else
...
template bool bVOPMeshRule::isMeshed<dtGmshEdge>(dtGmshEdge const *const);
template bool bVOPMeshRule::isMeshed<dtGmshFace>(dtGmshFace const *const);
...
template <typename T>
std::map<::GEntity *, dtInt>
bVOPMeshRule::prepareDistribution(std::list<T> list, std::vector<dtReal> weight)
{
  ...
  dt__forAllRefAuto(list, anEnt)
  {
    if (!isMeshed(anEnt) && isOwnMaster(anEnt))
    {
      ...
      rankBalance[currentRank] =
        rankBalance[currentRank] + weight[anEnt->tag() - 1];
...
void bVOPMeshRule::preUpdate(void)
{
  ...
  dt__forAllConstIter(std::vector<std::string>, _rule1D, it)
  {
    ...
    dt__forAllRefAuto(ee, aEdge)
    {
      if (!isMeshed(aEdge) &&
          ...
          (onRankMap[aEdge] == thisRank) && isOwnMaster(aEdge))
        (*current1D)(aEdge);
    }
  }
  ...
  if (optionHandling::debugTrue())
  {
    //      dt__onlyMaster {
    gm->writeMSH(
      dtXmlParser::reference().currentState() + "_" +
        ptrBoundedVolume()->getLabel() + "_" +
  ...
  dt__forAllConstIter(std::vector<std::string>, _rule2D, it)
  {
    ...
    dt__forAllRefAuto(ff, aFace)
    {
      if (!isMeshed(aFace) &&
          ...
          (onRankMap[aFace] == thisRank) && isOwnMaster(aFace))
      {
        timeHandling tt(
