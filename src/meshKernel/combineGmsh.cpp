...
bool combineGmsh::_registrated =
  boundedVolumeFactory::registrate(dt__tmpPtr(combineGmsh, new combineGmsh()));
...
void combineGmsh::makeGrid(void)
{
  ...
  {
    ge_newOldV.push_back(_dtGM[ii]->createTwin(
    ...
    dt__forAllRefAuto(_dtGM[ii]->edges(), ge) { _gm->add(ge_newOldV[ii][ge]); }
    dt__forAllRefAuto(_dtGM[ii]->faces(), gf)
    {
      _gm->add(ge_newOldV[ii][gf]);
      if (_dtGM[ii]->getPhysicalString(gf) != "")
      {
        _gm->tagPhysical(ge_newOldV[ii][gf], _dtGM[ii]->getPhysicalString(gf));
      }
    }
    dt__forAllRefAuto(_dtGM[ii]->regions(), gr)
    {
      ...
      {
        _gm->tagPhysical(ge_newOldV[ii][gr], _dtGM[ii]->getPhysicalString(gr));
      }
    }
  }
  ...
  dt__forAllRefAuto(ge_newOldV, ge_newOld)
  {
    ...
    dt__forAllRefAuto(_dtGM[cc]->edges(), ge)
    {
      ...
      {
        dtPoint3 pT = dtGmshModel::extractPosition(mv);
        ::MVertex *mv_clone = new ::MVertex(pT.x(), pT.y(), pT.z(), cloneEdge);
    ...
    dt__forAllRefAuto(_dtGM[cc]->faces(), gf)
    {
      ...
      {
        dtPoint3 pT = dtGmshModel::extractPosition(mv);
        ::MVertex *mv_clone = new ::MVertex(pT.x(), pT.y(), pT.z(), cloneFace);
