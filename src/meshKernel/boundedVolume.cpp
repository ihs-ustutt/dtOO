...
)
{
  ...
  dt__forAllRefAuto(
    _config.lookupDef<std::vector<jsonPrimitive>>(
      "bVObserver", std::vector<jsonPrimitive>()
    ),
    anObs
  )
  {
...
)
{
  ...
  if (qtXmlPrimitive::hasChild("bVObserver", element))
  {
    ...
    {
      bVOInterface *bVI = bVOInterfaceFactory::create(
...
bool boundedVolume::isMeshed(void) const { return _meshed; }
...
bool boundedVolume::isPreMeshed(void) const { return _preMeshed; }
...
jsonPrimitive &boundedVolume::config(void) { return _config; }

jsonPrimitive const &boundedVolume::config(void) const { return _config; }
} // namespace dtOO
