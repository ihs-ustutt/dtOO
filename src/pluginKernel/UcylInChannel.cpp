...
)
{
  ...
  _axis =
    dtXmlParser::getDtVector3(dtXmlParser::getChild("Vector_3", element), bC);
  _origin =
    dtXmlParser::getDtPoint3(dtXmlParser::getChild("Point_3", element), bC);
}
  ...
  dt__onlyMaster
  {
    ...
    try
    {
      ...
      ::Foam::IOobject UcylHeader(
        "Ucyl", runTime.timeName(), mesh, ::Foam::IOobject::MUST_READ
      );
      ...
      //
      dt__throwIf(
        !UcylHeader.headerOk() && !UHeader.headerOk() &&
...
#pragma omp for
          dt__forAllIndex(grid, ii)
          {
            dt__forAllIndex(grid[ii], jj)
            {
              ...
              else
              {
                ...
                UcylValue = interUCyl.interpolate(
                  ::Foam::vector(xyz.x(), xyz.y(), xyz.z()), cId
                );
              }
        ...
        dt__forFromToIndex(0, grid.size(), ii)
        {
          dt__forFromToIndex(0, grid[ii].size(), jj)
          {
            of << logMe::dtFormat("%4i, %4i, "
                    ...
                    Ucyl[ii][jj].y() % Ucyl[ii][jj].z() % U_n[ii][jj] %
                    Ucyl_n[ii][jj] % A[ii][jj]
               << std::endl;
          }
        }
        of.close();
        ...
        dt__forFromToIndex(0, grid.size(1), jj)
        {
          ...
          of << logMe::dtFormat("%16i, "
                  ...
                  (sumUcylAU_n.x() / sumAU_n) % (sumUcylAU_n.y() / sumAU_n) %
                  (sumUcylAU_n.z() / sumAU_n) % (sumU_nAU_n / sumAU_n)
             << std::endl;
        }
        of.close();
      }
