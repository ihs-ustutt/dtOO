...
namespace dtOO {
bool volVectorInChannelFieldRange::_registrated = dtPluginFactory::registrate(
      ...
      dt__forFromToIndex(0, grid.size(0), ii)
      {
        dt__forFromToIndex(0, grid.size(1), jj)
        {
          ...
          of << logMe::dtFormat(
                  ...
                  val[ii][jj].x() % val[ii][jj].y() % val[ii][jj].z() %
                  sf[ii][jj].x() % sf[ii][jj].y() % sf[ii][jj].z() % phi[ii][jj]
             << std::endl;
        }
      }
      of.close();
