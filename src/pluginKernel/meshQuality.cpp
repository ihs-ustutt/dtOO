...
void meshQuality::apply(void)
{
  ...
  dt__onlyMaster
  {
    ...
    try
    {
      ...
      barChart bcSkew("skew", minSkew.value(), maxSkew.value(), 10);
      barChart bcNonOrtho(
        "nonOrtho", minNonOrto.value(), maxNonOrto.value(), 10
      );
      std::vector<barChart> bcElemSkew(
        4, barChart("bcElemSkew", minSkew.value(), maxSkew.value(), 10)
      );
      ...
      //
      std::string filename =
        _case->getDirectory(_parser->currentState()) + "/" + "meshQuality.csv";
