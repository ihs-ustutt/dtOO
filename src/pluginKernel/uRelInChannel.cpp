        ...
        dt__forAllRefAuto(_aG, anAG) {
          ...
          dt__forFromToIndex(0, grid.size(), ii) {
            dt__forFromToIndex(0, grid[ii].size(), jj) {              
              ...
              << logMe::dtFormat(
                ...
                % uRel[ii][jj].x() % uRel[ii][jj].y() % uRel[ii][jj].z()
                % U_n[ii][jj] % uRel_n[ii][jj]
                % A[ii][jj]
              << std::endl;
            }
          }          
          ...
          dt__forFromToIndex(0, grid.size(1), jj) {
            ...
            << logMe::dtFormat(
              ...
              % avguRel.x() % avguRel.y() % avguRel.z()
              % avgU_n % avguRel_n
              % ( sumuRelAU_n.x() / sumAU_n ) 
              % ( sumuRelAU_n.y() / sumAU_n )
              % ( sumuRelAU_n.z() / sumAU_n )
              % ( sumU_nAU_n / sumAU_n )
            << std::endl;            
          }           
          of.close();          
