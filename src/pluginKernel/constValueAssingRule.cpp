...
namespace dtOO {
bool constValueAssingRule::_registrated = dtPluginFactory::registrate(
...
)
{
  ...
  _assignRule = dtXmlParser::getAttributeStrVector("assignRule", element);
...
void constValueAssingRule::apply(void)
{
  ...
  dt__forAllConstIter(std::vector<std::string>, _assignRule, it)
  {
    ...
    std::vector<dtReal> val =
      dtXmlParser::muParseCSString(dtXmlParser::replaceDependencies(
        stringPrimitive::getStringBetweenAndRemove(":", ":", &theString),
        _bC,
        _cV,
        _aF,
        _aG
