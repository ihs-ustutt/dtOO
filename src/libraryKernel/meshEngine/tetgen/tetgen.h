...
#define fastlookup(pool, index)                                                \
  ...
  bool
  orthosphere(REAL *, REAL *, REAL *, REAL *, REAL, REAL, REAL, REAL, REAL *, REAL *);
  ...
  int report_selfint_edge(
    point, point, face *sedge, triface *searchtet, enum interresult
  );
  ...
  // // PLC is also a vertex or an edge of the CDT, every polygon of the PLC is
  // a // union of triangles of the CDT. A crucial difference between a CDT and
  // a   // DT is that triangles in the PLC's polygons are not required to be
  // locally // Delaunay, which frees the CDT to better respect the PLC's
  // polygons. CDTs  // have optimal properties similar to those of DTs. //
  //                                                                           //
  ...
  // create Steiner CDTs.      // Almost all of the Steiner points are added in
  // the edges of the PLC. They  // guarantee the existence of a CDT of the
  // modified PLC.                     //
  //                                                                           //
  ...
  // // is constructed by two steps, (1) segment recovery and (2) facet
  ...
  // Constr- // ained Delaunay Tetrahedralizations," In Proceedings of the 14th
  // Internat- // ional Meshing Roundtable, 147--163, 2005.  It adds Steiner
  // points into    // non-Delaunay segments until all subsegments appear
  // together in a DT. The  // running time of this algorithm is proportional to
  // the number of added     // Steiner points. //
  //                                                                           //
  ...
  // Proceedings of the 19th  // ACM Symposium on Computational Geometry, 86-95,
  // 2003.                     //
  //                                                                           //
  ...
  // of // Steiner points. It is discussed in the paper of Si, H., and Shewchuk,
  // J.,// "Incrementally Constructing and Updating Constrained Delaunay //
  ...
  // subroutines,// delaunizecavity(), fillcavity(), and carvecavity(). Since it
  // may fail due // to non-coplanar vertices, the subroutine restorecavity() is
  // used to rest- // ore the original cavity. //
  //                                                                           //
  ...
  bool
  formcavity(triface *, arraypool *, arraypool *, arraypool *, arraypool *, arraypool *, arraypool *);
  // Facet recovery by cavity re-triangulation [Si and Gaertner 2011].
  void
  delaunizecavity(arraypool *, arraypool *, arraypool *, arraypool *, arraypool *, arraypool *);
  bool fillcavity(
    arraypool *,
    arraypool *,
    arraypool *,
  ...
  void
  refineregion(face &, arraypool *, arraypool *, arraypool *, arraypool *, arraypool *, arraypool *);
  ...
  // all // tetrahedra of the output mesh have a "radius-edge ratio" (equivalent
  // to   // the minimal face angle) bounded. However, it does not remove
  // slivers, a   // type of very flat tetrahedra which can have no small face
  // angles but have // very small (and large) dihedral angles. Moreover, it may
  // not terminate if // the input PLC contains "sharp features", e.g., two
  // edges (or two facets)  // meet at an acute angle (or dihedral angle). //
  //                                                                           //
  ...
  // domain. Moreover, it supports the generation // of adaptive mesh according
  // to a (isotropic) mesh sizing function.         //
  //                                                                           //
  ///////////////////////////////////////////////////////////////////////////////
