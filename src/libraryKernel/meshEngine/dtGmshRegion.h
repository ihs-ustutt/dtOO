...
public:
  ...
  void addFace(dtInt const &faceId, dtInt const &ori);
  ...
  static bool isEqual(::GRegion const *const gr0, ::GRegion const *const gr1);
  void replaceFaces(std::vector<GFace *> &new_faces);
  void replaceFace(::GFace const *const toReplace, ::GFace *const with);
  void setGrading(
    std::vector<dtReal> const &grading, const std::vector<dtReal> &type
  );
  std::list<dtGmshFace const *>
  constFaceList(std::vector<std::string> const &label) const;
  std::list<dtGmshFace *> faceList(std::vector<std::string> const &label);
