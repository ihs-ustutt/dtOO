...
{
  ::Foam::Map<::Foam::label>::const_iterator zoneFnd = physToZone.find(regPhys);
...
)
{
  ...
  dt__forAllRefAuto(oneD, aPair)
  {
    physicalNames.insert(
      ...
      ::Foam::string::validate<::Foam::word>(gm->getPhysicalName(1, aPair.first)
      )
    );
    dt__info(
      readPhysNames(),
  ...
  dt__forAllRefAuto(twoD, aPair)
  {
    physicalNames.insert(
      ::Foam::scalar(aPair.first),
      ::Foam::string::validate<::Foam::word>(gm->getPhysicalName(2, aPair.first)
      )
    );
    dt__info(
      readPhysNames(),
  ...
  dt__forAllRefAuto(threeD, aPair)
  {
    physicalNames.insert(
      ::Foam::scalar(aPair.first),
      ::Foam::string::validate<::Foam::word>(gm->getPhysicalName(3, aPair.first)
      )
    );
    dt__info(
      readPhysNames(),
...
)
{
  ...
  dt__forAllRefAuto(allElems, aPair)
  {
    ...
    else if (elmType == dtFoamLibrary::MSHTRI)
    {
      ...
      ::Foam::Map<::Foam::label>::iterator regFnd = physToPatch.find(regPhys);
    ...
    else if (elmType == dtFoamLibrary::MSHPRISM)
    {
      ...
      if (!keepOrientation && !dtFoamLibrary::correctOrientation(points, cell))
      {
    ...
    else if (elmType == dtFoamLibrary::MSHPNT)
    {
      ...
      dt__info(readCells(), << "Unhandled element " << elmType);
  ...
  if (cells.size() == 0)
    dt__throw(readCells(), << "No cells read.");
...
)
{
  ...
  forAll(boundaryPatchNames, patchI)
  {
    ...
    {
      boundaryPatchNames[patchI] = ::Foam::word("patch") + ::Foam::name(patchI);
    }
  ...
  ::Foam::polyMesh *ptrMesh = new ::Foam::polyMesh(
    ::Foam::IOobject(
      ::Foam::polyMesh::defaultRegion, runTime.constant(), runTime
    ),
#ifdef DTOO_HAS_FOAMEXT
...
#ifdef DTOO_HAS_OpenFOAM
    std::move(points
    ), // https://develop.openfoam.com/Development/openfoam/-/issues/713
#endif
    cells,
    boundaryFaces,
    boundaryPatchNames,
  ...
  ::Foam::List<::Foam::DynamicList<::Foam::label>> zoneFaces(patchFaces.size());

  // Go through all the patchFaces and find corresponding face in pp.
  forAll(patchFaces, patchI)
  {
    const ::Foam::DynamicList<::Foam::face> &pFaces = patchFaces[patchI];
    ...
    forAll(pFaces, i)
    {
      ...
      if (patchFaceI != -1)
      {
        //          dt__info(readMesh(), << "Correspoding patchFace " <<
        //          patchFaceI);
      ...
      else
      {
        ...
        // - might be useful.
        ::Foam::label meshFaceI = dtFoamLibrary::findInternalFace(mesh, f);

        if (meshFaceI != -1)
        {
          //            dt__info(readMesh(), << "Internal face: meshFaceI = " <<
          //            meshFaceI);
  ...
  if (nValidCellZones > 0)
  {
    ...
    forAll(zoneCells, zoneI)
    {
      if (zoneCells[zoneI].size())
      {
        ...
        dt__info(
          readMesh(),
          << "Writing zone " << zoneI << " to cellZone " << zoneName
          << " and cellSet"
        );
        ...
        cz[nValidCellZones] = new ::Foam::cellZone(
  ...
  if (nValidFaceZones > 0)
  {
    ...
    forAll(zoneFaces, zoneI)
    {
      if (zoneFaces[zoneI].size())
      {
        ...
        dt__info(
          readMesh(),
          << "Writing zone " << zoneI << " to faceZone " << zoneName
          << " and faceSet"
        );
        ...
        fz[nValidFaceZones] = new ::Foam::faceZone(
          zoneName,
          zoneFaces[zoneI],
          ::Foam::boolList(zoneFaces[zoneI].size(), true),
          nValidFaceZones,
          mesh.faceZones()
        );
...
)
{
  ...
  ::Foam::patchToFace(mesh, patch.name())
    .applyToSet(::Foam::topoSetSource::ADD, tS);
#endif
#ifdef DTOO_HAS_OpenFOAM
  ::Foam::faceSet tS(
    mesh,
  ...
  ::Foam::patchToFace(mesh, ::Foam::wordRe(patch.name()))
    .applyToSet(::Foam::topoSetSource::ADD, tS);
#endif
...
)
{
  ...
  ::Foam::boxToCell(mesh, ::Foam::treeBoundBox(min, max))
    .applyToSet(::Foam::topoSetSource::ADD, tS);
#endif
#ifdef DTOO_HAS_OpenFOAM
  ::Foam::cellSet tS(
  ...
  {
    dt__info(
      boxToCellSet(), << "Adding set " << tS.name() << " as a cellZone."
    );
    ::Foam::label sz = mesh.cellZones().size();
    mesh.cellZones().setSize(sz + 1);
    mesh.cellZones().set(
      sz,
      new ::Foam::cellZone(tS.name(), addressing.shrink(), sz, mesh.cellZones())
    );
...
)
{
  ...
  forAllConstIter(::Foam::dictionary, contentDict.subDict("controlDict"), it)
  {
    it().write(os());
    os.endl();
  }
...
)
{
  ...
  forAll(toc, i)
  {
        ...
        !contentDict.subOrEmptyDict(toc[i]).empty())
    {
      ...
      // create IODictionary
      //
      ::Foam::IOdictionary ioDict(::Foam::IOobject(
        fn.name(),
        fn.path(),
        fvMesh,
...
dt__pH(::Foam::argList
) dtFoamLibrary::initCase(std::string const &appName, std::string const &wDir)
{
  //
