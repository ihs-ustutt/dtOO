...
dtGmshModel::dtGmshModel(std::string name, ::FieldManager *fm) : GModel(name)
{
  _debug = "";
...
void dtGmshModel::addIfFaceToGmshModel(
  ...
  dtInt *const tag,
  std::list<::GEdge *> const &edges,
  std::vector<dtInt> const &ori
)
{
...
::GEntity *dtGmshModel::getGEntityByPhysical(std::string const &physical) const
{
  dt__throwIf(stringPrimitive::isWildcard(physical), getGEntityByPhysical());
...
std::list<dtGmshFace *>
dtGmshModel::getDtGmshFaceListByPhysical(std::string const &physical) const
{
...
dtGmshModel::getDtGmshRegionListByPhysical(std::string const &physical) const
{
...
dtGmshEdge *
dtGmshModel::getDtGmshEdgeByFromTo(dtInt const from, dtInt const to) const
{
...
dtGmshModel::cast2DtGmshEdge(std::vector<::GEdge *> edges)
{
...
std::vector<dtGmshFace *>
dtGmshModel::cast2DtGmshFace(std::vector<::GFace *> faces)
{
...
std::list<dtGmshFace *>
dtGmshModel::cast2DtGmshFace(std::list<::GEntity *> faces)
{
  std::list<dtGmshFace *> ret;
  dt__forAllRefAuto(faces, aFace) ret.push_back(cast2DtGmshFace(aFace));
  return ret;
...
std::list<dtGmshRegion *>
dtGmshModel::cast2DtGmshRegion(std::list<::GRegion *> regions)
{
...
std::list<dtGmshRegion *>
dtGmshModel::cast2DtGmshRegion(std::list<::GEntity *> regions)
{
  std::list<dtGmshRegion *> ret;
...
std::list<dtGmshVertex *>
dtGmshModel::cast2DtGmshVertex(std::list<::GVertex *> vertices)
{
...
std::vector<dtGmshVertex *>
dtGmshModel::cast2DtGmshVertex(std::vector<::GVertex *> vertices)
{
...
{
  return cast2DtGmshVertex(const_cast<::GVertex *>(gv))->cast2DtPoint3();
}
...
{
  for (::GModel::viter vIt = __caCThis->firstVertex();
...
{
  for (::GModel::eiter eIt = __caCThis->firstEdge();
...
{
  for (::GModel::fiter fIt = __caCThis->firstFace();
...
{
  for (::GModel::riter rIt = __caCThis->firstRegion();
...
unstructured3dMesh *dtGmshModel::toUnstructured3dMesh(void) const
{
  ...
  //
  for (::GModel::riter r_it = __caCThis->firstRegion();
...
)
{
  ...
  for (dtInt ii = 0; ii < elements.size(); ii++)
  {
    ...
    for (dtInt ii = 0; ii < verts.size(); ii++)
      vertices.push_back(verts[ii]);
  }
...
discrete3dPoints *
dtGmshModel::toDiscrete3dPoints(std::vector<::MVertex const *> const &vertices)
{
  std::vector<dtPoint3> pp(vertices.size());
  for (dtInt ii = 0; ii < vertices.size(); ii++)
  {
...
{
  int maxTag = 0;
  for (::GModel::viter v_it = GModel::vertices.begin();
...
{
  int maxTag = 0;
  for (::GModel::eiter e_it = GModel::edges.begin();
...
{
  int maxTag = 0;
  for (::GModel::fiter f_it = GModel::faces.begin();
...
{
  int maxTag = 0;
  for (::GModel::riter r_it = GModel::regions.begin();
...
{
  std::list<::GVertex *> ll;
  for (::GModel::viter it = GModel::vertices.begin();
...
{
  std::list<::GEdge *> ll;
  for (::GModel::eiter it = GModel::edges.begin(); it != GModel::edges.end();
       ++it)
    ll.push_back(*it);
...
) const
{
  ...
  {
    names.push_back(::GModel::getPhysicalName(dim, aNumber));
  }
  return names;
}
  ...
  dt__forFromToIndex(0, 4, dim)
  {
    ...
    dt__forAllConstIter(dtGmshModel::intGEntityVMap, ge_number, nIt)
    {
      logC() << "  id = " << nIt->first << " / "
             ...
             << nIt->second.size() << " GEntities" << std::endl;
      if (nIt->second.empty())
      {
        ...
        keepName[std::pair<int, dtInt>(dim, nIt->first)] = true;
    }
  }
  dt__forAllRefAuto(keepName, aPair)
  {
    if (_physicalNames.find(aPair.first) == _physicalNames.end())
      continue;
    logC() << "Check " << _physicalNames.at(aPair.first) << std::endl;
    if (!aPair.second)
    {
      _physicalNames.erase(_physicalNames.find(aPair.first));
      logC() << "  --> erase" << std::endl;
    }
  }
}
...
std::list<std::string>
dtGmshModel::getFullPhysicalList(::GEntity const *const ent) const
{
  ...
  else if (dtGmshFace::ConstDownCast(ent))
  {
    ...
    dt__forAllRefAuto(face->dtRegions(), aReg)
    {
      ...
      dt__forAllRefAuto(getFullPhysicalList(aReg), aPrefix)
      {
        ...
        dtInt faceI =
          std::distance(fL.begin(), std::find(fL.begin(), fL.end(), face));
  ...
  else if (dtGmshEdge::ConstDownCast(ent))
  {
    ...
    dt__forAllRefAuto(edge->dtFaces(), aFace)
    {
      ...
      dt__forAllRefAuto(getFullPhysicalList(aFace), aPrefix)
      {
        ...
        dtInt edgeI =
          std::distance(eL.begin(), std::find(eL.begin(), eL.end(), edge));
        //
        // add position alias
        //
  ...
  else if (dtGmshVertex::ConstDownCast(ent))
  {
    ...
    dt__forAllRefAuto(vertex->dtEdges(), aEdge)
    {
      ...
      dt__forAllRefAuto(getFullPhysicalList(aEdge), aPrefix)
      {
        ...
        dtInt vertexI =
          std::distance(vL.begin(), std::find(vL.begin(), vL.end(), vertex));
        //
        // add position alias
        //
...
std::list<::GEntity *>
dtGmshModel::getGEntityListByWildCardPhysical(std::string wildStr) const
{
  //    ::boost::smatch what;
  ...
  if (physV.size() > 0)
  {
    ...
    else
    {
      dt__forAllRefAuto(dtRegions(), aReg)
      {
        if (stringPrimitive::matchWildcard(physV[0], aReg->getPhysicalString()))
        {
    ...
    if (physV.size() > 1)
    {
      dt__forAllRefAuto(region, aReg)
      {
        ...
        else
        {
          ...
          {
            if (stringPrimitive::matchWildcard(
...
) const
{
  ...
  if (!wildStr.empty())
  {
    ...
    ::boost::smatch what;
    while (
      ::boost::regex_search(st, en, what, ::boost::regex("([-][>])?+([^->]+)"))
    )
    {
      physV.push_back(what.str(2));
      st = what[0].second;
  ...
  dt__forAllRefAuto(getFullPhysicalList(ge), aPhysical)
  {
    if (!stringPrimitive::isWildcard(wildStr))
    {
      ...
      {
        dt__debug(
          matchWildCardPhysical(), << "Direct match with wildStr = " << wildStr
        );
        return true;
      }
    }
    if (stringPrimitive::matchWildcard(wildStrWork, aPhysical))
    {
      dt__debug(
        ...
        << "Match with wildStrWork = " << wildStrWork << " == " << aPhysical
      );
      return true;
    }
  }
...
void dtGmshModel::setDebug(std::string const debug) { _debug = debug; }
...
) const
{
  ...
  dt__forAllRefAuto(dtGmshModel::cast2DtGmshVertex(vertices()), aGV)
  {
    ...
    dt__forAllRefAuto(dtGmshModel::cast2DtGmshEdge(aGV->edges()), aGE)
    {
  ...
  dt__forAllRefAuto(dtGmshModel::cast2DtGmshEdge(edges()), aGE)
  {
    ...
    clone->setBeginVertex(dim0[dtGmshVertex::DownCast(aGE->getBeginVertex())]);
    clone->setEndVertex(dim0[dtGmshVertex::DownCast(aGE->getEndVertex())]);
    // update vertices
    dt__forAllRefAuto(dtGmshModel::cast2DtGmshVertex(aGE->vertices()), aGV)
    {
      clone->addVertex(dim0[aGV]);
    }
    // update faces
    dt__forAllRefAuto(dtGmshModel::cast2DtGmshFace(aGE->faces()), aGF)
    {
  ...
  dt__forAllRefAuto(dtGmshModel::cast2DtGmshFace(faces()), aGF)
  {
    ...
    // update edges
    dt__forAllRefAuto(dtGmshModel::cast2DtGmshEdge(aGF->edges()), aGE)
    {
      clone->addEdge(dim1[aGE], aGF->edgeOrientation(aGE));
    }
    // update regions
    dt__forAllRefAuto(dtGmshModel::cast2DtGmshRegion(aGF->regions()), aGR)
    {
  ...
  dt__forAllRefAuto(dtGmshModel::cast2DtGmshRegion(regions()), aGR)
  {
    ...
    // update faces
    dt__forAllRefAuto(dtGmshModel::cast2DtGmshFace(aGR->faces()), aGF)
    {
