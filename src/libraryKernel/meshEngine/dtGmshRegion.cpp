...
dtGmshRegion::dtGmshRegion(::GModel *m, dtInt tag)
  : GRegion(m, tag), _geomType(::GEntity::GeomType::Unknown)
{
...
{
  dt__ptrAss(dtGmshModel const *const gm, dtGmshModel::ConstDownCast(model()));

  return *gm;
}

::GEntity::GeomType dtGmshRegion::geomType(void) const { return _geomType; }
...
)
{
  ...
  //
  std::vector<dtGmshFace *> ff = dtGmshModel::cast2DtGmshFace(faces());
  dt__throwIfWithMessage(
    ff.size() != 6, meshWNElements(), << "ff.size() = " << ff.size()
  );
...
void dtGmshRegion::meshRecombine(void) { this->meshAttributes.recombine3D = 1; }
...
void dtGmshRegion::meshUnstructured(void)
{
  ...
  std::vector<::GFace *> fl = faces();
  for (std::vector<::GFace *>::iterator f_it = fl.begin(); f_it != fl.end();
       ++f_it)
  {
    std::vector<::GEdge *> el = (*f_it)->edges();
    for (std::vector<::GEdge *>::iterator e_it = el.begin(); e_it != el.end();
         ++e_it)
...
{
  addFace(
    dynamic_cast<::GFace *>(refDtGmshModel().getDtGmshFaceByTag(faceId)), ori
  );
}
...
void dtGmshRegion::deleteFace(::GFace *face)
{
  ...
  for (i = l_faces.begin(), j = l_dirs.begin();
       i != l_faces.end() && j != l_dirs.end();
       ++i, ++j)
  {
    if ((*i) != face)
    {
...
void dtGmshRegion::addElement(::MElement *me)
{
  ...
  if (mtet)
    ...
    dt__throwUnexpected(addElement());
}
...
std::list<dtGmshFace const *>
dtGmshRegion::constFaceList(std::vector<std::string> const &label) const
{
  std::list<dtGmshFace const *> constFaceList;
  dt__forAllConstIter(std::vector<std::string>, label, it)
  {
    dt__forAllRefAuto(refDtGmshModel().getDtGmshFaceListByPhysical(*it), aFace)
    {
...
std::list<dtGmshFace *>
dtGmshRegion::faceList(std::vector<std::string> const &label)
{
