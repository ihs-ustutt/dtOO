...
dtGmshFace::dtGmshFace(::GModel *m, dtInt tag)
  : GFace(m, tag), _geomType(::GEntity::GeomType::Unknown)
{
}
...
dtGmshFace::dtGmshFace(
  ...
  : GFace(m, tag), _geomType(::GEntity::GeomType::Unknown)
{
  dt__forAllRefAuto(edges, aEdge) { aEdge->addFace(this); }
...
std::pair<SVector3, SVector3> dtGmshFace::firstDer(const SPoint2 &param) const
{
  if (!_mm->inRange(dtPoint2(param.x(), param.y())))
  {
    dt__ddebug(
      firstDer(),
      << dtFormat("Accessing derivative out of range at point (%e, %e). "
                  "Bounds U = [%e, %e] / V = [%e, %e]") %
...
void dtGmshFace::addEdgeLoop(std::list<::GEdge *> edgeL)
{
  ...
  {
    for (std::list<::GEdge *>::iterator it = edgeL.begin(); it != edgeL.end();
         ++it)
    {
  ...
  if (edgeL.size() != 0)
  {
    dt__throw(addEdgeLoop(), << dt__eval(edgeL.size()));
  }
}
...
void dtGmshFace::addEdgeLoop(std::vector<dtInt> const &edgeIds)
{
  ...
  {
    edgeL.push_back(this->refDtGmshModel().getDtGmshEdgeByTag(edgeId));
  }
...
void dtGmshFace::meshRecombine(void) { this->meshAttributes.recombine = 1; }
...
void dtGmshFace::correctIfTransfinite(void)
{
  ...
  if (ee[0]->meshAttributes.nbPointsTransfinite !=
      ee[2]->meshAttributes.nbPointsTransfinite)
  {
    correct[0] = true;
  }
  if (ee[1]->meshAttributes.nbPointsTransfinite !=
  ...
  //
  ee[0]->meshAttributes.nbPointsTransfinite = std::max(
    ee[0]->meshAttributes.nbPointsTransfinite,
    ee[2]->meshAttributes.nbPointsTransfinite
  );
  ee[1]->meshAttributes.nbPointsTransfinite = std::max(
...
) const
{
  ...
  int num_vertices_per_element = __caCThis->getMeshElement(0)->getNumVertices();
...
void dtGmshFace::addElement(::MElement *me)
{
  ...
  if (mquad)
    ...
    dt__throw(addElement(), << "Try to add unsupported element.");
}
...
void dtGmshFace::addGEntity(::GEntity *const gEnt)
{
  ...
  else
  {
    ...
    if ((getRegion(0) != gr) && (getRegion(1) != gr))
      addRegion(gr);
  }
}
...
{
  if (isOnFace(ge->getBeginVertex()) && isOnFace(ge->getEndVertex()))
  {
    return true;
  }
  else
