...
#pragma omp for
  ...
  */
  ...
  dtInt nCorrLoops = staticPropertiesHandler::getInstance()->getOptionInt(
  ...
  dt__forFromToIndex(0, 4, ii)
  {
    ...
    dt__forFromToIndex(0, nCorrLoops, cc)
    {
      ...
      if (bound[ii].front() < bound[ii].back())
      {
        ...
        // last
        if (bound[ii][boundSize - 2] > bound[ii][boundSize - 1])
        {
      ...
      {
        if (ii == 0)
          pUV[jj][0] = dtPoint2(bound[ii][jj], pUV[jj][0].y());
        else if (ii == 1)
          pUV[L][jj] = dtPoint2(pUV[L][jj].x(), bound[ii][jj]);
        else if (ii == 2)
          pUV[jj][H] = dtPoint2(bound[ii][jj], pUV[jj][H].y());
        else if (ii == 3)
          pUV[0][jj] = dtPoint2(pUV[0][jj].x(), bound[ii][jj]);
      }
    }
    ...
    logC() << dtLog::dtFormat("[ %3d ]   Ok") % ii << std::endl;
  }
...
#pragma omp for
    for (int i = 0; i < L; i++)
    {
      for (int j = 0; j < H; j++)
      {
        ...
        {
          //            dtgf->quadrangles.push_back( new MQuadrangle(v1, v2, v3,
          //            v4) );
