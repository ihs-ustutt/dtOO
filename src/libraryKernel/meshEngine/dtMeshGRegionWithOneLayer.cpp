...
)
{
  ...
  jE.append<std::string>(
    "_faceMaster", qtXmlPrimitive::getAttributeStr("faceMaster", element)
  );
  jE.append<std::string>(
    "_faceSlave", qtXmlPrimitive::getAttributeStr("faceSlave", element)
  );
...
{
  Msg::Info("Meshing volume %d ( dtMeshGRegionWithOneLayer )", dtgr->tag());

  //
  // get master and slave face and check if faces belong to this volume
  //
  dtGmshFace *masterFace = dtgr->refDtGmshModel().getDtGmshFaceByPhysical(
    config().lookup<std::string>("_faceMaster")
  );
  dt__throwIf(
    std::find(dtgr->dtFaces().begin(), dtgr->dtFaces().end(), masterFace) ==
      dtgr->dtFaces().end(),
    operator()()
  );
  dtGmshFace *slaveFace = dtgr->refDtGmshModel().getDtGmshFaceByPhysical(
    config().lookup<std::string>("_faceSlave")
  );
  dt__throwIf(
    std::find(dtgr->dtFaces().begin(), dtgr->dtFaces().end(), slaveFace) ==
      dtgr->dtFaces().end(),
    operator()()
  );
  ...
  dt__forFromToIndex(0, masterFace->getNumMeshElements(), ii)
  {
    ...
    if (dynamic_cast<::MTriangle *>(me))
    {
      ...
      {
        dt__ptrAss(
          ::MVertex * mv, slaveFace->correspondingVertices[me->getVertex(jj)]
        );
        vv.push_back(mv);
      }
