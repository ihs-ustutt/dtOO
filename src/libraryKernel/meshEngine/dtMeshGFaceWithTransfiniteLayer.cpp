...
)
{
  ...
  {
    nSmooth = dtXmlParserBase::getAttributeIntMuParse("nSmooth", element, cV);
  }
...
#pragma omp for
      dt__forAllIndex(ordered[ii].second->mesh_vertices, jj)
      {
        ...
        edgeVertex[ii][jj + 1] = std::make_pair(
          aVert, m2d()->reparamOnFace(dtGmshModel::extractPosition(aVert))
        );
      }
    }
    ...
    reparamMeshVertexOnFace(mv, dtgf, param);
    edgeVertex[ii].back() = std::make_pair(mv, dtPoint2(param[0], param[1]));

    //
    // correct orientation
    //
    if (ordered[ii].first != 1)
      progHelper::reverse(edgeVertex[ii]);
  }
  progHelper::reverse(edgeVertex[2]);
  progHelper::reverse(edgeVertex[3]);

  std::vector<dtInt> nLayers = config().lookup<std::vector<dtInt>>("_nLayers");
  //
  // sheet 0
  //
  twoDArrayHandling<::MVertex *> sheet0(edgeVertex[0].size(), nLayers[0] + 1);
  twoDArrayHandling<dtVector3> nn0(edgeVertex[0].size(), nLayers[0] + 1);
  twoDArrayHandling<dtPoint2> uv0(edgeVertex[0].size(), nLayers[0] + 1);
  ...
  dt__forFromToIndex(1, nLayers[0] + 1, layer)
  {
    ...
    dtReal distInc =
      (dtLinearAlgebra::length(dist1) + dtLinearAlgebra::length(dist0)) / 2.0;

    //
    // calculate normals
    //
    std::vector<dtVector3> NN(sheet0.fixJ(layer).size());
    NN[0] = dtLinearAlgebra::normalize(dist0);
    NN[sheet0.fixJ(layer).size() - 1] = dtLinearAlgebra::normalize(dist1);
    ...
    dt__forInnerIndex(sheet0.fixJ(layer), node)
    {
      ...
      //
      NN[node] = dtLinearAlgebra::normalize(dtLinearAlgebra::crossProduct(
        dtVector3(
    ...
    if (dtLinearAlgebra::dotProduct(
          NN[sheet0.fixJ(layer).size() - 1], NN[sheet0.fixJ(layer).size() - 2]
        ) < 0.)
    {
    ...
    dt__forFromToIndex(0, config().lookup<dtInt>("_nSmooth"), thisSmooth)
    {
      dt__forInnerIndex(sheet0.fixJ(layer), node)
      {
        NN[node] = dtLinearAlgebra::normalize(NN[node - 1] + NN[node + 1]);
      }
    }
    ...
    dt__forInnerIndex(sheet0.fixJ(layer), node)
    {
      ...
      if (dtLinearAlgebra::dotProduct(nn, checkDist) < 0.)
        nn = -1. * nn;
  ...
  progHelper::reverse(edgeVertex[2]);
  progHelper::reverse(edgeVertex[3]);
  twoDArrayHandling<::MVertex *> sheet1(edgeVertex[2].size(), nLayers[1] + 1);
  twoDArrayHandling<dtVector3> nn1(edgeVertex[2].size(), nLayers[1] + 1);
  twoDArrayHandling<dtPoint2> uv1(edgeVertex[2].size(), nLayers[1] + 1);
  ...
  dt__forFromToIndex(1, nLayers[1] + 1, layer)
  {
    ...
    NN[sheet1.fixJ(layer).size() - 1] = -1. * dtLinearAlgebra::normalize(dist1);
    ...
    if (dtLinearAlgebra::dotProduct(
          NN[sheet1.fixJ(layer).size() - 1], NN[sheet1.fixJ(layer).size() - 2]
        ) < 0.)
    {
  ...
  dt__forFromToIndex(nLayers[0] + 1, edgeVertex[1].size() - nLayers[1] - 1, ii)
  {
  ...
  progHelper::reverse(edgeVertex[3]);
  dt__forFromToIndex(nLayers[1] + 1, edgeVertex[3].size() - nLayers[0] - 1, ii)
  {
  ...
  {
    ge->addLine(new ::MLine(ge->getMeshVertex(ii - 1), ge->getMeshVertex(ii)));
  }
  ge->addLine(new ::MLine(
  ...
  if (optionHandling::debugTrue())
  {
    gm.writeMSH(
      dtgf->getMap2dTo3d()->getLabel() +
        "dtMeshGFaceWithTransfiniteLayer_0.msh",
      2.2,
      false,
      true
    );
  ...
  if (optionHandling::debugTrue())
  {
    gm.writeMSH(
      dtgf->getMap2dTo3d()->getLabel() +
        "dtMeshGFaceWithTransfiniteLayer_1.msh",
      2.2,
      false,
      true
    );
  ...
  dt__forFromToIndex(0, nLayers[0], layer)
  {
    ...
    {
      dtgf->addElement(new MQuadrangle(
        sheet0[node][layer],
        sheet0[node + 1][layer],
  ...
  if (optionHandling::debugTrue())
  {
    ...
    mb.write_mesh(std::string(dtgf->getPhysicalString() + "_debug.vtk").c_str()
    );
  }
  ...
  dt__forFromToIndex(0, gf->getNumMeshVertices(), ii)
  {
    ...
    dtgf->model()->setMaxVertexNumber(dtgf->model()->getMaxVertexNumber() + 1);
    gf->getMeshVertex(ii)->forceNum(dtgf->model()->getMaxVertexNumber());
  }
