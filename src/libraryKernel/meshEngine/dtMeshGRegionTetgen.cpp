...
namespace dtOO {
bool dtMeshGRegionTetgen::_registrated = dtMeshOperatorFactory::registrate(
...
)
{
  ...
  _tetOpt = dtXmlParserBase::replaceDependencies(
...
void dtMeshGRegionTetgen::operator()(dtGmshRegion *dtgr)
{
  ...
  // debug output
  //
  const_cast<dtGmshModel &>(dtgr->refDtGmshModel())
    .writeSTL(
      "transferToTetgen" +
...
)
{
  ...
  // faces
  //
  std::vector<dtGmshFace *> ff = dtGmshModel::cast2DtGmshFace(gr->faces());
  dt__forAllRefAuto(ff, fit)
  {
    // face vertices and elements
    enqueueMVertices(fit->mesh_vertices, nodeIndex, MVertexInt, intMVertex);
    enqueueMTriangles(fit->triangles, triangle);

    //
    // edges
    //
    std::vector<dtGmshEdge *> ee = dtGmshModel::cast2DtGmshEdge(fit->edges());
    dt__forAllRefAuto(ee, eit)
    {
      // edge vertices
      enqueueMVertices(eit->mesh_vertices, nodeIndex, MVertexInt, intMVertex);
      //
      // vertices
      //
      ...
      {
        // vertex vertices
        enqueueMVertices(vit->mesh_vertices, nodeIndex, MVertexInt, intMVertex);
      }
    }
  }
...
)
{
  ...
  dt__info(
    convertFromTetgen(),
    << logMe::dtFormat("%d points %d edges and %d faces in the initial mesh") %
...
void dtMeshGRegionTetgen::enqueueMTriangles(
  std::vector<::MTriangle *> &toEnqueue, std::vector<::MTriangle *> &triangle
)
{
