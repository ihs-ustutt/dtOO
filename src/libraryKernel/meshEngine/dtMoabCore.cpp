...
namespace dtOO {
dtMoabCore::dtMoabCore() : moab::Core() { query_interface(_readUtilIface); }
...
void dtMoabCore::addVertices(std::vector<::MVertex const *> const &mv)
{
  ...
  moab::Tag globalId;
  int zero = 0;
  moab::ErrorCode result = this->tag_get_handle(
    "GLOBAL_ID",
    1,
  ...
  moab__throwIf(
    (result != moab::MB_SUCCESS) && (result != moab::MB_ALREADY_ALLOCATED),
    addVertices()
  );
  ...
  for (long i = 0; i < num_nodes; ++i, ++handle)
  {
    ...
    dt__warnIfWithMessage(
      !_node_id_map.insert(std::pair<long, moab::EntityHandle>(id, handle))
         .second,
      addVertices(),
...
moab::Range dtMoabCore::addElements(std::vector<::MElement const *> const &me)
{
  ...
  dt__forAllIndex(me, ii)
  {
    ...
    else if (line)
    {
      ...
      dt__throw(addElements, << "Unknown element type.");
    ...
    dtInt prefStart = 1;
    result = _readUtilIface->get_element_connect(
    ...
    memcpy(
      conn_array,
      &connectivity[0],
      connectivity.size() * sizeof(moab::EntityHandle)
    );
    ...
    //
    result = _readUtilIface->update_adjacencies(
    ...
    dt__warnIfWithMessage(
      !_element_id_map
         .insert(std::pair<long, moab::EntityHandle>(ii + 1, handle))
         .second,
      addElements(),
...
void dtMoabCore::addVertexField(dtOMVertexField<bool> const &field)
{
  ...
  moab::Tag fieldTag;
  result = tag_get_handle(
    field.getLabel().c_str(),
    1,
    moab::MB_TYPE_INTEGER,
    fieldTag,
  ...
  result = tag_set_data(fieldTag, &(ent[0]), ent.size(), &(val[0]));
  dt__debug(addVertexField(), << "result = " << moab::ErrorCodeStr[result]);
  moab__throwIf(result != moab::MB_SUCCESS, addVertexField());
}
...
void dtMoabCore::addFaceField(dtOMFaceField<dtInt> const &field)
{
  ...
  int ii = 0;
  dt__forFromToIter(
    omConstFaceI, field.refMesh().faces_begin(), field.refMesh().faces_end(), it
  )
  {
    //			::MVertex const * mv = field.refMesh().at(*vIt);
