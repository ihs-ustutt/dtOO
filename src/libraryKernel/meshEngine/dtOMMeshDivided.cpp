...
void dtOMMeshDivided::add(dtOMMesh const &om) { _part.push_back(om); }
...
dtOMMesh dtOMMeshDivided::connect(void)
{
  ...
  while (_part.size() > 0)
  {
    ...
    //
    omEdgeH connectEdge;
    dt__forFromToIter(omEdgeI, result.edges_begin(), result.edges_end(), eIt)
    {
    ...
    dt__forAllIter(std::list<dtOMMesh>, _part, aPartIt)
    {
      ...
      if (aPart.contains(mv0) && aPart.contains(mv1))
      {
        ...
        dt__forFromToIter(
          ...
          aPart.cvih_begin(omVH0_part),
          aPart.cvih_end(omVH0_part),
          vihIt
        )
        {
        ...
        if (!connectHalfedge_part.is_valid())
        {
          dt__forFromToIter(
            ...
            aPart.cvoh_begin(omVH0_part),
            aPart.cvoh_end(omVH0_part),
            vohIt
          )
          {
        ...
        //
        if (aPart.at(aPart.from_vertex_handle(connectHalfedge_part)) ==
        ...
        {
          dt__info(
            connect(), << "Add part ( " << _part.size() - 1 << " left ) "
          );
          result.addInv(aPart);
        }
