  ...
  : _nTri(0), _nQuad(0)
{
  ...
  {
    addFace(om.data(*it));
    if (om.data(*it).nVertices() == 3)
      _nTri++;
    else if (om.data(*it).nVertices() == 4)
      _nQuad++;
    else
    {
...
dtOMMeshManifold::dtOMMeshManifold(dtOMMeshManifold const &orig)
  : dtOMMesh(orig)
{
...
bool dtOMMeshManifold::divideable(void) const
{
  ...
  if (((_dihedralAngleV.size() >= 3) && isClosed) ||
...
{
  if (std::find(_isBoundary.begin(), _isBoundary.end(), true) ==
      _isBoundary.end())
  {
    return true;
  }
...
dtOMMeshManifold dtOMMeshManifold::divide(void)
{
  ...
  if (isClosed)
  {
    ...
    for (int ii = 0; ii < (maxAngleIt - _dihedralAngleV.begin()); ii++)
      maxEdgeIt++;
    *maxAngleIt = 0.;
    ...
    dt__forAllIndex(_dihedralAngleV, tmp)
    {
      ...
      for (dtInt ii = 0; ii < (max2ndAngleIt - _dihedralAngleV.begin()); ii++)
        max2ndEdgeIt++;
      *max2ndAngleIt = 0.;
      //				if (
    ...
    //			dt__info(
    //				divide(),
    //				<< "Dividing manifold at " << maxAngleIt -
    //_dihedralAngleV.begin()
    //				<< " and " << max2ndAngleIt -
  ...
  else
  {
    ...
    dt__forAllIndex(_dihedralAngleV, tmp)
    {
      ...
      for (int ii = 0; ii < maxAngleBegPos; ii++)
        maxEdgeIt++;
      *maxAngleIt = 0.;
      //				if ( (maxAngleBegPos>0) &&
      //(maxAngleEndPos<-0) ) break;
    }
 ...
 */
std::vector<dtOMMeshManifold> dtOMMeshManifold::divide(dtReal const &angle)
{
  //    if ( )
