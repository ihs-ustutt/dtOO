...
namespace dtOO {
template <typename T> class dtOMVertexField : public dtOMField {
public:
...
template <typename T> dtOMVertexField<T>::~dtOMVertexField() {}

template <typename T> T &dtOMVertexField<T>::operator[](omVertexH const &vH)
{
  return _field[vH];
}

template <typename T> T const &dtOMVertexField<T>::at(omVertexH const &vH) const
{
  return _field.at(vH);
}
...
template <typename T> dtLongUnsInt dtOMVertexField<T>::size(void) const
{
  // return _om.n_vertices();
  return _field.size();
}

template <typename T> void dtOMVertexField<T>::update(void)
{
  dt__forFromToIter(
    omConstVertexI, refMesh().vertices_begin(), refMesh().vertices_end(), vIt
  )
