...
dtAffTransformation3
dtLinearAlgebra::getRotation(dtVector3 const vector, const dtReal angle)
{
  dtReal const one = 1.;
  dtReal const zero = 0.;
  dtMatrix EE(3, 3);
...
dtAffTransformation3 dtLinearAlgebra::getTranslation(dtVector3 const vector)
{
  return dtAffTransformation3(dtAffTranslation(), vector);
}

dtVector3
dtLinearAlgebra::crossProduct(dtVector3 const &v0, dtVector3 const &v1)
{
  return CGAL::cross_product(v0, v1);
}

dtReal dtLinearAlgebra::dotProduct(dtVector3 const &v0, dtVector3 const &v1)
{
  return v0 * v1;
}

dtVector3 dtLinearAlgebra::normalize(dtVector3 const &v0)
{
  dtReal sqL = v0.squared_length();
  if (sqL == 0.)
    return dtVector3(0., 0., 0.);
  return (1. / sqrt(sqL)) * v0;
}

dtVector2 dtLinearAlgebra::normalize(dtVector2 const &v0)
{
  dtReal sqL = v0.squared_length();
  if (sqL == 0.)
    return dtVector2(0., 0.);
  return (1. / sqrt(sqL)) * v0;
}
...
{
  dtVector3 nn(0, 0, 0);
  dt__forAllConstIter(std::vector<dtVector3>, vv, it) { nn = nn + (*it); }
  return nn;
}
...
{
  dtReal nn = 0.;
  dt__forAllConstIter(std::vector<dtReal>, vv, it) { nn = nn + (*it); }
  return nn;
}
...
{
  dtReal nn = 0.;
  dt__forFromToIndex(from, to, ii) { nn = nn + vv[ii]; }
  return nn;
}
...
{
  dtReal nn = 0.;
  dt__forAllRefAuto(vv, it) { nn = nn + it * it; }
  return nn;
}
...
std::vector<dtReal>
dtLinearAlgebra::multiply(dtReal const &aa, std::vector<dtReal> const &bb)
{
...
std::vector<dtPoint3>
dtLinearAlgebra::toDtPoint3(std::vector<dtVector3> const &vv)
{
...
dtLinearAlgebra::toDtVector3(std::vector<dtPoint3> const &pp)
{
...
)
{
  ...
  dt__forAllIndex(pp, ii) { pp[ii] = dtPoint3(xx[ii], yy[ii], zz[ii]); }
...
{
  return std::vector<dtReal>{
    static_cast<dtReal>(pp.x()), static_cast<dtReal>(pp.y())
  };
}
...
dtMatrix
dtLinearAlgebra::createMatrixGiveColumns(std::vector<dtVector3> const &cols)
{
...
dtMatrix
dtLinearAlgebra::createMatrixGiveRows(std::vector<dtVector3> const &rows)
{
...
gsl_matrix *convertToGSLMatrix(dtMatrix const &mat)
{
  ...
  dt__forFromToIndex(0, R, r) dt__forFromToIndex(0, C, c)
...
dtMatrix dtLinearAlgebra::invertMatrix(dtMatrix const &mat)
{
  ...
  dt__throwIfWithMessage(
    status, invertMatrix(), << std::string(gsl_strerror(status))
  );
...
dtMatrixVector
dtLinearAlgebra::solveMatrix(dtMatrix const &mat, dtMatrixVector const &rhs)
{
  // get dimensions
  dtInt const M = mat.row_dimension();
  dtInt const N = mat.column_dimension();
  ...
  dt__throwIfWithMessage(
    status, solveMatrix(), << std::string(gsl_strerror(status))
  );
...
dtPoint3
dtLinearAlgebra::returnFarthestPointTo(dtPoint3 pp, dtPoint3 p0, dtPoint3 p1)
{
...
std::pair<dtPoint2, dtPoint2>
dtLinearAlgebra::boundingBox(std::vector<dtPoint2> const &pp)
{
...
std::pair<dtPoint3, dtPoint3>
dtLinearAlgebra::boundingBox(std::vector<dtPoint3> const &pp)
{
...
)
{
  ...
  if (fabs(diff.z()) < eps)
    isSmall[2] = true;
  if ((isSmall[0] && isSmall[1]) || (isSmall[0] && isSmall[2]) ||
      (isSmall[1] && isSmall[2]))
  {
    dt__info(
      isStraightLine(),
      << dtLog::dtFormat("Bounding box: %f, %f, %f") % diff.x() % diff.y() %
             diff.z()
      << std::endl
...
dtReal dtLinearAlgebra::angleDegree(dtVector3 const &v0, dtVector3 const &v1)
{
...
dtPoint2
dtLinearAlgebra::intersectionPoint(dtLine2 const &line0, dtLine2 const &line1)
{
  dtPoint2 iPoint;
  ...
  if (CGAL::assign(iPoint, res))
  {
    ...
    dt__throwUnexpected(intersectionPoint());
}
...
)
{
  ...
  if (dis == 0.)
    return std::vector<dtReal>(1, -bb / (2.0 * aa));
...
{
  if (dtTetrahedron3(p000, p001, p011, p101).bounded_side(pt) !=
      CGAL::ON_UNBOUNDED_SIDE)
    return true;
  if (dtTetrahedron3(p011, p101, p111, p110).bounded_side(pt) !=
      CGAL::ON_UNBOUNDED_SIDE)
    return true;
  if (dtTetrahedron3(p000, p100, p110, p101).bounded_side(pt) !=
      CGAL::ON_UNBOUNDED_SIDE)
    return true;
  if (dtTetrahedron3(p000, p010, p110, p011).bounded_side(pt) !=
      CGAL::ON_UNBOUNDED_SIDE)
    return true;
  if (dtTetrahedron3(p011, p101, p000, p110).bounded_side(pt) !=
      CGAL::ON_UNBOUNDED_SIDE)
    return true;
...
)
{
  ...
  {
    logC() << dtLog::dtFormat("Point %3d : %5.2f %5.2f") % ii % points[ii][0] %
                points[ii][1]
           << std::endl;
  }
  logC() << dtLog::dtFormat("ChkPoint: %5.2f %5.2f") % pt[0] % pt[1]
         << std::endl;
...
{
  return sqrt(dtTriangle3(p0, p1, p2).squared_area()) +
...
{
  return fabs(dtTetrahedron3(p000, p001, p011, p101).volume()) +
...
dtLinearAlgebra::unitGrid(dtInt const &nU, dtInt const &nV)
{
