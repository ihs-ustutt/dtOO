...
bool gslGradMinFloatAttr::perform()
{
  ...
  dt__forAllIndex(guess(), jj)
  {
    ...
    {
      // output
      logC() << dtLog::dtFormat("%3d : [ ") % jj;
      dt__forAllIndex(guess()[jj], kk)
      {
        logC() << dtLog::dtFormat("%5.2d ") % guess()[jj][kk];
      }
      logC() << "]";
      ...
      do
      {
        ...
        {
          logC() << dtLog::dtFormat("%+5.2e ") % gsl_vector_get(x, hh);
        }
        gsl_vector *grad = gsl_multimin_fdfminimizer_gradient(minf);
        logC() << " / grad = ";
        dt__forFromToIndex(0, dimension(), hh)
        {
          logC() << dtLog::dtFormat("%+5.2e ") % gsl_vector_get(grad, hh);
        }
        logC() << dtLog::dtFormat(" -> f = %+5.2e ( dx = ") % minf->f;
        gsl_vector *dx = gsl_multimin_fdfminimizer_dx(minf);
        dt__forFromToIndex(0, dimension(), hh)
        {
          logC() << dtLog::dtFormat("%+5.2e ") % gsl_vector_get(dx, hh);
        }
        logC() << ")";
      ...
      if (minf->f < gF)
      {
        ...
        result(tRes);
        logC() << dtLog::dtFormat("-> Global minimum update -> %+5.2d\n") % gF;
      }
  ...
  logC() << dtLog::dtFormat("=> min f( ");
  dt__forAllIndex(result(), kk)
  {
    logC() << dtLog::dtFormat("%+5.2d ") % result()[kk];
  }
  logC() << dtLog::dtFormat(") = %+5.2d") % gF;
  if (!converged())
    logC() << " -> F";
  logC() << std::endl;
