...
)
{
  ...
  dt__forAllConstIter(dt__pVH(dtCurve2d), dtC2d, it)
  {
    ...
    pp.push_back(std::pair<dtVector2, dtVector2>(
      dtLinearAlgebra::toDtVector2(theCurve.controlPoint(0)),
  ...
  dt__forAllConstIter(dt__pVH(dtCurve2d), dtC2d, it)
  {
    ...
    theClone->setControlPoint(0, dtLinearAlgebra::toDtPoint2(pp[ii].first));
    theClone->setControlPoint(
      theClone->nControlPoints() - 1, dtLinearAlgebra::toDtPoint2(pp[ii].second)
    );
