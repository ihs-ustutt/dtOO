#include "geomShape_readIgesOCC.h"
...
namespace dtOO {
	geomShape_readIgesOCC::geomShape_readIgesOCC() {

	}

	geomShape_readIgesOCC::geomShape_readIgesOCC( std::string const & filename ) {
    ...
    dt__info( geomShape_readIgesOCC(), << nGeos << " successful translations." );
    ...
    dt__forFromToIndex(1, reader.NbShapes()+1, ii) {
      ...
      for( 
        ...
        if ( !aSurf.IsNull() ) {
          ...
          else dt__throwUnexpected(geomShape_readIgesOCC());
        }
        else dt__throwUnexpected(geomShape_readIgesOCC());
      }       
      for( 
  ...
  geomShape_readIgesOCC::resultCurveVectorHandling( void ) const {
  ...
  geomShape_readIgesOCC::resultSurfaceVectorHandling( void ) const {
  ...
  geomShape_readIgesOCC::resultCurveRef( void ) const {
    return _dtC;
  }
  
  ptrVectorHandling< dtSurface > const & 
  geomShape_readIgesOCC::resultSurfaceRef( void ) const {
    return _dtS;
  }
}
