  ...
  )
{
  ...
  else
  {
    ...
    //							 /
    //							 ( sin(angleOut) -
    //(cos(angleOut)*sin(angleIn) / cos(angleIn)) );
    ...
    pV.push_back(
      dtPoint2(cos(angleIn) * tt * pointOne, sin(angleIn) * tt * pointOne)
    );
    pV.push_back(dtPoint2(cos(angleIn) * tt, sin(angleIn) * tt));
    pV.push_back(dtPoint2(
...
dtCurve2d *
bSplineCurve2d_angleDeltaXDeltaYPointOnePointTwoConstructOCC ::result(void)
{
  return _dtC2d->clone();
}
} // namespace dtOO
