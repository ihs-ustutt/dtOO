...
geomShape_readOCC::geomShape_readOCC(std::string const &filename)
{
  ...
  if (stringPrimitive::stringContains(".stp", filename) ||
  ...
  dt__forFromToIndex(1, reader->NbShapes() + 1, ii)
  {
    ...
    for (explorer.Init(reader->Shape(ii), ::TopAbs_FACE); explorer.More();
         explorer.Next())
    {
      ...
      if (!aSurf.IsNull())
      {
        ...
        _dtS.push_back(geomSurface_baseConstructOCC(base).result());
      }
      else
        dt__throwUnexpected(geomShape_readOCC());
    }
    for (explorer.Init(reader->Shape(ii), ::TopAbs_EDGE); explorer.More();
         explorer.Next())
    {
      ...
      if (!aCurve.IsNull())
      {
        ...
        _dtC.push_back(geomCurve_baseConstructOCC(base).result());
      }
      else
        dt__throwUnexpected(geomShape_readOCC());
    }
...
vectorHandling<dtCurve *> geomShape_readOCC::resultCurveVectorHandling(void
) const
{
...
vectorHandling<dtSurface *> geomShape_readOCC::resultSurfaceVectorHandling(void
) const
{
...
ptrVectorHandling<dtCurve> const &geomShape_readOCC::resultCurveRef(void) const
{
  return _dtC;
}

ptrVectorHandling<dtSurface> const &geomShape_readOCC::resultSurfaceRef(void
) const
{
  return _dtS;
}
