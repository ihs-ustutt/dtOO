...
dtInt dtSurface::continuity(void) const { return -1; }
...
dtPoint2 dtSurface::reparam(dtPoint3 const ppXYZ) const
{
  ...
  logC() << dtLog::dtFormat("U = %5.2e, V = %5.2e / dist = %5.2e\n") %
              ppUV.x() % ppUV.y() % dist;
  if (dtSurface::inXYZTolerance(dist))
  {
    logC() << dtLog::dtFormat("In Tolerance: U = %5.2e V = %5.2e") % ppUV.x() %
                ppUV.y();
  }
  else
  {
...
dtReal dtSurface::maxU(void) const { return maxPara(0); }

dtReal dtSurface::minV(void) const { return minPara(1); }

dtReal dtSurface::maxV(void) const { return maxPara(1); }

bool dtSurface::closedU(void) const { return closed(0); }
...
std::vector<dtPoint3> dtSurface::getPoint3d(
  std::vector<dtReal> const &uu, std::vector<dtReal> const &vv
) const
{
  std::vector<dtPoint3> pp(uu.size());
  dt__forAllIndex(uu, ii) { pp[ii] = point(uu[ii], vv[ii]); }

  return pp;
}

std::vector<dtPoint3> dtSurface::getPointPercent3d(
  std::vector<dtReal> const &uu, std::vector<dtReal> const &vv
) const
{
  std::vector<dtPoint3> pp(uu.size());
...
int dtSurface::nControlPointsU(void) const { return nControlPoints(0); }

int dtSurface::nControlPointsV(void) const { return nControlPoints(1); }
...
{
  return segmentConstU(u_uPercent(uu), v_vPercent(vvMin), v_vPercent(vvMax));
}
...
{
  return segmentConstV(v_vPercent(vv), u_uPercent(uuMin), u_uPercent(uuMax));
}
...
{
  if (dir == 0)
    return segmentConstU(at, from, to);
  else if (dir == 1)
    return segmentConstV(at, from, to);
  else
    dt__throwUnexpected(segmentConst());
}

dtCurve *dtSurface::segmentConst(dtInt const &dir, dtReal const at) const
{
  if (dir == 0)
    return segmentConstU(at);
  else if (dir == 1)
    return segmentConstV(at);
  else
    dt__throwUnexpected(segmentConst());
}

dtCurve *dtSurface::segmentConstPercent(dtInt const &dir, dtReal const at) const
{
  if (dir == 0)
    return segmentConstUPercent(at);
  else if (dir == 1)
    return segmentConstVPercent(at);
  else
...
{
  if (dir == 0)
    return segmentConstUPercent(at, from, to);
  else if (dir == 1)
    return segmentConstVPercent(at, from, to);
  else
    dt__throwUnexpected(segmentConstPercent());
}
