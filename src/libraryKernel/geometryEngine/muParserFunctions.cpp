...
{
  if (!_pH)
    _pH.reset(new muParserFunctions());
  return _pH.get();
}
...
::mu::value_type muParserFunctions::YFix(::mu::char_type const *expr)
{
  ...
  return dtXmlParser::muParseString(dtXmlParser::replaceDependencies(
    str, NULL, muParserFunctions::instance()->_aFPtrVec, NULL
  ));
}

::mu::value_type
muParserFunctions::Y(::mu::char_type const *expr, ::mu::value_type val)
{
  std::string str(expr);
  str = dtXmlParser::replaceStringInString(
    "()", "(" + dtXmlParser::floatToString(val) + ")", str
  );
