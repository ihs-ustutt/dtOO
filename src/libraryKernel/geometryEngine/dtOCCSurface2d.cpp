...
{
  dt__tryOcc(_surface.reset(new dtOCCSurfaceBase());
             _surface->setOCC(Handle(Geom_Surface
             )::DownCast(orig.getOCC()->Copy()));
...
dtReal dtOCCSurface2d::minPara(dtInt const dim) const
{
  ...
  Standard_Real V2;
  dt__tryOcc(
    _ptr->Bounds(U1, U2, V1, V2); switch (dim) {
      case 0:
...
dtOCCSurface2d::firstDer(dtReal const uu, dtReal const vv) const
{
  Standard_Real uR = static_cast<Standard_Real>(uu);
  Standard_Real vR = static_cast<Standard_Real>(vv);
  gp_Pnt pp;
  ...
  dt__tryOcc(_ptr->D1(uR, vR, pp, D1U, D1V);,
                                            << dt__eval(uu) << std::endl
                                            << dt__eval(vv) << std::endl
                                            << dt__eval(uR) << std::endl
                                            << dt__eval(vR));
...
std::vector<dtVector2>
dtOCCSurface2d::secondDer(dtReal const uu, dtReal const vv) const
{
  Standard_Real uR = static_cast<Standard_Real>(uu);
  Standard_Real vR = static_cast<Standard_Real>(vv);
  gp_Pnt pp;
  ...
  dt__tryOcc(
    _ptr->D2(uR, vR, pp, D1U, D1V, D2U, D2V, D2UV);,
                                                   << dt__eval(uu) << std::endl
                                                   << dt__eval(vv) << std::endl
                                                   << dt__eval(uR) << std::endl
                                                   << dt__eval(vR)
  ...
  retVec[2] = dtLinearAlgebra::ignoreZ(dtVector3(
...
std::string dtOCCSurface2d::dumpToString(void) const
{
  ...
  ss << dt__dumpToString(<< dt__eval(_surface->dumpToString()));
...
dtOCCSurfaceBase &dtOCCSurface2d::OCCRef(void) { return *(_surface.get()); }
