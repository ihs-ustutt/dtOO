...
namespace dtOO {
dtOCCCompositeSurface::dtOCCCompositeSurface() : dtOCCSurface() { _ptr = NULL; }

dtOCCCompositeSurface::dtOCCCompositeSurface(const dtOCCSurfaceBase &orig)
  : dtOCCSurface(orig)
{
  dt__mustCast(
    OCCRef().getOCC().get(), ShapeExtend_CompositeSurface const, _ptr
  );
}
...
) const
{
  ...
  if (_ptr->Patch(uR, v0R).get() == _ptr->Patch(uR, v1R).get())
  {
    ...
    dt__tryOcc(cc = _ptr->Patch(uR, v0R)->UIso(_ptr->UGlobalToLocal(
...
) const
{
  ...
  {
    Handle(Geom_Curve) cc;
    dt__tryOcc(cc = _ptr->Patch(u0R, vR)->VIso(_ptr->VGlobalToLocal(
