...
{
  dt__throwIf(
    (angle == 2. * M_PI) && !dtOCCSurfaceOfRevolution::Is(&dtS),
    analyticRotatingMap1dTo3d()
  );
  dt__throwIf(
    (angle != 2. * M_PI) && !dtOCCRectangularTrimmedSurface::Is(&dtS),
    analyticRotatingMap1dTo3d()
  );
  _vv = dtVector3(vv);
  _pp = dtPoint3(pp);
...
analyticRotatingMap1dTo3d::analyticRotatingMap1dTo3d(
  dtSurface const &dtS, dtVector3 const &vv, dtReal const &angle
)
  : analyticSurface(&dtS)
{
  dt__throwIf(
...
dtPoint3 analyticRotatingMap1dTo3d::getOrigin(void) const { return _pp; }

dtVector3 analyticRotatingMap1dTo3d::getRotVector(void) const { return _vv; }

dtReal analyticRotatingMap1dTo3d::getAngle(void) const { return _angle; }

dtReal analyticRotatingMap1dTo3d::u_phi(dtReal const &arg) const
{
  return map2dTo3d::getUMin() +
         arg * (map2dTo3d::getUMax() - map2dTo3d::getUMin()) / (2. * M_PI);
}
...
dtReal analyticRotatingMap1dTo3d::phi_u(dtReal const &arg) const
{
  if (arg >= 0.)
  {
    return 2. * M_PI * (arg - map2dTo3d::getUMin()) /
           (map2dTo3d::getUMax() - map2dTo3d::getUMin());
  }
  else
  {
    dtReal uu = 2. * M_PI * (fabs(arg) - map2dTo3d::getUMin()) /
                (map2dTo3d::getUMax() - map2dTo3d::getUMin());
...
dtPoint2
analyticRotatingMap1dTo3d::uv_phiZ(dtReal const &arg0, dtReal const &arg1) const
{
...
dtPoint2
analyticRotatingMap1dTo3d::uv_phiM(dtReal const &arg0, dtReal const &arg1) const
{
  return dtPoint2(u_phi(arg0), v_m(arg1));
}

dtReal analyticRotatingMap1dTo3d::zMin(void) const { return z_v(getMin(1)); }

dtReal analyticRotatingMap1dTo3d::zMax(void) const { return z_v(getMax(1)); }
...
dtCurve const *analyticRotatingMap1dTo3d::ptrConstRadiusCurve(void) const
{
  ...
  dt__ptrAss(analyticCurve const *sC3, analyticCurve::ConstDownCast(m1d));
