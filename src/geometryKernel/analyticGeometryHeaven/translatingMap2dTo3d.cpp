...
translatingMap2dTo3d::translatingMap2dTo3d(const translatingMap2dTo3d &orig)
  : map3dTo3d(orig)
{
...
translatingMap2dTo3d *
translatingMap2dTo3d::cloneTransformed(dtTransformer const *const dtT) const
{
...
map2dTo3d *translatingMap2dTo3d::segmentConstU(dtReal const &uu) const
{
  ...
  {
    dt__pH(dtSurface)
      dtS(geomSurface_surfaceTranslateConstructOCC(aS->ptrDtSurface(), _vv * uu)
            .result());
...
map2dTo3d *translatingMap2dTo3d::segmentConstV(dtReal const &vv) const
{
  ...
  if (aS)
  {
    ...
    cV[1] = geomCurve_curveTranslateConstructOCC(s3d->ptrConstDtCurve(), _vv)
              .result();
    dt__pH(dtSurface)
      dtS(bSplineSurface_skinConstructOCC(cV, 1, 1, 0).result());
    cV.destroy();
