...
analyticCurve *analyticCurve::cloneTransformed(dtTransformer const *const dtT
) const
{
  return new map1dTo3dTransformed<analyticCurve>(*this, dtT);
}

analyticCurve *analyticCurve::create(void) const { return new analyticCurve(); }
...
map1dTo3d *analyticCurve::segment(dtReal const &u0, dtReal const &u1) const
{
  return new analyticCurve(
    dt__pH(dtCurve)(trimmedCurve_uBounds(_dtC.get(), u0, u1).result()).get()
  );
  //    scaLinearOneD fun(0, 1, u0, u1);
  //    return new scaOneDInMap1dTo3d(&fun, this);
}

vectorHandling<renderInterface *> analyticCurve::getExtRender(void) const
{
...
dtCurve *analyticCurve::ptrDtCurve(void) const { return _dtC.get(); }
...
dtReal analyticCurve::l_u(dtReal const &uu) const { return _dtC->l_u(uu); }

dtReal analyticCurve::u_l(dtReal const &ll) const { return _dtC->u_l(ll); }

bool analyticCurve::isClosed(dtInt const &dir) const { return _dtC->closed(); }
