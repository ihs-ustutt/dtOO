...
analyticGeometryGroup::analyticGeometryGroup(analyticGeometryGroup const &orig)
  : analyticGeometry(orig)
{
  dt__forAllRefAuto(orig._vec, aG) { _vec.push_back(aG.clone()); }
}
...
analyticGeometryGroup::analyticGeometryGroup(
  ...
  : analyticGeometry()
{
  dt__forAllRefAuto(vec, aG) { _vec.push_back(aG->clone()); }
}
...
::std::vector<dtVector3> analyticGeometryGroup::firstDer(dtReal const *const uvw
) const
{
...
dtReal analyticGeometryGroup::getMax(dtInt const &dir) const { return 0.; }

dtInt analyticGeometryGroup::dim(void) const { dt__throwUnexpected(dim()); }

vectorHandling<renderInterface *> analyticGeometryGroup ::getRender(void) const
{
...
vectorHandling<renderInterface *> analyticGeometryGroup ::getExtRender(void
) const
{
