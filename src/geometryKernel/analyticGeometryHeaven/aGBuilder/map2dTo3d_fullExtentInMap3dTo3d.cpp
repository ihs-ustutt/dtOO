...
)
{
  ...
  if ((m2dParDir == 0) && (toM3dParDir == 1))
  {
    ...
    //
    dtC.push_back(bSplineCurve_pointConstructOCC(
                    dtPoint3(u0, m3d->getVMin(), m3d->getWMin()), from[0]
    )
                    .result());
    dtC.push_back(dtS->segmentConstUPercent(0.));
    dtC.push_back(bSplineCurve_pointConstructOCC(
                    from[1], dtPoint3(u1, m3d->getVMin(), m3d->getWMax())
    )
                    .result());
    dtC.push_back(bSplineCurve_pointConstructOCC(
    ...
    dY0 = v3d->DYdtVector3((*v3d) % analyticFunction::aFXTwoD(1., 0.));
    dY1 = v3d->DYdtVector3((*v3d) % analyticFunction::aFXTwoD(1., 1.));
    ...
    //
    dtC.push_back(bSplineCurve_pointConstructOCC(
                    from[0], dtPoint3(u0, m3d->getVMax(), m3d->getWMin())
    )
                    .result());
    dtC.push_back(bSplineCurve_pointConstructOCC(
                    ...
                    .result());
    dtC.push_back(bSplineCurve_pointConstructOCC(
                    dtPoint3(u1, m3d->getVMax(), m3d->getWMax()), from[1]
    )
                    .result());
    ...
    //
    _m2d.reset(new vec3dTwoDInMap3dTo3d(
  ...
  {
    dt__throw(
      map2dTo3d_fullExtentInMap3dTo3d(), << "Case not yet implemented."
    );
  }
}
