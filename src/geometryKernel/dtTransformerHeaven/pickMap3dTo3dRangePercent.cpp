...
namespace dtOO {
bool pickMap3dTo3dRangePercent::_registrated = dtTransformerFactory::registrate(
...
pickMap3dTo3dRangePercent::pickMap3dTo3dRangePercent(jsonPrimitive const &jE)
  : dtTransformer(jE)
{
...
pickMap3dTo3dRangePercent::apply(lvH_analyticGeometry const *const aGeoVecP
) const
{
...
bool pickMap3dTo3dRangePercent::isNecessary(void) const { return true; }
...
)
{
  ...
  if (dtXmlParserBase::hasAttribute("parameter_percent_three", *tE))
  {
    config.append(
      "_w0",
      dtXmlParserBase::muParseString(dtXmlParserBase::replaceDependencies(
        dtXmlParserBase::getAttributeStr("parameter_percent_three", *tE), cV, aF
      ))
    );
  }
