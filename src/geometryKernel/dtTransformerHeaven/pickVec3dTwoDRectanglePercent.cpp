...
bool pickVec3dTwoDRectanglePercent::_registrated =
  dtTransformerFactory::registrate(dt__tmpPtr(
    pickVec3dTwoDRectanglePercent, new pickVec3dTwoDRectanglePercent()
  ));

pickVec3dTwoDRectanglePercent::pickVec3dTwoDRectanglePercent() : dtTransformer()
{
}
...
pickVec3dTwoDRectanglePercent::apply(lvH_analyticFunction const *const aFVecP
) const
{
  ...
  dt__forAllRefAuto(*aFVecP, theF)
  {
    dt__throwIfWithMessage(
      theF->isTransformed(), apply(), << theF->dumpToString()
    );
    dtSurface const *const oldSurf =
      vec3dSurfaceTwoD::MustConstDownCast(theF)->ptrDtSurface();
    dt__pH(dtSurface)
      newSurf(rectangularTrimmedSurface_uvBounds(
                oldSurf,
...
bool pickVec3dTwoDRectanglePercent::isNecessary(void) const { return true; }
