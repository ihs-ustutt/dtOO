...
namespace dtOO {
bool pickMap1dTo3dLinePercent::_registrated = dtTransformerFactory::registrate(
...
pickMap1dTo3dLinePercent ::pickMap1dTo3dLinePercent(
  const pickMap1dTo3dLinePercent &orig
)
  : dtTransformer(orig)
{
}

pickMap1dTo3dLinePercent::pickMap1dTo3dLinePercent(jsonPrimitive const &jE)
  : dtTransformer(jE)
{
...
pickMap1dTo3dLinePercent::apply(lvH_analyticGeometry const *const aGeoVecP
) const
{
  ...
  dt__forAllRefAuto(*aGeoVecP, aG)
  {
    ...
    aGeoRet.push_back(m1d->segmentPercent(
      config().lookup<dtReal>("_u0"), config().lookup<dtReal>("_u1")
    ));
  }
...
bool pickMap1dTo3dLinePercent::isNecessary(void) const { return true; }
...
)
{
  ...
  std::vector<::QDomElement> el = dtXmlParserBase::getChildVector("float", *tE);
