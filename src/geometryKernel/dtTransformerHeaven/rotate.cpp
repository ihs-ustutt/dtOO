...
bool rotate::_registrated =
  dtTransformerFactory::registrate(dt__tmpPtr(rotate, new rotate()));
...
rotate::rotate(jsonPrimitive const &jE) : dtTransformerInvThreeD(jE)
{
  this->jInit(jE, NULL, NULL, NULL, NULL);
}

dtTransformerInvThreeD *rotate::clone(void) const { return new rotate(*this); }

dtTransformerInvThreeD *rotate::create(void) const { return new rotate(); }

std::vector<dtPoint3> rotate::apply(std::vector<dtPoint3> const *const toTrans
) const
{
  ...
  dtAffTransformation3 rot = dtLinearAlgebra::getRotation(rotVector, angle);
...
std::vector<dtPoint3> rotate::retract(std::vector<dtPoint3> const *const toTrans
) const
{
  dtReal angle = config().lookup<dtReal>("_angle");
  dtVector3 rotVector = config().lookup<dtVector3>("_rotVector");
...
std::vector<dtVector3> rotate::apply(std::vector<dtVector3> const *const toTrans
) const
{
  dtReal angle = config().lookup<dtReal>("_angle");
  dtVector3 rotVector = config().lookup<dtVector3>("_rotVector");
...
rotate::retract(std::vector<dtVector3> const *const toTrans) const
{
  dtReal angle = config().lookup<dtReal>("_angle");
  dtVector3 rotVector = config().lookup<dtVector3>("_rotVector");
  dtAffTransformation3 rot =
...
lvH_analyticGeometry rotate::apply(lvH_analyticGeometry const *const aGeoVecP
) const
{
...
bool rotate::isNecessary(void) const { return true; }
