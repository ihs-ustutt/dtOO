...
namespace dtOO {
bool thicknessIncreasing::_registrated = dtTransformerFactory::registrate(
...
thicknessIncreasing::thicknessIncreasing(const thicknessIncreasing &orig)
  : dtTransformer(orig)
{
  _thicknessDistributionP.reset(
    scaOneD::SecureCast(orig._thicknessDistributionP->clone())
  );
  _paraOnePercentFunP.reset(scaOneD::SecureCast(orig._paraOnePercentFunP->clone(
  )));
...
thicknessIncreasing::apply(lvH_analyticFunction const *const sFunP) const
{
...
bool thicknessIncreasing::isNecessary(void) const { return true; }
