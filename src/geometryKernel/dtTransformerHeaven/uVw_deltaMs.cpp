...
bool uVw_deltaMs::_registrated =
  dtTransformerFactory::registrate(dt__tmpPtr(uVw_deltaMs, new uVw_deltaMs()));
...
uVw_deltaMs::uVw_deltaMs(jsonPrimitive const &jE) : dtTransformer(jE)
{
...
dtTransformer *uVw_deltaMs::clone(void) const { return new uVw_deltaMs(*this); }

dtTransformer *uVw_deltaMs::create(void) const { return new uVw_deltaMs(); }

lvH_analyticFunction uVw_deltaMs::apply(lvH_analyticFunction const *const aFP
) const
{
...
std::vector<dtPoint3>
uVw_deltaMs::apply(std::vector<dtPoint3> const *const toTrans) const
{
...
std::vector<dtPoint3>
uVw_deltaMs::retract(std::vector<dtPoint3> const *const toRetract) const
{
  std::vector<dtPoint3> retVec;
  dt__forAllIndex(*toRetract, ii)
  {
    ...
    aFY ms = _ms_uSPercentVSPercent->Y(
      analyticFunction::aFXTwoD(_msCut->percent_u(vv), _msCut->percent_v(ww))
    );
    dtReal mm = ms[0];
    dtReal ss = ms[1];
    ...
    dt__debug(
      ...
      << std::endl
      << dtFormat("to point (delta, mm, ss) = (%E, %E, %E)") % delta % mm % ss
    );
...
bool uVw_deltaMs::isNecessary(void) const { return true; }
...
)
{
  ...
  _tM2d = translatingMap2dTo3d::PointerDownCast(
  ...
  dt__forAllIndex(ms, ii)
  {
    dt__forAllIndex(ms[ii], jj)
    {
      ...
      ms[ii][jj] = dtPoint2(
        m_uSVS(_msCut->u_percent(uPercent), _msCut->v_percent(vPercent)),
        s_uSVS(_msCut->u_percent(uPercent), _msCut->v_percent(vPercent))
      );
    }
  }
...
)
{
  ...
  config.append<analyticGeometry const *>(
    "_tM2d", aG->get(dtXmlParserBase::getAttributeStr("part_label", *tE))
  );
...
{
  ptrHandling<map1dTo3d> m1d(_msCut->segmentConstV(vv));
  return m1d->l_u(uu);
}

dtReal uVw_deltaMs::s_uSVS(dtReal const &uu, dtReal const &vv) const
{
  ptrHandling<map1dTo3d> m1d(_msCut->segmentConstU(uu));
