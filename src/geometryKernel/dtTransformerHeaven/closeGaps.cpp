...
bool closeGaps::_registrated =
  dtTransformerFactory::registrate(dt__tmpPtr(closeGaps, new closeGaps()));
...
dtTransformer *closeGaps::create(void) const { return new closeGaps(); }

lvH_analyticGeometry closeGaps::apply(lvH_analyticGeometry const *const toTrans
) const
{
  ...
  dt__forAllRefAuto(*toTrans, aTrans)
  {
    ...
    if (_vvStartAGeo)
    {
      for (int ii = 0; ii < aS->ptrDtSurface()->nControlPointsU(); ii++)
      {
        ...
        dtPoint3 cPNearest = _vvStartAGeo->getPoint(nearest.x(), nearest.y());
    ...
    if (_vvEndAGeo)
    {
      ...
      for (int ii = 0; ii < aS->ptrDtSurface()->nControlPointsU(); ii++)
      {
        ...
        dtPoint3 cPNearest = _vvEndAGeo->getPoint(nearest.x(), nearest.y());
...
bool closeGaps::isNecessary(void) const { return true; }
...
)
{
  ...
  _vvStartAGeo.reset(
    analyticSurface::DownCast(aG->get(dtXmlParserBase::getAttributeStr(
