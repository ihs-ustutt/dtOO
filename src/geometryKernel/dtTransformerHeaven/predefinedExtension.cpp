...
namespace dtOO {
bool predefinedExtension::_registrated = dtTransformerFactory::registrate(
  dt__tmpPtr(predefinedExtension, new predefinedExtension())
);

predefinedExtension::predefinedExtension() : dtTransformer() {}

predefinedExtension::predefinedExtension(predefinedExtension const &orig)
  : dtTransformer(orig)
{
  _yIn = orig._yIn;
  _yOut = orig._yOut;
...
predefinedExtension::apply(lvH_analyticFunction const *const sFunP) const
{
  ...
  for (int ii = 0; ii < sFunP->size(); ii++)
  {
    ...
    if (vS2)
    {
      ...
      std::vector<dtPoint2> pV = calculateExtPoints(start, vvStart, end, vvEnd);
    ...
    else if (vS3)
    {
      ...
      std::vector<dtPoint2> pV = calculateExtPoints(
        dtPoint2(start.x(), start.y()),
        dtVector2(vvStart.x(), vvStart.y()),
        dtPoint2(end.x(), end.y()),
        dtVector2(vvEnd.x(), vvEnd.y())
      );
    ...
    {
      dt__throw(apply(), << dt__eval(vS2) << std::endl << dt__eval(vS3));
    }
  }
  return transSFun;
}
...
)
{
  ...
  _alphaIn =
    dtXmlParserBase::muParseString(dtXmlParserBase::replaceDependencies(
      dtXmlParserBase::getAttributeStr("alpha_in", *tE), cV, aF
    ));
  _yIn = dtXmlParserBase::muParseString(dtXmlParserBase::replaceDependencies(
    dtXmlParserBase::getAttributeStr("y_in", *tE), cV, aF
  ));
  _alphaOut =
    dtXmlParserBase::muParseString(dtXmlParserBase::replaceDependencies(
      dtXmlParserBase::getAttributeStr("alpha_out", *tE), cV, aF
    ));
  _yOut = dtXmlParserBase::muParseString(dtXmlParserBase::replaceDependencies(
    dtXmlParserBase::getAttributeStr("y_out", *tE), cV, aF
  ));
}

bool predefinedExtension::isNecessary(void) const { return true; }
...
) const
{
  ...
  //
  dtReal xx =
    _alphaIn * (_yIn - start.y()) * vvStart.x() / vvStart.y() + start.x();
