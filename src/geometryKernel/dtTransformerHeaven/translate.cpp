...
bool translate::_registrated =
  dtTransformerFactory::registrate(dt__tmpPtr(translate, new translate()));

translate::translate(jsonPrimitive const &jE) : dtTransformer(jE)
{
...
dtTransformer *translate::clone(void) const { return new translate(*this); }

dtTransformer *translate::create(void) const { return new translate(); }

std::vector<dtPoint3>
translate::apply(std::vector<dtPoint3> const *const toTrans) const
{
...
lvH_analyticFunction translate::apply(lvH_analyticFunction const *const aFunVecP
) const
{
  ...
  dt__forAllRefAuto(*aFunVecP, aF)
  {
    ...
    //
    scaCurve2dOneD const *const sC2d1d = scaCurve2dOneD::ConstDownCast(aF);
    vec2dCurve2dOneD const *const v2dC1d = vec2dCurve2dOneD::ConstDownCast(aF);
    vec3dCurveOneD const *const v3dC1d = vec3dCurveOneD::ConstDownCast(aF);

    if (sC2d1d)
    {
      retSFun.push_back(new scaCurve2dOneD(dt__tmpPtr(
        dtCurve2d,
        geomCurve2d_curve2dTranslateConstructOCC(
          sC2d1d->constPtrDtCurve2d(), config().lookup<dtVector2>("_v2")
        )
          .result()
      )));
    ...
    {
      retSFun.push_back(new vec2dCurve2dOneD(dt__tmpPtr(
        dtCurve2d,
        geomCurve2d_curve2dTranslateConstructOCC(
          v2dC1d->ptrDtCurve2d(), config().lookup<dtVector2>("_v2")
        )
          .result()
      )));
    ...
    {
      retSFun.push_back(new vec3dCurveOneD(dt__tmpPtr(
        dtCurve,
        geomCurve_curveTranslateConstructOCC(
          v3dC1d->ptrDtCurve(), config().lookup<dtVector3>("_v3")
        )
          .result()
      )));
...
lvH_analyticGeometry translate::apply(lvH_analyticGeometry const *const aGeoVecP
) const
{
    ...
    if (m1d)
    {
      ...
      if (s3 && !s3->isTransformed())
      {
        retAGeo.push_back(new analyticCurve(dt__tmpPtr(
          dtCurve,
          geomCurve_curveTranslateConstructOCC(
            s3->ptrConstDtCurve(), config().lookup<dtVector3>("_v3")
          )
            .result()
        )));
      }
      ...
      if (aS && !aS->isTransformed())
      {
        retAGeo.push_back(new analyticSurface(dt__tmpPtr(
          ...
          geomSurface_surfaceTranslateConstructOCC(
            aS->ptrDtSurface(), config().lookup<dtVector3>("_v3")
          )
            .result()
        )));
      }
...
std::vector<dtPoint3>
translate::retract(std::vector<dtPoint3> const *const toRetract) const
{
...
bool translate::isNecessary(void) const { return true; }
