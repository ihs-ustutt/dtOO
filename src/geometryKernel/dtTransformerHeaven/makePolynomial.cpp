...
namespace dtOO {
bool makePolynomial::_registrated = dtTransformerFactory::registrate(
...
makePolynomial::apply(lvH_analyticGeometry const *const aGeoVecP) const
{
  ...
  for (int ii = 0; ii < aGeoVecP->size(); ii++)
  {
    ...
    //
    //      map2dTo3d const * map2dTo3dP = dynamic_cast< map2dTo3d const * >(
    //      retAGeo[ii] );
    ...
    //          }
    //        }
    //        analyticSurface * aS = new analyticSurface(&point, _orderOne,
    //        _orderTwo); aS->setLabel( retAGeo[ii]->getLabel() ); delete
    //        retAGeo[ii]; retAGeo[ii] = aS;
    //      }
    //
    // curve mapping
    //
    if (map1dTo3dP)
    {
      ...
      for (int ll = 1; ll <= _nPointsOne - 1; ll++)
      { // first direction
...
bool makePolynomial::isNecessary(void) const { return true; }
...
)
{
  ...
  {
    _nPointsOne =
      dtXmlParserBase::muParseStringInt(dtXmlParserBase::replaceDependencies(
        dtXmlParserBase::getAttributeStr("number_points_one", *tE), cV, aF
      ));
  }
  if (dtXmlParserBase::hasAttribute("number_points_two", *tE))
  {
    _nPointsTwo =
      dtXmlParserBase::muParseStringInt(dtXmlParserBase::replaceDependencies(
        dtXmlParserBase::getAttributeStr("number_points_two", *tE), cV, aF
      ));
  }
  if (dtXmlParserBase::hasAttribute("order_one", *tE))
  {
    _orderOne =
      dtXmlParserBase::muParseStringInt(dtXmlParserBase::replaceDependencies(
        dtXmlParserBase::getAttributeStr("order_one", *tE), cV, aF
      ));
  }
  if (dtXmlParserBase::hasAttribute("order_two", *tE))
  {
    _orderTwo =
      dtXmlParserBase::muParseStringInt(dtXmlParserBase::replaceDependencies(
        dtXmlParserBase::getAttributeStr("order_two", *tE), cV, aF
      ));
  }
}
} // namespace dtOO
