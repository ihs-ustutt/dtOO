...
namespace dtOO {
bool approxInSurface::_registrated = dtTransformerFactory::registrate(
...
approxInSurface::apply(lvH_analyticGeometry const *const aGeoVecP) const
{
  ...
  for (int ii = 0; ii < aGeoVecP->size(); ii++)
  {
    ...
    //
    dt__pH(dtCurve2d)
      dtC2d(bSplineCurve2d_pointConstructOCC(ppUV, _order).result());
...
bool approxInSurface::isNecessary(void) const { return true; }
