...
bool uVw_phirMs::_registrated =
  dtTransformerFactory::registrate(dt__tmpPtr(uVw_phirMs, new uVw_phirMs()));

uVw_phirMs::uVw_phirMs() : dtTransformer() {}

uVw_phirMs::uVw_phirMs(uVw_phirMs const &orig) : dtTransformer(orig)
{
  _rM2d.reset(orig._rM2d->clone());
  _ms_uSPercentVSPercent.reset(orig._ms_uSPercentVSPercent->clone());
}

uVw_phirMs::uVw_phirMs(jsonPrimitive const &jE) : dtTransformer(jE)
{
...
dtTransformer *uVw_phirMs::clone(void) const { return new uVw_phirMs(*this); }

dtTransformer *uVw_phirMs::create(void) const { return new uVw_phirMs(); }

lvH_analyticFunction uVw_phirMs::apply(lvH_analyticFunction const *const aFP
) const
{
...
std::vector<dtPoint3>
uVw_phirMs::retract(std::vector<dtPoint3> const *const toRetract) const
{
  std::vector<dtPoint3> retVec;
  dtVector3 scale = config().lookupDef("_ss", dtVector3(1.0, 1.0, 1.0));
...
bool uVw_phirMs::isNecessary(void) const { return true; }
