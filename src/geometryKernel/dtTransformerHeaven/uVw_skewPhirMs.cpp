...
namespace dtOO {
bool uVw_skewPhirMs::_registrated = dtTransformerFactory::registrate(
...
uVw_skewPhirMs::uVw_skewPhirMs(uVw_skewPhirMs const &orig) : dtTransformer(orig)
{
  ...
  _nW = orig._nW;
  if (orig._fU)
    _fU.reset(orig._fU->clone());
  if (orig._fV)
    _fV.reset(orig._fV->clone());
...
lvH_analyticFunction uVw_skewPhirMs::apply(lvH_analyticFunction const *const aFP
) const
{
...
std::vector<dtPoint3>
uVw_skewPhirMs::apply(std::vector<dtPoint3> const *const toTrans) const
{
...
std::vector<dtPoint3>
uVw_skewPhirMs::retract(std::vector<dtPoint3> const *const toRetract) const
{
  std::vector<dtPoint3> retVec;
...
bool uVw_skewPhirMs::isNecessary(void) const { return true; }
...
)
{
  ...
  {
    _fU.reset(scaOneD::MustDownCast(aF->get(dtXmlParserBase::getAttributeStr(
  ...
  {
    _fV.reset(scaOneD::MustDownCast(aF->get(dtXmlParserBase::getAttributeStr(
  ...
  {
    _relaxM = dtXmlParserBase::getAttributeFloatMuParse("relaxM", *tE, cV, aF);
  }
  if (dtXmlParserBase::hasAttribute("relaxPhir", *tE))
  {
    _relaxPhir =
      dtXmlParserBase::getAttributeFloatMuParse("relaxPhir", *tE, cV, aF);
  }
  if (dtXmlParserBase::hasAttribute("nSteps", *tE))
  {
    _nSteps = dtXmlParserBase::getAttributeIntMuParse("nSteps", *tE, cV, aF);
  }
  if (dtXmlParserBase::hasAttribute("angle", *tE))
  {
    _angle = dtXmlParserBase::getAttributeFloatMuParse("angle", *tE, cV, aF);
  }

  std::vector<std::vector<std::vector<dtPoint3>>> phirMS(
    _nU, std::vector<std::vector<dtPoint3>>(_nV, std::vector<dtPoint3>(_nW))
  );
  ...
  dt__forFromToIndex(0, _nW, kk)
  {
    ...
    logC() << dtLog::dtFormat("%12s | %12s | %12s") % "" % "|| isM ||" %
                "|| isPhir ||"
           << std::endl
           << dtLog::dtFormat("%12s | %12s | %12s") % "step" % "------------" %
                "------------"
           << std::endl
           << dtLog::dtFormat("%12s | %12s | %12s") % "" % "size(isM)" %
                "size(isPhir)"
           << std::endl;
    ...
    dt__forAllIndex(phirM_1, ii)
    {
      ...
      {
        dtVector3 vXYZ =
          _rM2d->origin() - _rM2d->constRefMap2dTo3d().getPointPercent(
        ...
        phirM_0[ii][jj] =
          dtVector2(uPercent[ii] * 2. * M_PI * dtLinearAlgebra::length(rr), mm);
    ...
    dt__forFromToIndex(0, _nSteps, thisStep)
    {
      ...
      // output to container
      //
      logC() << dtLog::dtFormat("%12i | %12.5f | %12.5f") % thisStep %
    ...
    std::fstream of;
    of.open(
      getLabel() + "_phirM_" + stringPrimitive::floatToString(wPercent[kk]) +
        ".csv",
      std::ios::out | std::ios::trunc
    );
    of.precision(8);
    ...
    dt__forAllIndex(phirM_1, ii)
    {
      dt__forAllIndex(phirM_1[ii], jj)
      {
        of << phirM_1[ii][jj].x() << ", " << phirM_1[ii][jj].y() << ", "
           << wPercent[kk] << std::endl;
      }
      of << std::endl << std::endl;
    }
    dt__forAllIndex(phirM_1[0], jj)
    ...
    dt__forAllIndex(sumPhirM, ii)
    {
      dt__forAllIndex(sumPhirM[ii], jj)
      {
        ...
        phirMS[ii][jj][kk] =
          dtPoint3(sumPhirM[ii][jj].x(), sumPhirM[ii][jj].y(), wPercent[kk]);
      }
    }
    ...
    dt__forAllIndex(sumPhirM, ii)
    {
      std::fstream of;
      of.open(
        getLabel() + "_const_phir_" + stringPrimitive::intToString(ii) + "_" +
          stringPrimitive::intToString(kk) + ".csv",
        std::ios::out | std::ios::trunc
      );
      of.precision(8);
      of.fixed;
    ...
    dt__forAllIndex(sumPhirM[0], jj)
    {
      std::fstream of;
      of.open(
        getLabel() + "_const_m_" + stringPrimitive::intToString(jj) + "_" +
          stringPrimitive::intToString(kk) + ".csv",
        std::ios::out | std::ios::trunc
      );
      of.precision(8);
  ...
  dt__forFromToIndex(1, phirMS.size(), ii)
  {
    dt__forFromToIndex(1, phirMS[ii].size(), jj)
    {
      ...
      {
        theMap.add(new vec3dTriLinearThreeD(
          phirMS[ii - 1][jj - 1][kk - 1],
          phirMS[ii - 0][jj - 1][kk - 1],
          phirMS[ii - 1][jj - 0][kk - 1],
          phirMS[ii - 0][jj - 0][kk - 1],
          phirMS[ii - 1][jj - 1][kk - 0],
          phirMS[ii - 0][jj - 1][kk - 0],
          phirMS[ii - 1][jj - 0][kk - 0],
          phirMS[ii - 0][jj - 0][kk - 0],
