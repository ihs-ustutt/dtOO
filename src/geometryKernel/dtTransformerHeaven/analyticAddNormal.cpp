...
namespace dtOO {
bool analyticAddNormal::_registrated = dtTransformerFactory::registrate(
  dt__tmpPtr(analyticAddNormal, new analyticAddNormal())
);

analyticAddNormal::analyticAddNormal() : dtTransformer() {}

analyticAddNormal::analyticAddNormal(analyticAddNormal const &orig)
  : dtTransformer(orig)
{
...
analyticAddNormal::apply(lvH_analyticFunction const *const aFP) const
{
  ...
  for (int ii = 0; ii < aFP->size(); ii++)
  {
    ...
    if (vec3d2d)
    {
      retV.push_back(new vec3dThickedTwoD(vec3d2d, _tt.get(), _nf));
      retV.back()->setLabel(aF->getLabel());
    }
    else
    {
...
bool analyticAddNormal::isNecessary(void) const { return true; }

void analyticAddNormal::handleAnalyticFunction(
...
void analyticAddNormal::handleDtVector3(
