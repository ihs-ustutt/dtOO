...
bool xYz_rPhiZ::_registrated =
  dtTransformerFactory::registrate(dt__tmpPtr(xYz_rPhiZ, new xYz_rPhiZ()));

xYz_rPhiZ::xYz_rPhiZ() : dtTransformer() {}

xYz_rPhiZ::xYz_rPhiZ(xYz_rPhiZ const &orig) : dtTransformer(orig) {}

xYz_rPhiZ::xYz_rPhiZ(jsonPrimitive const &jE) : dtTransformer(jE)
{
...
dtTransformer *xYz_rPhiZ::clone(void) const { return new xYz_rPhiZ(*this); }

dtTransformer *xYz_rPhiZ::create(void) const { return new xYz_rPhiZ(); }

std::vector<dtPoint3>
xYz_rPhiZ::apply(std::vector<dtPoint3> const *const toTrans) const
{
...
std::vector<dtPoint3>
xYz_rPhiZ::retract(std::vector<dtPoint3> const *const toRetract) const
{
  std::vector<dtPoint3> retVec;
  ...
  dt__forAllRefAuto(*toRetract, aPoint)
  {
    ...
    //
    dtVector3 ref2Axis =
      dtLinearAlgebra::normalize(dtLinearAlgebra::crossProduct(rotAxis, refAxis)
      );

    //
    // phi coordinate
    //
    dtReal coordPhi = atan2(
...
bool xYz_rPhiZ::isNecessary(void) const { return true; }
...
)
{
  ...
  config.append(
    "_rotAxis",
    dtLinearAlgebra::normalize(dtXmlParserBase::getDtVector3(
