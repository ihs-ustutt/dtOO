...
constValue::constValue(std::string const &label, dtReal const &val)
  : labelHandling(label)
{
  _loadable = true;
  _value = val;
...
)
{
  ...
  {
    aCV = new sliderFloatParam(label, stringPrimitive::stringToFloat(valueStr));
  }
  else if (type == "intParam")
  {
    aCV = new intParam(label, stringPrimitive::stringToInt(valueStr));
  }
...
dtReal constValue::getValue(void) const { return _value; }
...
dtReal constValue::operator()(void) const { return this->getValue(); }
...
dtReal constValue::getMax(void) const { return _max; }

dtReal constValue::getMin(void) const { return _min; }

bool constValue::loadable(void) const { return this->_loadable; }

void constValue::resolveConstraint(lvH_constValue const *const cVArr) {}

void constValue::setState(std::string const &state)
{
  dt__debug(
    setState(), << "cV[ " << getLabel() << " ] : " << _state << " -> " << state
  );
  _state = state;
}

std::string constValue::getState(void) const { return _state; }

void constValue::freeState(void)
{
  dt__debug(
    freeState(), << "cV[ " << getLabel() << " ] : " << _state << " -> "
  );
  _state = "";
}

bool constValue::inState(void) const { return (_state != ""); }
