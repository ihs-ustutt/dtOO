...
int vec2dOneD::xDim(void) const { return 1; }

aFY vec2dOneD::Y(aFX const &xx) const { return Y(xx[0]); }
...
void vec2dOneD::setMin(dtReal const &min) { _min = min; }

void vec2dOneD::setMax(dtReal const &max) { _max = max; }
...
dtReal vec2dOneD::length(void) const { return length(xMax(0)); }
