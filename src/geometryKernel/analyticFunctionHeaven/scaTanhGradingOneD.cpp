...
namespace dtOO {
scaTanhGradingOneD::scaTanhGradingOneD() : scaOneDPolyInterface()
{
  setMinMax(0., 1.);
  _gg = 1.;
  _ggMin = 1.;
...
scaTanhGradingOneD::scaTanhGradingOneD(scaTanhGradingOneD const &orig)
...
{
  return new analyticFunctionTransformed<scaTanhGradingOneD>(*this, dtT);
}
...
{
  return _cc[0] + _cc[1] * tanh(_gg * (_cc[2] + _cc[3] * xx)) / tanh(_gg);
}

dtInt scaTanhGradingOneD::nDOF(void) const { return 1; }

void scaTanhGradingOneD::setDOF(std::vector<dtReal> const value)
{
