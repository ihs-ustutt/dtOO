...
template <typename funT> scaMultiOneD<funT>::scaMultiOneD() : funT() {}
...
template <typename funT> scaMultiOneD<funT>::~scaMultiOneD() {}
...
funT const &scaMultiOneD<funT>::findF_x(dtReal const &xx) const
{
  ...
  if (iter == _ff.end())
  {
    dt__throw(findF_x(), << "Cannot determine range " << dt__eval(xx));
  }
...
funT const &scaMultiOneD<funT>::findF_y(dtReal const &yy) const
{
  auto iter = std::find_if(
    ...
    _ff.cend(),
    [=](const std::pair<std::pair<dtReal, dtReal>, funT *> &fn) {
      return yy >= fn.second->YFloat(fn.first.first) &&
...
template <typename funT> void scaMultiOneD<funT>::add(funT const &aFun)
{
  ...
  // add function
  //
  _ff[std::pair<dtReal, dtReal>(xMin, xMax)] = aFun.clone();
