...
analyticFunction *analyticFunction::weakClone(void) const { return clone(); }

analyticFunction *
analyticFunction::cloneTransformed(dtTransformer const *const dtT) const
{
...
bool analyticFunction::isCompound(void) const { return false; }

vectorHandling<analyticFunction *> const &analyticFunction::vecRef(void) const
{
  dt__throwUnexpected(vecRef);
}

std::map<int, dtInt> const &analyticFunction::mapRef(void) const
{
  dt__throwUnexpected(mapRef);
}

bool analyticFunction::isTransformed(void) const { return false; }
...
aFX analyticFunction::aFXZeroD(void) { return aFX(0, 0.); }
...
aFY analyticFunction::aFYZeroD(void) { return aFY(0, 0.); }
...
aFY analyticFunction::aFY_aFX(aFX const &xx) { return aFY(xx); }

aFX analyticFunction::aFX_aFY(aFY const &yy) { return aFX(yy); }
...
std::vector<dtReal> const analyticFunction::operator()(dtReal const &xx) const
{
...
analyticFunction::operator()(dtReal const &xx, dtReal const &yy) const
{
