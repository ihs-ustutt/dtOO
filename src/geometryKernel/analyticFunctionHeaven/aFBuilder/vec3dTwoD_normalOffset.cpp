...
)
{
  ...
  dt__forAllIndex(uGrid[0], jj)
  {
    dt__forAllIndex(uGrid, ii)
    {
      ...
      // solve AA * xx = bb
      //
      dtMatrix AA = dtLinearAlgebra::createMatrixGiveColumns(
        ::boost::assign::list_of(der[0])(der[1])(
          -nn + dtLinearAlgebra::dotProduct(nn, nf) * nf
        )
      );
      dtMatrixVector bb = dtLinearAlgebra::createMatrixVector(-tt * nn);
      ...
      dt__info(
        ...
        << logMe::dtFormat(
             ...
             "-> (%f, %f, %f) / (%f, %f, %f)"
               ...
               grid[1][jj].z() % grid[grid.size(0) - 2][jj].x() %
               grid[grid.size(0) - 2][jj].y() % grid[grid.size(0) - 2][jj].z()
      );
    }
  }
  if (closeU)
  ...
  dt__forFromToIndex(0, grid.size(1), jj)
  {
    ...
    _vC.push_back(new vec3dCurveOneD(dt__tmpPtr(
      dtCurve, bSplineCurve_pointConstructOCC(grid.fixJ(jj), order).result()
    )));
  }
  cV3d.reset(new vec3dSurfaceTwoD(dt__tmpPtr(
    dtSurface,
...
vec3dTwoD *vec3dTwoD_normalOffset::result(void) const { return _vS->clone(); }
