...
dtInt scaThreeD::xDim(void) const { return 3; }
...
aFX scaThreeD::x_percent(dtReal const &x0, dtReal const &x1, dtReal const &x2)
  const
{
  return x_percent(analyticFunction::aFXThreeD(x0, x1, x2));
}

aFX scaThreeD::percent_x(dtReal const &x0, dtReal const &x1, dtReal const &x2)
  const
{
...
std::vector<dtReal>
scaThreeD::DYFloat(dtReal const &x0, dtReal const &x1, dtReal const &x2) const
{
  aFX xx(2, 0);
  xx[0] = x0;
  xx[1] = x1;
