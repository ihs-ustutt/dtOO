...
int vec3dThreeD::xDim(void) const { return 3; }

aFY vec3dThreeD::Y(dtReal const &x0, dtReal const &x1, dtReal const &x2) const
{
  aFX xx(3, 0);
...
aFX vec3dThreeD::x_percent(dtReal const &x0, dtReal const &x1, dtReal const &x2)
  const
{
  aFX xx(3, 0.);
  xx[0] = x0;
...
aFX vec3dThreeD::percent_x(dtReal const &x0, dtReal const &x1, dtReal const &x2)
  const
{
  aFX xx(3, 0.);
  xx[0] = x0;
...
std::vector<dtVector3> vec3dThreeD::DYdtVector3(aFX const &xx) const
{
  ...
  dxdy[0] = (yy[0][1] - yy[0][0]) / (x_percent(uv[0][1], xP[1], xP[2])[0] -
                                     x_percent(uv[0][0], xP[1], xP[2])[0]);
  dxdy[1] = (yy[1][1] - yy[1][0]) / (x_percent(xP[0], uv[1][1], xP[2])[1] -
                                     x_percent(xP[0], uv[1][0], xP[2])[1]);
  dxdy[2] = (yy[2][1] - yy[2][0]) / (x_percent(xP[0], xP[1], uv[2][1])[2] -
...
vectorHandling<renderInterface *> vec3dThreeD::getRender(void) const
{
  ...
  int nW = staticPropertiesHandler::getInstance()->getOptionInt(
