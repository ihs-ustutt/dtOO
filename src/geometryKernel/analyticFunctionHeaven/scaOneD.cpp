...
dtReal scaOneD::YFloat(aFX const &xx) const
{
  if (xx.size() != 1)
  {
    dt__throw(YFloat(), << dt__eval(xx.size()) << std::endl << "Should be 1.");
  }
...
dtInt scaOneD::xDim(void) const { return 1; }
...
void scaOneD::setMin(dtReal const min) { _xxMin = min; }

void scaOneD::setMax(dtReal const max) { _xxMax = max; }
...
dtReal scaOneD::getLength(void) const
{
  ...
  while (value[0] <= scaOneD::xMax(0))
  {
    ...
    jjF = jjF + 1.;
    value[0] =
      scaOneD::xMin(0) + (scaOneD::xMax(0) - scaOneD::xMin(0)) * jjF * epsge;
    value[1] = YFloat(value[0]);
    ...
    curLength =
      curLength + sqrt(
                    (value[0] - valueOld[0]) * (value[0] - valueOld[0]) +
