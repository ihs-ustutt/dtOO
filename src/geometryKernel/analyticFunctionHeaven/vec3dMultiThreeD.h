...
template <typename funT> vec3dMultiThreeD<funT>::vec3dMultiThreeD() : funT() {}

template <typename funT>
vec3dMultiThreeD<funT>::vec3dMultiThreeD(vec3dMultiThreeD const &orig)
  : funT(orig)
{
  //
  // copy functions
...
template <typename funT> vec3dMultiThreeD<funT>::~vec3dMultiThreeD()
{
...
template <typename funT> aFY vec3dMultiThreeD<funT>::Y(aFX const &xx) const
{
...
funT const &vec3dMultiThreeD<funT>::findF_x(aFX const &xx) const
{
  ...
  if ((glMinX[0] > xx[0]) || (glMinX[1] > xx[1]) || (glMinX[2] > xx[2]) ||
  ...
  std::vector<dtReal> max =
    ::boost::assign::list_of(xx[0] + _tol)(xx[1] + _tol)(xx[2] + _tol);
  dtInt nEnt = const_cast<vec3dMultiThreeD<funT> *>(this)->_x_rTree.Search(
  ...
  else if (nEnt > 1)
  {
    dt__forFromToIndex(0, nEnt, ii)
    {
      ...
      if ((minX[0] <= xx[0]) && (minX[1] <= xx[1]) && (minX[2] <= xx[2]) &&
...
template <typename funT> void vec3dMultiThreeD<funT>::add(funT *aFun)
{
  ...
  {
    if (funT::xMin(ii) > thisMin[ii])
      funT::setMin(ii, thisMin[ii]);
    if (funT::xMax(ii) < thisMax[ii])
      funT::setMax(ii, thisMax[ii]);
  }
  ...
  //
  std::vector<dtReal> min =
    ::boost::assign::list_of(thisMin[0] - _tol)(thisMin[1] - _tol)(
      thisMin[2] - _tol
    );
  std::vector<dtReal> max =
    ::boost::assign::list_of(thisMax[0] + _tol)(thisMax[1] + _tol)(
      thisMax[2] + _tol
    );
  _x_rTree.Insert(&min[0], &max[0], aFun);
}

template <typename funT> void vec3dMultiThreeD<funT>::dump(void) const
{
