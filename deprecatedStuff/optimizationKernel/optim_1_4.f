...
c
      ...
      integer iiter,niter
      integer npar
      integer n,nred,nsimp1,nsimp2,ngen,nzuf,npp,restart,nredfix,ueber
...
c Steuerfile einlesen
     ...
     * n,nred,mset,niter,eps,restart)
      ...
      nredfix=nred
      call calcn(n,npp,nsimp1,nsimp2,ngen,nzuf,eps,npar,nred,
     * nredfix,ueber)
...
c          text="./opti_shell "//char(0)
c          call system(text)
      call c_optimizer_callback()
c==========================================

c Einlesen (evtl. nred reduzieren und n's neu rechnen)
          nred=0
          call leserg(n,erg,mset,nred,nredfix,ueber)
...
c der vorherigen Optimierung (evtl nred reduzieren und n's neu rechnen)
      else 
         nredtmp=0
...
c reduzierung von nred bei unphysikalischen Ergebnissen
             if(erg(i).lt.0) nredtmp=nredtmp+1
         ...
         ueber=ueber+nredtmp
         if(nredtmp.lt.nredfix) nred=nredtmp

         call calcn(n,npp,nsimp1,nsimp2,ngen,nzuf,eps,npar,nred,
     * nredfix,ueber)

c 14.10.09: tut nicht mehr wegen calcn()....
...
c Alle in engem Radius => Konvergenz
      if(radmax.lt.eps.and.ueber.gt.nredfix) goto 200
...
c subroutine leserg
ccccccccccccccccccc
      subroutine leserg(n,erg,mset,nred,nredfix,ueber)

      implicit none

      real erg
      integer n,i,nred,nredfix,ueber,tmp
      integer mset
      dimension erg(mset)
...
c subroutine calcn
cccccccccccccccccc
      subroutine calcn(n,npp,nsimp1,nsimp2,ngen,nzuf,eps,npar,nred,
     * nredfix,ueber)

      implicit none

      real eps
      integer n,npp,nsimp1,nsimp2,ngen,nzuf,npar
      integer nred,nredfix,ueber,tmp

      tmp=nredfix/2
      if(ueber.gt.tmp) then
          npp=(n-nredfix)/6
      else
          npp=0
      end if
      ...
      Write(*,*) n,nred,ngen,nsimp1,nsimp2,nzuf,eps,npar,ueber
      return
      end