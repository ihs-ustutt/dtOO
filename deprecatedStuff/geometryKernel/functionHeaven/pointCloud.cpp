...
namespace dtOO { 
  pointCloud::pointCloud() : scaFunction() {
    
  }
  
  pointCloud::pointCloud(std::vector< dtPoint2> const * point) : scaFunction() {
    _point.clear();
  ...
  pointCloud::pointCloud(const pointCloud& orig) : scaFunction(orig){
    _point = orig._point;
  }

  pointCloud * pointCloud::clone( void ) const {
    return new pointCloud( *this );
  }
  
  pointCloud * pointCloud::create( void ) const {
  ...
  void pointCloud::dump(void) {
    
  }
  float pointCloud::getValue(float const xx) const {
  }

  std::vector< float > pointCloud::getVectorValue(float const xx) const {
    
  }  
  
  coDoSet* pointCloud::toCoDoSet(char const * const str) const {
    //
    // get min and max
    //
  ...
  //              << "S1 COLOR " << 1%12+2 << "\n"
      ...
      xMin = getSetRenderingMinX();
      xMax = getSetRenderingMaxX();
      yMin = getSetRenderingMinY();
      yMax = getSetRenderingMaxY();      
      outStream << "\0"
                << "TITLE \"" << getLabel() << "\"\n"
                << "xaxis LABEL \"" << "x" << "\"\n"
                << "yaxis LABEL \"" << "y" << "\"\n"
                ...
                << "YAXIS TICK MAJOR " << (yMax-yMin)/5 << "\nYAXIS TICK MINOR " << (yMax-yMin)/10 << "\n"
                << "XAXIS TICK MAJOR " << (xMax-xMin)/5 << "\nXAXIS TICK MINOR " << (xMax-xMin)/10 << "\n";
      std::string renderString = getRenderingOption();
      while ( stringPrimitive().stringContains("[", renderString) ) {
        outStream << stringPrimitive().getStringBetweenAndRemove("[", "]", &renderString) << "\n";
      }
    }
  ...
  coDoVec2* pointCloud::toCoDoVec2(char const * const str) {

  }  
  
  int pointCloud::getSurroundingPoints(float const xx, float const yy, float const rX, float const rY, float ** xxSur, float ** yySur) {
    int nPoints = 9;
    *xxSur = new float[nPoints];
    *yySur = new float[nPoints];
    
    float dPhi = 2. * M_PI / ((float) (nPoints-1));
    for (int ii=0;ii<nPoints;ii++) {
      float iiF = (float) ii;
      (*xxSur)[ii] = xx + rX * cos(iiF * dPhi);
      (*yySur)[ii] = yy + rY * sin(iiF * dPhi);
    }
    
    return nPoints;
  }

  covise::coDoSet * pointCloud::packToCoDoSet( char const * const str) const {
    covise::coDistributedObject ** elem = new covise::coDistributedObject*[3];
    
    elem[0] = const_cast< covise::coDistributedObject * >(
              scaFunction::packToCoDoSet(
                ("pointCloud::scaFunction::"+getLabel()).c_str()
              )->getElement(0)
            );
  ...
  void pointCloud::unpackFromCoDoSet( covise::coDoSet const * const set ) {
