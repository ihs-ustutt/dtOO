...
class rotatingMap2dTo3d_splitFitted(dtBundleBuilder):
  ...
  def build(self) -> None:
    ...
    for internal in self.internals_:
      ...
      for segment in self.segments_:
        ...
        # reparam in channel
        #
        p_0 = self.channel_.reparamInVolume( bb.first )
        p_1 = self.channel_.reparamInVolume( bb.second )
        p_mid = self.channel_.reparamInVolume( midPoint )
        logging.debug(
          "\n"
          "Segment position = %f ( %d ) \n"
          "  bb_0 = (%f, %f, %f)\n"
          "  bb_1 = (%f, %f, %f)\n"
          "  p_0 = (%f, %f, %f)\n"
          "  p_1 = (%f, %f, %f)\n"
          ...
          (
            segment, ii,
            bb.first[0], bb.first[1], bb.first[2],
            bb.second[0], bb.second[1], bb.second[2],
            p_0[0], p_0[1], p_0[2],
            p_1[0], p_1[1], p_1[2],
            self.segmentDir_, p_mid[self.segmentDir_], 
            self.channel_.percent_uvw(p_mid)[self.segmentDir_]
          )
        )
        #
        # determine min and max values for split and store segment value of mid
        # point in segment_val
        #
    ...
    for ii in range(nSegments):#self.segments_:
      logging.info(
        ...
        %
        (
          self.segments_[ii], ii, 
          segment_val[ii], gMin, split_min[ii], split_max[ii], gMax
        )
      )
      ii = ii + 1
        ...
        pps_min.push_back( 
          dtPoint3(
            self.channel_.getPoint(
              0.0, split_min[ii], self.channel_.w_percent(segment)
            )
          ) 
        )
        pps_max.push_back( 
          dtPoint3(
            self.channel_.getPoint(
              0.0, split_max[ii], self.channel_.w_percent(segment)
            )
          ) 
        )
        ii = ii + 1
    ...
    if (self.segmentDir_==2) and (self.splitDir_==1):
      ...
      for ii in range(segment_val.size):
        ...
        #)
        ccs_0.push_back(analyticCurve.MustDownCast(line_0).ptrDtCurve().clone())
        ccs_1.push_back(analyticCurve.MustDownCast(line_1).ptrDtCurve().clone())
        ccs_2.push_back(analyticCurve.MustDownCast(line_2).ptrDtCurve().clone())
        if self.debug():
      ...
      # create surface
      #
      occSurf_0 = bSplineSurface_skinConstructOCC(ccs_0, 2, 2, 0).result()
      occSurf_1 = bSplineSurface_skinConstructOCC(ccs_1, 2, 2, 0).result()
      occSurf_2 = bSplineSurface_skinConstructOCC(ccs_2, 2, 2, 0).result()
      if self.debug():
        self.appendAnalyticGeometry(
          analyticSurface( occSurf_0 ), self.label_+"_debug_surface_0"
        )
        self.appendAnalyticGeometry(
          analyticSurface( occSurf_1 ), self.label_+"_debug_surface_1"
        )
        self.appendAnalyticGeometry(
          analyticSurface( occSurf_2 ), self.label_+"_debug_surface_2"
        )

      self.appendAnalyticGeometry( 
        rotatingMap2dTo3d(
          self.channel_.rotationAxis(),
      ...
      ValueError("Segment direction must be 2 and split direction must be 1.")
  ...
  def detectDirection( 
    ...
    for p1 in [
      ...
      logging.debug( "%d : dist = %f" % (detDir, dist) )
      if dist> maxDiff:
        maxDetDir = detDir
        maxDiff = dist
      detDir = detDir + 1
