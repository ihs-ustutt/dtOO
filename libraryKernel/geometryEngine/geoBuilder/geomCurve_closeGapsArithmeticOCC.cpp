...
namespace dtOO {
    ...
    //
		dt__forAllConstIter(dt__pVH(dtCurve), dtC, it) {
			...
			theClone->setControlPoint(
			  0, dtLinearAlgebra::toDtPoint3(pp[ii].first) 
			);
			theClone->setControlPoint( 
			  theClone->nControlPoints()-1, 
				dtLinearAlgebra::toDtPoint3(pp[ii].second)
			);
	...
	dt__pVH(dtCurve) geomCurve_closeGapsArithmeticOCC::result( void ) {
		return _dtC;		
	}
}
