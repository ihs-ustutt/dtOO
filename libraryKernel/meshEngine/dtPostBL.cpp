...
//					exit(0);
			...
			moab::Tag GDTag, GIDTag, NTag, MTag, STag, MNTag, MatIDTag, BLNodeIDTag;
...
//																	moab::MB_TAG_SPARSE|moab::MB_TAG_CREAT),mb);
			...
			std::vector< moab::EntityHandle > this_setVec;
			for (set_it = n_sets.begin(); set_it != n_sets.end(); set_it++)  {
					...
					m_LogFile << "set_id = " << set_id << std::endl;
					if(abs(set_id) == m_NeumannSet) {
            m_LogFile <<  "Pushing " << m_NeumannSet << ". Found as " << set_id << std::endl;						
						this_setVec.push_back(this_set);
					}
//							break;
//					this_set = 0;
			}
			...
			moab::Range quadsPos, quadsNeg;//, fixmat_ents;
			int dims; // variable to store global id of boundary layer specified in the input file


			// Method 1: INPUT by NeumannSet
			if(m_NeumannSet != -1 && this_setVec.size() != 0){
				for (int ii=0; ii<this_setVec.size(); ii++) {
					this_set = this_setVec[ii];
					int set_id;
					MBERRCHK(mb->tag_get_data(NTag, &this_set, 1, &set_id), mb);
					moab::Range tmpQuads;
					MBERRCHK(mb->get_entities_by_dimension(this_set, m_BLDim, tmpQuads, true),mb);
					quads.merge(tmpQuads);
					if (set_id < 0) {
						quadsNeg.merge(tmpQuads);
					}
					else {
						quadsPos.merge(tmpQuads);
					}
					if (quads.size() <=0){
							m_LogFile <<  " No quads found, aborting.. " << std::endl;
							throw MeshKit::Error(MeshKit::MK_FAILURE, "");
					}
				}
...
//					get_normal_quad (old_hex_conn, surf_normal);
			...
			for (Range::iterator kter = nodes.begin(); kter != nodes.end(); ++kter){
					...
					if(all_bl[count] == 0){
							...
							moab::Range adj_for_normPos = intersect(quadsPos, adj_for_normal);
							moab::Range adj_for_normNeg = intersect(quadsNeg, adj_for_normal);
...
//													v=-v;
//											}
											if (adj_for_normPos.find(*qter) == adj_for_normPos.end()) {
												v = -v;
											}
											//int tagData;
											std::vector< moab::Tag > tags;
											MBERRCHK(mb->tag_get_tags_on_entity(*qter, tags), mb);
											m_LogFile << "tags.size() = " << tags.size() << std::endl;
											std::string name;
											MBERRCHK(mb->tag_get_name(tags[0], name), mb);
											m_LogFile << "name = " << name << std::endl;
//									}
//									else if(m_GD==2){
//											if(sense == 1){
...
//									}
	...
	void dtPostBL::PrepareIO (int argc, char *argv[], std::string  TestDir)
	// ---------------------------------------------------------------------------
	//! Function: Parser for reading the dtPostBL specification (.inp) file. \n
...
//							if(m_Thickness < 0 || szFormatString.fail())
	...
	void dtPostBL::IOErrorHandler (ErrorStates ECode) const
	// ---------------------------------------------------------------------------
	...
	void dtPostBL::get_normal_edge (std::vector<EntityHandle>conn, moab::CartVect BC, moab::CartVect &v)
	// ---------------------------------------------------------------------------
	...
	void dtPostBL::find_min_edge_length(moab::Range adj_qn, moab::EntityHandle node , moab::Range bl_nodes, double &e_len)
	// ---------------------------------------------------------------------------
	...
	void dtPostBL::get_det_jacobian(std::vector<moab::EntityHandle> conn, int offset, double &AvgJ)
	// ---------------------------------------------------------------------------
