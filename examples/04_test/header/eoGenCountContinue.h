...
#ifndef _eoGenCountContinue_h
#define _eoGenCountContinue_h
...
*/
template< class EOT>
class eoGenCountContinue: public eoCountContinue<EOT>, public eoValueParam<unsigned>
{
public:
  ...
  /// Ctor for setting a
  eoGenCountContinue( unsigned long _totalGens)
          : eoCountContinue<EOT>( ),
  ...
  eoGenCountContinue( unsigned long _totalGens, unsigned long& _currentGen)
  ...
  virtual bool operator() ( const eoPop<EOT>& _vEO ) {
    ...
    if (thisGeneration >= repTotalGenerations)
      {
	  std::cout << "STOP in eoGenCountContinue: Reached maximum number of generations [" << thisGeneration << "/" << repTotalGenerations << "]\n";
	  //eo::log << eo::logging << "STOP in eoGenContinue: Reached maximum number of generations [" << thisGeneration << "/" << repTotalGenerations << "]\n";
	  ofstream outFileF ("Testdaten2.txt" , ios::app);
	  outFileF << " | " << thisGeneration << " | " << "too many Generations" << " | ";
	  outFileF.close();
        return false;
      }
    return true;
